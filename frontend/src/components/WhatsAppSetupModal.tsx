...
import {
  MessageCircle, X, CheckCircle, AlertCircle, ChevronRight,
  Phone, Key, Send, Loader, Copy, ExternalLink, Smartphone
} from 'lucide-react';
...
export function useWhatsAppConfig() {
  const [config, setConfig] = useState<WhatsAppConfig>({ configured: false, phone: null });
...
export function WhatsAppSetupModal({ onClose, onSuccess }: WhatsAppSetupModalProps) {
  ...
  const handleSetup = async () => {
    ...
    if (!phone.trim().startsWith('+')) {
      setError('Phone number must start with + and country code. Example: +919876543210');
      return;
    }
    if (!apiKey.trim()) {
    ...
    try {
      ...
      if (!testRes.ok) {
        ...
        if (typeof detail === 'object') {
          throw new Error(detail.troubleshoot?.[0] || detail.error || 'Test message failed');
        }
        ...
        <div style={{
          ...
          {[
            ...
            <div key={n} style={{
              ...
              borderBottom: step >= n ? '2px solid #25D366' : '2px solid transparent',
          ...
          {step === 1 && (
            <div>
              <div style={{
                ...
                <Smartphone size={20} color="#25D366" style={{ flexShrink: 0, marginTop: 2 }} />
                <div style={{ fontSize: 13, color: '#94A3B8', lineHeight: 1.6 }}>
                  <strong style={{ color: '#E2E8F0' }}>CallMeBot</strong> — bilkul free WhatsApp API.
                  Koi Twilio account ya credit card ki zarurat nahi!
                  Sirf ek WhatsApp message bhejne se activate hota hai.
                </div>
              </div>

              <div style={{ marginBottom: 16 }}>
                <div style={{ color: '#64748B', fontSize: 12, fontWeight: 700, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>
                  Step 1: Yeh number WhatsApp pe save karo
                </div>
                <div style={{
                  background: '#111622', border: '1px solid #1E293B',
                  borderRadius: 10, padding: '12px 16px',
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                }}>
                  <div>
                    <div style={{ color: '#25D366', fontWeight: 800, fontSize: 18, fontFamily: 'monospace' }}>
                      {CALLMEBOT_NUMBER}
                    </div>
                    <div style={{ color: '#475569', fontSize: 11, marginTop: 4 }}>Save as "CallMeBot" in contacts</div>
                  </div>
                  <button onClick={() => copyText(CALLMEBOT_NUMBER)} style={{
                    background: 'rgba(37,211,102,0.1)', border: '1px solid rgba(37,211,102,0.3)',
                    color: '#25D366', borderRadius: 8, padding: '6px 10px',
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{
                  ...
                  <div style={{
                    color: '#E2E8F0', fontStyle: 'italic', fontSize: 14,
                    fontFamily: 'monospace',
                  }}>
                    "{CALLMEBOT_MSG}"
                  </div>
                  <button onClick={() => copyText(CALLMEBOT_MSG)} style={{
                    background: 'rgba(37,211,102,0.1)', border: '1px solid rgba(37,211,102,0.3)',
                    color: '#25D366', borderRadius: 8, padding: '6px 10px',
                    cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0, marginLeft: 8,
                  }}>
                    <Copy size={12} /> Copy
                  </button>
                </div>
              ...
              <div style={{
                ...
                fontSize: 12, color: '#94A3B8', display: 'flex', gap: 8,
              }}>
                <AlertCircle size={14} color="#60A5FA" style={{ flexShrink: 0, marginTop: 1 }} />
                <span>
                  <strong style={{ color: '#93C5FD' }}>Message bhejne ke baad</strong>{' '}
              ...
              <a
                ...
                style={{
                  ...
                  color: '#25D366', fontSize: 12, marginBottom: 20,
                  textDecoration: 'none',
                }}
              >
            ...
            <div>
              <div style={{ marginBottom: 18 }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    background: '#111622', border: `1px solid ${phone && !phone.startsWith('+') ? '#EF4444' : '#1E293B'}`,
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => { setStep(1); setError(''); }} style={{
                  ...
                  color: '#64748B', borderRadius: 10, fontSize: 14, cursor: 'pointer',
                }}>
                  ← Back
                </button>
                <button
                  ...
                  style={{
                    ...
                    background: loading || !phone || !apiKey
                      ? 'rgba(37,211,102,0.3)'
                ...
                >
                  {loading ? (
                    <><Loader size={16} style={{ animation: 'spin 1s linear infinite' }} /> Sending test...</>
                  ) : (
          ...
          {step === 3 && success && (
            <div style={{ textAlign: 'center', padding: '10px 0' }}>
              ...
              <div style={{ color: '#94A3B8', fontSize: 14, marginBottom: 24, lineHeight: 1.6 }}>
                ...
                pe! Ab jab bhi AI agent CRITICAL risk detect karega,
                aapko turant WhatsApp alert milega.
              </div>
              ...
              <div style={{
                ...
                <div style={{ color: '#64748B', fontSize: 11, fontWeight: 700, marginBottom: 10, textTransform: 'uppercase', letterSpacing: 1 }}>
...
export function WhatsAppBadge({ onClick }: WhatsAppBadgeProps) {
  ...
  return (
    <button
      onClick={onClick}
      title={config.configured ? `WhatsApp alerts active: ${config.phone}` : 'Setup WhatsApp alerts'}
      style={{
    ...
    >
      <MessageCircle size={13} color={config.configured ? '#25D366' : '#475569'} />
      <span style={{
        fontSize: 11, fontWeight: 700,
