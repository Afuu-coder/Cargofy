...
async function parseCommand(query: string): Promise<CommandResult> {
  ...
  // Product detection
  if (q.includes('milk') || q.includes('dairy')) result.filters.product = 'dairy';
  else if (q.includes('seafood') || q.includes('fish')) result.filters.product = 'seafood';
  else if (q.includes('pharma') || q.includes('medicine')) result.filters.product = 'pharma';
  else if (q.includes('frozen')) result.filters.product = 'frozen';
  else if (q.includes('produce') || q.includes('vegetable')) result.filters.product = 'produce';

  // Region detection
  if (q.includes('assam')) { result.filters.region = 'Assam'; result.map_focus = { lat: 26.2006, lng: 92.9376, zoom: 7 }; }
  else if (q.includes('mumbai')) { result.filters.region = 'Mumbai'; result.map_focus = { lat: 19.0760, lng: 72.8777, zoom: 8 }; }
  else if (q.includes('delhi')) { result.filters.region = 'Delhi'; result.map_focus = { lat: 28.6139, lng: 77.2090, zoom: 8 }; }
  else if (q.includes('guwahati')) { result.filters.region = 'Guwahati'; result.map_focus = { lat: 26.1445, lng: 91.7362, zoom: 9 }; }

  // Risk detection
  if (q.includes('critical')) result.filters.risk = 'CRITICAL';
  else if (q.includes('high risk') || q.includes('high-risk')) result.filters.risk = 'HIGH';
  else if (q.includes('breach') || q.includes('breaches')) result.filters.risk = 'HIGH';

  // Status detection
  if (q.includes('delay') || q.includes('late')) result.filters.status = 'delayed';
  else if (q.includes('arriving') || q.includes('next 2 hours')) result.filters.status = 'arriving_soon';
    ...
    <div style={{ position: 'relative', zIndex: 100 }}>
      ...
      >
        ...
        <input
          ref={inputRef}
          value={query}
          onChange={e => { setQuery(e.target.value); if (e.target.value) setShowExamples(false); }}
        ...
        {/* Shortcut hint */}
        {!query && (
          <span style={{ fontSize: '11px', color: 'rgba(148,163,184,0.6)', whiteSpace: 'nowrap' }}>
            ⌘K
          </span>
        )}
          ...
          {activeResult && (
            <motion.button
              ...
              style={{
                ...
                color: '#F87171', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '4px',
              }}
            >
              <X size={12} /> Clear
            </motion.button>
        ...
        {activeResult && (
          ...
          >
            ...
            {activeResult.map_focus && (
              <span style={{ marginLeft: '4px', color: 'rgba(196,181,253,0.6)' }}>
      ...
      <AnimatePresence>
        {showExamples && (
          ...
          >
            <div style={{ padding: '10px 14px 6px', fontSize: '11px', color: 'rgba(148,163,184,0.6)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
              Suggested Queries
            </div>
