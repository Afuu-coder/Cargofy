...
const DEMO_TRUCKS: TruckPosition[] = [
  ...
  },
  {
    ...
    temperature: 6.1,
    time_to_spoil: 145,
    driver_name: "Suresh Pandey",
    speed_kmh: 68,
  },
  {
    shipment_code: "SHP-KA-003",
    product_type: "fish",
    lat: 12.9716,
    lng: 77.5946,
    risk_score: 0.15,
    ...
    temperature: 1.8,
    time_to_spoil: 310,
    driver_name: "Arun Verma",
    speed_kmh: 45,
  },
  {
    shipment_code: "SHP-TN-004",
    product_type: "fruits",
    lat: 13.0827,
    lng: 80.2707,
    risk_score: 0.71,
...
export const CargoMap3D = forwardRef<CargoMap3DHandle, CargoMap3DProps>(
  function CargoMap3DInner(
    { trucks, mapboxToken, apiBase = "", onRerouteAlert },
    ref,
  ) {
    ...
    const [truckData, setTruckData] = useState<TruckPosition[]>(
      trucks || DEMO_TRUCKS,
    );
        ...
        map.addSource("india-routes", {
          ...
          data: {
            ...
            features: [
              {
                type: "Feature",
                properties: { name: "Mumbai-Delhi NH48" },
                geometry: {
                  type: "LineString",
                  coordinates: [
                    [72.8777, 19.076],
              ...
              {
                type: "Feature",
                properties: { name: "Bangalore-Chennai NH44" },
                geometry: {
                  type: "LineString",
                  coordinates: [
                    [77.5946, 12.9716],
              ...
              {
                type: "Feature",
                properties: { name: "Delhi-Kolkata NH19" },
                geometry: {
                  type: "LineString",
                  coordinates: [
                    [77.209, 28.6139],
    ...
    useEffect(() => {
      ...
      const updateMarkers = () => {
        ...
        const auraSource = map.getSource("risk-auras") as
        ...
        if (auraSource) {
          const auraFeatures = truckData
            .filter(
              (t) =>
                t.risk_category === "CRITICAL" || t.risk_category === "HIGH",
            )
            .map((t) => ({
            ...
            try {
              ...
              if (data.shipment_id && data.event === "REROUTE_DECISION") {
                const truck = truckData.find(
                  (t) => t.shipment_code === data.shipment_id,
                );
                    ...
                    prev.map((t) =>
                      t.shipment_code === data.shipment_id
                        ? {
                            ...t,
                            risk_category: "CRITICAL",
                            risk_score: data.risk_score ?? 0.9,
                          }
                        : t,
                    ),
                  );
    ...
    const focusTruck = useCallback((shipmentCode: string) => {
      const map = mapRef.current;
      if (!map) return;
      const truck = truckDataRef.current.find(
        (t) => t.shipment_code === shipmentCode,
      );
      if (!truck) return;
          ...
          >
            <span
              style={{
                ...
                animation:
                  wsStatus === "live" ? "cargofy-pulse 2s infinite" : "none",
              }}
            />
            {statusLabel}
            {wsStatus === "live" && (
