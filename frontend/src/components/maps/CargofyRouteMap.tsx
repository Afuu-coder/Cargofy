  ...
  useEffect(() => {
    ...
    if (hasOrigin) {
      ...
      const m = new mapboxgl.Marker({ element: el, anchor: "bottom" })
        ...
        .setPopup(
          new mapboxgl.Popup({ offset: 30, className: "cargofy-popup" })
    ...
    if (routeData?.route_geometry) {
      ...
      if (geom.coordinates.length > 1) {
        const mid = geom.coordinates[
          Math.floor(geom.coordinates.length / 2)
        ] as [number, number];
        const el = document.createElement("div");
    ...
    routeData?.cold_hubs_on_route?.forEach((hub) => {
      ...
      const m = new mapboxgl.Marker({ element: el, anchor: "center" })
        ...
        .setPopup(
          new mapboxgl.Popup({ offset: 20, className: "cargofy-popup" })
            ...
            routeData.cold_hubs_on_route.length > 0 && (
              <>
                ...
                <StatPill
                  icon="❄️"
                  label="Cold Hubs"
                  value={`${routeData.cold_hubs_on_route.length} nearby`}
                  valueColor="#4DD9AC"
                />
              </>
            )}
...
function StatPill({
  ...
  return (
    ...
    >
      ...
      <div style={{ color: valueColor, fontSize: 12, fontWeight: 700 }}>
        {value}
      </div>
    </div>
  );
