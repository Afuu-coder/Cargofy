...
function zoneTemp(base: number, zoneId: string): number {
  const offsets: Record<string, number> = {
    ...
    RL: +1.1,
    RC: +0.8,
    RR: +1.4, // rear always warmer (door heat)
  };
    ...
    () =>
      ZONES.map((z) => {
        ...
        return {
          ...
          isBreach,
          isCold,
          color: tempToColor(t, tempMin, tempMax),
        };
      }),
  ...
  return (
    <div className="bg-[#080B14] border border-[#1E2530] rounded-xl p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-[9px] text-[#4A5568] mt-0.5">
            {productType ? `${productType.toUpperCase()} · ` : ""}Safe range:{" "}
            {tempMin}–{tempMax}°C · 6-zone cross-section
          </div>
        </div>
            ...
            return (
              <g key={z.id}>
                <motion.rect
                  ...
                  transition={
                    z.isBreach ? { repeat: Infinity, duration: 1.1 } : undefined
                  }
                />
                {/* Zone ID */}
                <text
                ...
                <text
                  ...
                  fontSize="14"
                  fontWeight="bold"
                  fill={z.isBreach ? "#fff" : "rgba(255,255,255,0.9)"}
                  fontFamily="monospace"
                >
                  {z.temp}°
                </text>
      ...
      <div className="flex items-center gap-3 text-[9px] text-[#4A5568]">
        ...
        <div className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-sm inline-block bg-[#34D399]" />
          Safe
        </div>
        <div className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-sm inline-block bg-[#FBBF24]" />
          Warming
        </div>
        <div className="flex items-center gap-1">
