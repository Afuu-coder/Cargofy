...
const SIMULATED_EVENTS: Omit<SwarmEvent, 'id' | 'timestamp'>[] = [
  { agent: 'Risk Agent',        action: 'Computed risk score 87/100 for cargo SHP-004',          shipment_code: 'SHP-004', severity: 'critical', icon: '🔴' },
  { agent: 'WhatsApp Bot',      action: 'Pinged driver +91-9876-123456 for Temp Breach ACK',    shipment_code: 'SHP-007', severity: 'warning',  icon: '💬' },
  { agent: 'Route Optimizer',   action: 'Found alternate NH-27 route — saves 22 min ETA',       shipment_code: 'SHP-004', severity: 'success',  icon: '🗺️' },
  { agent: 'Blockchain Oracle', action: 'SLA contract 0xA3f...c12 updated — penalty flagged',   shipment_code: 'SHP-002', severity: 'warning',  icon: '⛓️' },
  { agent: 'Spoilage Predictor',action: 'Milk cargo SHP-011: 94 min to spoilage threshold',     shipment_code: 'SHP-011', severity: 'critical', icon: '🧪' },
  { agent: 'Fleet Monitor',     action: 'Vehicle MH-12 compressor anomaly detected — flagged',  shipment_code: 'SHP-009', severity: 'warning',  icon: '🚛' },
  { agent: 'Risk Agent',        action: 'Batch re-scored 14 shipments after weather API update', severity: 'info',    icon: '⚡' },
  { agent: 'WhatsApp Bot',      action: 'Negotiated emergency cold storage at Meghalaya Hub',    severity: 'success',  icon: '🤝' },
  { agent: 'Route Optimizer',   action: 'Analyzing congestion on NH-37 near Jorhat...',          severity: 'info',    icon: '📡' },
  { agent: 'Blockchain Oracle', action: 'Deployed SLA contract for SHP-017 on Sepolia testnet', shipment_code: 'SHP-017', severity: 'success', icon: '✅' },
];
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontSize: '13px', fontWeight: 600, color: '#C7D2FE', letterSpacing: '0.03em' }}>
            Swarm Activity
          </span>
          {/* Live pulse */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          {collapsed ? <ChevronDown size={14} color="#64748B" /> : <ChevronUp size={14} color="#64748B" />}
        </div>
      </div>
                ...
                return (
                  <motion.div
                    ...
                    style={{
                      ...
                      padding: '8px 14px',
                      background: i === 0 && newCount > 0 ? sev.bg : 'transparent',
                      borderBottom: '1px solid rgba(30,41,59,0.5)',
                      transition: 'background 0.3s',
                    }}
                  >
                    {/* Icon */}
                    <span style={{ fontSize: '16px', lineHeight: 1, flexShrink: 0 }}>{evt.icon}</span>
