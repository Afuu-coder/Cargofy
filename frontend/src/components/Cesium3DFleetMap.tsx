...
export function Cesium3DFleetMap({ shipments, onShipmentClick, className = '' }: Props) {
  ...
  return (
    <div className={`relative ${className}`}>
      <div
        ref={mapContainer}
        style={{ width: '100%', height: '100%', minHeight: '300px', background: '#080B12' }}
      />
      ...
      {mapReady && shipments.length > 0 && shipsWithCoords.length === 0 && (
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-10">
          <div className="bg-[#0D1117]/90 border border-[#1E2530] rounded-xl px-5 py-4 text-center backdrop-blur-sm">
            ...
            <div className="text-sm text-[#94A3B8] font-medium">No GPS coordinates yet</div>
            <div className="text-xs text-[#4A5568] mt-1">Truck markers appear once IoT/GPS data arrives</div>
          </div>
        </div>
      )}
      ...
      {loadErr && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-[#080B12] z-10 pointer-events-none">
          ...
          <div className="text-xs text-[#F87171] font-medium">Map failed to load</div>
        </div>
      )}
