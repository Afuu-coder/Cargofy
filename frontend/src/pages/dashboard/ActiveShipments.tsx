...
function getContractAddress(code: string): string {
  ...
  return (
    "0x" +
    h.toString(16).padStart(8, "a") +
    "...c" +
    (h % 999).toString().padStart(3, "0")
  );
}
...
function getRisk(s: Shipment): RiskLevel {
  ...
  if (cat === "CRITICAL" || cat === "HIGH" || cat === "MEDIUM" || cat === "LOW")
    return cat as RiskLevel;
  return "LOW";
}
...
function formatTemp(s: Shipment) {
  const r = getRisk(s);
  const col =
    r === "CRITICAL" ? "#F87171" : r === "HIGH" ? "#FBBF24" : "#34D399";
...
export function ActiveShipments() {
  ...
  const filtered = displayShipments
    .filter((s) => {
      ...
      if (
        ...
        !filterProduct.includes(s.product_type?.toLowerCase())
      )
        return false;
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "spoil")
        return (
          (a.current_risk?.time_to_spoil_minutes || 9999) -
          (b.current_risk?.time_to_spoil_minutes || 9999)
        );
      if (sortBy === "id")
        return a.shipment_code.localeCompare(b.shipment_code);
      return 0;
    });
  ...
  const chips: Array<{ label: string; clear: () => void }> = [
    ...filterRisk.map((r) => ({
      label: `Risk: ${r}`,
      clear: () => setFilterRisk((f) => f.filter((x) => x !== r)),
    })),
    ...filterProduct.map((p) => ({
      label: `Product: ${p}`,
      clear: () => setFilterProduct((f) => f.filter((x) => x !== p)),
    })),
    ...(search
  ...
  function exportCSV() {
    ...
    const csv = [
      "Shipment ID,Product,Origin,Destination,Risk,Status,Blockchain,AI Failure Risk",
    ]
      .concat(
        rows.map(
          (s) =>
            `${s.shipment_code},${s.product_type},${s.origin || ""},${s.destination || ""},${getRisk(s)},${s.status},${getBlockchainStatus(s)},${aiPredictions[s.id]?.failureRisk ?? "N/A"}%`,
        ),
      )
      .join("\n");
    ...
    a.download = "cargofy_shipments.csv";
    a.click();
  ...
  async function handleAIPredict() {
    ...
    setAiLoading(false);
    addToast(
      `✅ AI sorted ${Object.keys(predictions).length} trucks by failure probability`,
    );
  }
    ...
    >
      ...
      <div className="fixed top-4 right-4 z-[100] flex flex-col gap-2 pointer-events-none">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`px-4 py-3 rounded-lg text-sm font-medium shadow-xl animate-slide-in ${t.type === "ok" ? "bg-[#0D2B22] border border-[#4DD9AC]/40 text-[#4DD9AC]" : "bg-[#2B0D0D] border border-[#EF4444]/40 text-[#F87171]"}`}
          >
            {t.msg}
          </div>
        ))}
      ...
      {ctxMenu && (
        <div
          className="fixed z-50 bg-[#0D1117] border border-[#1E2530] rounded-lg shadow-2xl py-1 min-w-[200px]"
          ...
          {[
            {
              ...
              action: () => navigate(`/shipments/${ctxMenu.ship.id}`),
            },
            {
              icon: <Map size={14} />,
            ...
            null,
            {
              ...
              action: () => {
                addToast(
                  `Alert sent to driver on ${ctxMenu.ship.shipment_code}`,
                );
                setCtxMenu(null);
              },
            },
            {
              ...
              label: "Send WhatsApp",
              action: () => {
                addToast(`WhatsApp sent for ${ctxMenu.ship.shipment_code}`);
                setCtxMenu(null);
              },
            },
            null,
            {
              ...
              action: () => {
                addToast("Note modal coming soon");
                setCtxMenu(null);
              },
            },
            {
            ...
            null,
            {
              icon: <CheckCircle size={14} className="text-[#34D399]" />,
              label: "Mark as Delivered",
              action: () => {
                addToast(`${ctxMenu.ship.shipment_code} marked delivered`);
                setCtxMenu(null);
              },
            },
            {
      ...
      <header className="shrink-0 h-14 bg-[#0A0D14] border-b border-[#1E2530] flex items-center px-4 gap-4 z-40">
        <div
          className="text-[#4DD9AC] font-black text-xl tracking-tighter font-mono cursor-pointer"
        ...
        <div className="relative flex-1 max-w-sm hidden md:block group">
          <Search
            size={16}
            className="absolute left-3 top-1/2 -translate-y-1/2 text-[#64748B] group-focus-within:text-[#4DD9AC] transition-colors"
          />
          <input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search shipment, product, route..."
        ...
        <button
          ...
          className="bg-[#4DD9AC] text-[#003829] font-bold text-xs px-4 py-2 rounded hover:bg-[#6EF6C7] active:scale-95 transition-all flex items-center gap-1.5"
        >
          <span>+</span>
        ...
        <button
          ...
          className="w-8 h-8 rounded-full bg-[#4DD9AC]/20 border border-[#4DD9AC]/30 flex items-center justify-center text-[#4DD9AC] text-sm font-bold"
          title="Logout"
        >
          R
        </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="shrink-0 w-[220px] bg-[#0D1117] border-r border-[#1E2530] flex-col hidden md:flex overflow-y-auto">
          <nav className="flex-1 py-3 space-y-0.5">
            {[
              ...
              },
              {
                ...
                href: "/risk",
                badge: critical.length + high.length || undefined,
                badgeRed: true,
              },
              {
                icon: <Zap size={18} />,
                label: "Mobile IoT Node",
                href: "/mobile",
              },
              ...
              item === null ? (
                ...
                >
                  ...
                  {item.badge !== undefined && item.badge > 0 && (
                    <span
                      className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${item.badgeRed ? "bg-[#EF4444]/20 text-[#F87171]" : "bg-[#60A5FA]/20 text-[#60A5FA]"}`}
                    >
                      {item.badge}
                    </span>
                  )}
          ...
          <div className="p-3 border-t border-[#1E2530]">
            ...
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-[#4DD9AC]/20 border border-[#4DD9AC]/40 flex items-center justify-center text-[#4DD9AC] text-xs font-bold">
                R
              </div>
              <div>
        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="shrink-0 bg-[#0D1117] border-b border-[#1E2530] px-6 py-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h1 className="text-lg font-bold text-[#F1F5F9] flex items-center gap-3">
                  Active Shipments
                  <span className="text-sm font-mono bg-[#1E2530] text-[#94A3B8] px-2 py-0.5 rounded-full">
                ...
                <div className="flex items-center gap-4 mt-1 text-sm">
                  ...
                  <span className="text-[#FBBF24]">
                    {high.length} WATCHLIST
                  </span>
                  <span className="text-[#64748B]">·</span>
                  <span className="text-[#34D399]">
                    {filtered.length - critical.length - high.length} STABLE
                  </span>
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                ...
                {chips.length > 0 && (
                  <span className="w-4 h-4 bg-[#4DD9AC] text-[#003829] rounded-full text-[9px] flex items-center justify-center font-bold">
                    {chips.length}
                  </span>
                )}
              </button>
              ...
              <div className="relative">
                ...
                {showSort && (
                  <div
                    className="absolute top-full left-0 mt-1 bg-[#0D1117] border border-[#1E2530] rounded-lg shadow-2xl z-30 py-1 min-w-[220px]"
                    onClick={(e) => e.stopPropagation()}
                  >
                    {(
                      [
                      ...
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-sm transition-colors flex items-center gap-2 ${
                          sortBy === "ai_predict"
                            ...
                            : "text-[#CBD5E1] hover:bg-[#1E2530]"
                        }`}
                      >
                        <Brain size={13} className="text-[#A855F7]" />
                        🤖 AI Predictive Sort
                        <span className="ml-auto text-[10px] bg-[#A855F7]/20 text-[#A855F7] px-1.5 py-0.5 rounded">
                          BigQuery ML
                        </span>
                      </button>
                    </div>
              ...
              <button
                onClick={() => navigate("/create-shipment")}
                className="flex items-center gap-2 px-3 py-1.5 rounded bg-[#4DD9AC]/10 border border-[#4DD9AC]/30 text-xs font-medium text-[#4DD9AC] hover:bg-[#4DD9AC]/20 transition-all ml-auto"
              >
                + Create Shipment
              </button>
            </div>
            ...
            {chips.length > 0 && (
              <div className="flex flex-wrap items-center gap-2 mt-2">
                {chips.map((c, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-1 bg-[#4DD9AC]/10 border border-[#4DD9AC]/30 text-[#4DD9AC] text-xs px-2 py-0.5 rounded-full"
                  >
                    {c.label}{" "}
                    <button
                      onClick={c.clear}
          ...
          <div className="flex flex-1 overflow-hidden relative">
                    ...
                    <div className="flex-1">
                      ...
                      <span className="text-xs text-[#94A3B8] ml-2">
                        ...
                        <span className="text-[#F87171] font-mono font-bold">
                          {filtered[0]?.vehicle_number ||
                            filtered[0]?.shipment_code ||
                            "—"}
                        </span>{" "}
                        has{" "}
                        <span className="text-[#F87171] font-bold">
                          {(aiPredictions as any)[filtered[0]?.id]
                            ?.failureRisk ?? 0}
                          %
                        </span>{" "}
                        failure risk (
                        {(aiPredictions as any)[filtered[0]?.id]?.failureReason}
                        )
                      </span>
                    </div>
                    <button
                      onClick={clearAIPredict}
                      className="text-[#64748B] hover:text-[#F87171] transition-colors text-lg"
                    >
                      ×
                    </button>
                  </motion.div>
            ...
            {showFilter && (
              <div
                className="shrink-0 w-72 bg-[#0D1117] border-r border-[#1E2530] overflow-y-auto p-5 space-y-5"
                onClick={(e) => e.stopPropagation()}
              >
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    ...
                    className="flex-1 py-2 text-xs border border-[#1E2530] text-[#64748B] hover:text-white rounded-lg transition-colors"
                  >
                    Clear All
                  </button>
                  <button
                    onClick={() => setShowFilter(false)}
                    className="flex-1 py-2 text-xs bg-[#4DD9AC] text-[#003829] font-bold rounded-lg hover:bg-[#6EF6C7] transition-colors"
                  >
                    Apply
                  </button>
                </div>
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-40 text-[#64748B]">
                  ...
                  <button
                    ...
                    className="mt-2 text-[#4DD9AC] text-sm underline"
                  >
                    Clear filters
                  </button>
                </div>
          ...
          {selected.size > 0 && (
            <div className="shrink-0 bg-[#0D231A] border-t border-[#4DD9AC]/30 px-6 py-3 flex items-center gap-3">
              <span className="text-sm text-[#4DD9AC] font-semibold shrink-0 flex items-center gap-1.5">
                <CheckCircle size={16} /> {selected.size} selected
              </span>
                ...
                {[
                  {
                    ...
                    l: "Alert All Drivers",
                    a: () => {
                      addToast(`Alerts sent to ${selected.size} drivers`);
                      setSelected(new Set());
                    },
                  },
                  {
                    ...
                    l: "Escalate All",
                    a: () => {
                      addToast(`${selected.size} shipments escalated`, "warn");
                      setSelected(new Set());
                    },
                  },
                  {
                  ...
                  },
                  {
                    ...
                    l: "Mark Delivered",
                    a: () => {
                      addToast(`${selected.size} shipments marked delivered`);
                      setSelected(new Set());
                    },
                  },
                ].map((btn) => (
                  <button
                    key={btn.l}
                    onClick={btn.a}
                    className="flex items-center gap-1.5 text-xs bg-[#111827] border border-[#1E2530] text-[#CBD5E1] hover:border-[#4DD9AC]/40 hover:text-[#4DD9AC] px-3 py-1.5 rounded transition-all"
                  >
                    {btn.icon}
                    {btn.l}
                  </button>
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs text-[#64748B] hover:text-[#F87171] transition-colors shrink-0"
              >
                <X size={14} /> Cancel
              </button>
            </div>
              ...
              <tr
                ...
                style={{
                  background: isSelected
                    ? "rgba(77,217,172,0.05)"
                    : rm.rowBg || undefined,
                  borderLeft: `3px solid ${isHovered || isSelected ? rm.border : "transparent"}`,
                }}
                ...
                onContextMenu={(e) => {
                  e.preventDefault();
                  setCtxMenu({ x: e.clientX, y: e.clientY, ship: s });
                }}
              >
                <td
                ...
                <td className="px-3 py-3.5">
                  ...
                  <span className="text-xs text-[#CBD5E1] ml-1.5 capitalize">
                    {s.product_type}
                  </span>
                </td>
                <td className="px-3 py-3.5">
                ...
                <td className="px-3 py-3.5">
                  <span
                    className={`text-sm font-mono font-bold ${risk === "CRITICAL" ? "text-[#F87171]" : risk === "HIGH" ? "text-[#FBBF24]" : "text-[#CBD5E1]"}`}
                  >
                ...
                <td className="px-3 py-3.5">
                  {risk === "CRITICAL" ? (
                    <span className="flex items-center gap-1 w-max text-[11px] font-bold text-[#F87171] bg-[#EF4444]/10 px-2 py-0.5 rounded">
                      <ShieldAlert size={12} /> AT RISK
                    </span>
                  ) : (
                    <span className="flex items-center gap-1 w-max text-[11px] text-[#34D399] bg-[#34D399]/10 px-2 py-0.5 rounded">
                ...
                >
                  {isHovered ? (
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => navigate(`/shipments/${s.id}`)}
                        className="text-[10px] text-[#60A5FA] hover:text-white bg-[#60A5FA]/10 px-1.5 py-0.5 rounded transition-colors"
                      >
                        Track
                      </button>
                      <button
                        onClick={() =>
                          addToast(`Alert sent for ${s.shipment_code}`)
                        }
                        className="text-[10px] text-[#FBBF24] hover:text-white bg-[#FBBF24]/10 px-1.5 py-0.5 rounded transition-colors"
                      >
                        Alert
                      </button>
                      <button
                        ...
                        className="text-[#64748B] hover:text-white transition-colors px-1"
                      >
                    ...
                    <button
                      ...
                      className="text-[#64748B] hover:text-white transition-colors px-2"
                    >
                      <MoreVertical size={16} />
                    </button>
                  )}
...
function BoardView({
  ...
  filtered.forEach((s) => {
    ...
    if (risk === "CRITICAL" || risk === "HIGH") {
      colMap["watchlist"].push(s);
      return;
    }
    colMap[s.status] = colMap[s.status] || [];
    (
      colMap[s.status] ||
      ((colMap["active"] = colMap["active"] || []) && colMap["active"])
    ).push(s);
  });
    ...
    <div className="flex gap-3 p-4 overflow-x-auto h-full">
      {BOARD_COLS.map((col) => {
        ...
        return (
          <div
            key={col.key}
            className="shrink-0 w-60 bg-[#0D1117] rounded-xl border border-[#1E2530] flex flex-col"
          >
            <div className="px-3 py-2.5 border-b border-[#1E2530] flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[#1E2530] text-[#94A3B8]">
                {cards.length}
              </span>
            </div>
            <div className="flex-1 overflow-y-auto p-2 space-y-2">
              {cards.length === 0 ? (
                <div className="text-center text-[#4A5568] text-xs py-6">
                  No shipments
                </div>
              ) : (
                cards.map((s) => (
      ...
      <div className="p-3">
        ...
        {s.current_risk && (
          <div className="border-t border-[#1E2530] pt-2 space-y-1">
            <div className="flex items-center justify-between text-xs">
              ...
              <span
                className={`font-mono font-bold ${risk === "CRITICAL" ? "text-[#F87171]" : risk === "HIGH" ? "text-[#FBBF24]" : "text-[#CBD5E1]"}`}
              >
                {formatSpoil(s.current_risk.time_to_spoil_minutes)}
              </span>
            </div>
            {s.vehicle_number && (
              <div className="flex items-center gap-1.5 text-[10px] text-[#64748B] truncate">
                <Truck size={12} /> {s.vehicle_number}
              </div>
            )}
          </div>
        )}
          ...
          {(() => {
            ...
            if (status === "BREACH")
              return (
                <div
                  title={addr}
                  className="flex items-center gap-1.5 text-[10px] font-bold text-[#F87171] bg-[#EF4444]/10 border border-[#EF4444]/30 px-2 py-1 rounded-lg w-full justify-center animate-pulse"
                >
            ...
            if (status === "PENDING")
              return (
                <div
                  title={addr}
                  className="flex items-center gap-1.5 text-[10px] text-[#FBBF24] bg-[#F59E0B]/10 border border-[#F59E0B]/30 px-2 py-1 rounded-lg w-full justify-center"
                >
                  ⏳ Deploying Contract…
                </div>
              );
            return (
              <div
                title={addr}
                className="flex items-center gap-1.5 text-[10px] font-bold text-[#A78BFA] bg-[#7C3AED]/10 border border-[#7C3AED]/30 px-2 py-1 rounded-lg w-full justify-center"
              >
        ...
        <div className="flex gap-1.5 mt-2.5 border-t border-[#1E2530] pt-2">
          <button
            ...
            className="flex-1 text-[10px] bg-[#1E2530] text-[#94A3B8] hover:text-white py-1 rounded transition-colors"
          >
            Track
          </button>
          <button
            ...
            className="flex-1 text-[10px] bg-[#FBBF24]/10 text-[#FBBF24] hover:bg-[#FBBF24]/20 py-1 rounded transition-colors"
          >
            Alert
          </button>
        </div>
      ...
      {ROUTE_CLUSTERS.map((region) => (
        <div key={region.region}>
          <div className="flex items-center gap-3 mb-3">
            <span className="text-sm font-bold text-[#F1F5F9] uppercase tracking-widest">
              {region.region}
            </span>
          ...
          <div className="space-y-2">
            {region.routes.map((route) => {
              ...
              const routeShips = filtered.filter(
                (s) =>
                  route.origins.some((o) =>
                    (s.origin || "").toLowerCase().includes(o),
                  ) &&
                  route.dests.some((d) =>
                    (s.destination || "").toLowerCase().includes(d),
                  ),
              );
              // If no ships on this specific route, show other ships in a catch-all "Other" group
              const isExp = expanded.has(key);
              const critical = routeShips.filter(
                (s) => getRisk(s) === "CRITICAL",
              ).length;
              const high = routeShips.filter(
                (s) => getRisk(s) === "HIGH",
              ).length;
              ...
              return (
                <div
                  key={key}
                  className="bg-[#0D1117] border border-[#1E2530] rounded-xl overflow-hidden"
                >
                  <button
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-xs bg-[#1E2530] text-[#94A3B8] px-2 py-0.5 rounded-full font-mono">
                        {routeShips.length} shipments
                      </span>
                      {critical > 0 && (
                      ...
                      {critical === 0 && high === 0 && (
                        <span className="flex items-center gap-1 text-[11px] text-[#34D399]">
                          <CheckCircle size={12} /> all stable
                        </span>
                      )}
                    </div>
                  </button>
                      ...
                      {routeShips.map((s) => {
                        ...
                        return (
                          <div
                            ...
                            onClick={() => navigate(`/shipments/${s.id}`)}
                          >
                            <span className="font-mono text-sm font-bold text-[#F1F5F9] w-28 shrink-0">
                              {s.shipment_code}
                            </span>
                            <span className="text-sm">
                              {getIcon(s.product_type)}
                            </span>
                            <span className="text-xs text-[#94A3B8] capitalize w-20 shrink-0">
                              {s.product_type}
                            </span>
                            <span
                            ...
                            <div className="ml-auto flex items-center gap-1 text-xs text-[#4DD9AC]">
...
function FilterGroup({
  ...
  return (
    <div>
      <div className="text-[10px] uppercase tracking-widest text-[#64748B] mb-2 font-semibold">
        {label}
      </div>
