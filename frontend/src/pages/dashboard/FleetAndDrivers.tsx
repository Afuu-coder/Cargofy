...
import { 
  CheckCircle, AlertTriangle, XCircle, Info, Truck, User, 
  Droplet, Snowflake, Thermometer, ShieldAlert, Phone, MessageSquare, 
  Briefcase, Activity, Calendar, Box, ActivitySquare, AlertCircle, Wrench,
  MapPin, Bell, Radio, ArrowRight, FileText, Plus, Award,
  Bot, Sparkles, Loader2, TrendingUp, TrendingDown
} from 'lucide-react';
...
interface TripRow { code: string; icon: React.ReactNode; route: string; duration: string; onTime: boolean; delayMin?: number; excursions: number; }
...
const DRIVERS: Driver[] = [
  {
    ...
    tripHistory:[
      {code:'AXN-2041',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'GHY→SHL',duration:'4h 02m',onTime:true,excursions:0},
      {code:'AXN-1998',icon:<ActivitySquare size={14} className="text-[#A78BFA]"/>,route:'BOM→PUN',duration:'2h 18m',onTime:true,excursions:0},
      {code:'AXN-1847',icon:<Snowflake size={14} className="text-[#34D399]"/>,route:'CCU→PAT',duration:'6h 44m',onTime:false,delayMin:18,excursions:1},
      {code:'AXN-1721',icon:<Snowflake size={14} className="text-[#FBBF24]"/>,route:'DEL→JAI',duration:'3h 55m',onTime:true,excursions:0},
      {code:'AXN-1680',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'GHY→SHL',duration:'4h 28m',onTime:true,excursions:0},
    ],
    alertHistory:[
  ...
  },
  {
    ...
    tripHistory:[
      {code:'AXN-2044',icon:<Snowflake size={14} className="text-[#34D399]"/>,route:'CCU→PAT',duration:'5h 10m',onTime:true,excursions:1},
      {code:'AXN-1920',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'GHY→SHL',duration:'4h 12m',onTime:true,excursions:0},
      {code:'AXN-1810',icon:<Snowflake size={14} className="text-[#EF4444]"/>,route:'MUM→PUN',duration:'2h 05m',onTime:true,excursions:0},
      {code:'AXN-1742',icon:<Snowflake size={14} className="text-[#FBBF24]"/>,route:'DEL→JAI',duration:'3h 40m',onTime:false,delayMin:12,excursions:1},
      {code:'AXN-1700',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'GHY→SHL',duration:'4h 22m',onTime:true,excursions:0},
    ],
    alertHistory:[
  ...
  },
  {
    ...
    tripHistory:[
      {code:'AXN-2010',icon:<Snowflake size={14} className="text-[#34D399]"/>,route:'SIL→GUW',duration:'4h 55m',onTime:true,excursions:0},
      {code:'AXN-1905',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'GHY→SHL',duration:'4h 08m',onTime:false,delayMin:9,excursions:1},
      {code:'AXN-1800',icon:<Snowflake size={14} className="text-[#34D399]"/>,route:'CCU→PAT',duration:'5h 30m',onTime:true,excursions:2},
      {code:'AXN-1650',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'GHY→SHL',duration:'4h 20m',onTime:true,excursions:0},
      {code:'AXN-1600',icon:<Snowflake size={14} className="text-[#34D399]"/>,route:'SIL→GUW',duration:'4h 44m',onTime:true,excursions:0},
    ],
    alertHistory:[
  ...
  },
  {
    ...
    products:['Dairy'],
    tripHistory:[
      {code:'AXN-2841',icon:<ActivitySquare size={14} className="text-[#A78BFA]"/>,route:'DIB→ITA',duration:'7h 22m',onTime:false,delayMin:42,excursions:3},
      {code:'AXN-1960',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'GHY→SHL',duration:'4h 50m',onTime:false,delayMin:30,excursions:2},
      {code:'AXN-1820',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'GHY→SHL',duration:'5h 10m',onTime:false,delayMin:25,excursions:1},
    ],
    alertHistory:[
  ...
  },
  {
    ...
    products:['Dairy'],
    tripHistory:[
      {code:'AXN-2099',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'SIL→GUW',duration:'6h 10m',onTime:false,delayMin:55,excursions:4},
      {code:'AXN-1990',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'GHY→SHL',duration:'5h 44m',onTime:false,delayMin:44,excursions:3},
      {code:'AXN-1870',icon:<Snowflake size={14} className="text-[#60A5FA]"/>,route:'SIL→GUW',duration:'6h 00m',onTime:false,delayMin:35,excursions:2},
    ],
    alertHistory:[
...
const BEHAVIOR_DIMENSIONS = [
  { key:'ack',    label:'Alert Response', icon:'⚡', desc:'Alert acknowledgement speed and consistency' },
  { key:'temp',   label:'Temp Discipline',icon:'🌡', desc:'Temperature excursions caused by driver behavior' },
  { key:'time',   label:'Punctuality',    icon:'⏱', desc:'On-time delivery rate and delay minimization' },
  { key:'safety', label:'Safety Score',   icon:'🛡', desc:'Door-open events, harsh braking, reefer handling' },
  { key:'comm',   label:'Communication',  icon:'💬', desc:'WhatsApp response rate, escalation cooperation' },
];

function buildBehaviorScores(d: Driver): Record<string, number> {
  return {
    ack:    Math.min(100, Math.round(d.ackRate * 0.95 + (d.excursions < 2 ? 5 : 0))),
    temp:   Math.min(100, Math.round(100 - d.excursions * 12 - (d.avgDelay > 20 ? 8 : 0))),
    time:   Math.min(100, Math.round(100 - d.avgDelay * 1.5 - d.slaBreachRate * 2)),
...
function getAIVerdicts(d: Driver, scores: Record<string, number>): string[] {
  ...
  if (avg >= 90) verdicts.push(`${first} is a top-tier cold-chain driver. Recommend for priority CRITICAL shipments.`);
  else if (avg >= 75) verdicts.push(`${first} performs well. Minor improvements in temperature discipline recommended.`);
  else verdicts.push(`${first} shows inconsistencies. Recommend refresher training on reefer management.`);
  if (scores.ack < 80) verdicts.push('Alert ACK rate below threshold. Consider mandatory WhatsApp check-in protocol.');
  if (scores.temp < 70) verdicts.push('Repeated temperature excursions. Vehicle reefer calibration check recommended.');
  if (d.excursions === 0) verdicts.push('Zero excursions this period — exceptional temperature discipline.');
  return verdicts;
}
...
function AIBehaviorScore({ driver }: { driver: Driver }) {
  ...
  function runAnalysis() {
    setAnalysing(true); setAnalysed(false); setScores({}); setVerdicts([]);
  ...
  const sc = (s: number) => s >= 85 ? '#34D399' : s >= 65 ? '#FBBF24' : '#F87171';
    ...
    <div className="bg-[#080B12] border border-[#1E3A5F] rounded-xl overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-[#0D1A2E] to-[#080B12] border-b border-[#1E3A5F] flex items-center justify-between">
        <div className="flex items-center gap-2">
          <motion.div animate={{rotate:analysing?360:0}} transition={{repeat:Infinity,duration:2,ease:'linear'}}>
            <Bot size={14} className="text-[#A78BFA]"/>
          </motion.div>
          <div>
            <div className="text-xs font-bold text-[#F1F5F9] flex items-center gap-1.5">
              AI Behavior Analysis
              {analysing && <span className="text-[9px] text-[#A78BFA] animate-pulse">● SCORING</span>}
              {analysed  && <span className="text-[9px] text-[#34D399]">✓ DONE</span>}
            </div>
            <div className="text-[9px] text-[#64748B]">Gemini Pro · {driver.trips} trips analysed</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {avg !== null && (
            <div className="text-center mr-1">
              <div className="text-lg font-black font-mono" style={{color:sc(avg)}}>{avg}</div>
          ...
          <motion.button whileHover={{scale:1.05}} whileTap={{scale:0.95}}
            ...
            className={`flex items-center gap-1 text-[10px] px-2.5 py-1.5 rounded-lg font-bold transition-all ${
              analysing ? 'bg-[#A78BFA]/10 text-[#A78BFA] cursor-wait' : 'bg-[#A78BFA] text-[#030712] hover:brightness-110'
            }`}>
      ...
      <div className="px-4 py-3 space-y-2">
        {Object.keys(scores).length === 0 && !analysing && (
          <div className="text-[10px] text-[#4A5568] text-center py-3 flex flex-col items-center gap-1">
            ...
            Click <strong className="text-[#A78BFA]">AI Score</strong> to analyse this driver across 5 behavioural dimensions
          </div>
        )}
          ...
          return (
            <div key={dim.key}>
              <div className="flex items-center justify-between text-[10px] mb-0.5">
                <span className="text-[#94A3B8]">{dim.icon} {dim.label}</span>
                {s !== undefined && <span className="font-mono font-bold" style={{color:sc(s)}}>{s}/100</span>}
              </div>
              <div className="h-1.5 bg-[#1E2530] rounded-full overflow-hidden">
                {s !== undefined && (
                  <motion.div initial={{width:0}} animate={{width:`${s}%`}} transition={{duration:0.7,ease:'easeOut'}}
                    className="h-full rounded-full" style={{background:`linear-gradient(90deg,${sc(s)}60,${sc(s)})`}}/>
                )}
              </div>
            </div>
          );
    ...
    <div className="flex flex-col h-full overflow-y-auto" style={{fontFamily:'Inter,sans-serif'}}>
      ...
      <div className="shrink-0 px-5 py-4 border-b border-[#1E2530] sticky top-0 bg-[#0A0D14] z-10">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#4DD9AC]/20 to-[#60A5FA]/20 border border-[#1E2530] flex items-center justify-center text-lg"><User size={20} className="text-[#60A5FA]"/></div>
            <div>
        ...
        <div className="text-xs text-[#94A3B8] space-y-1 mt-2">
          <div className="flex items-center gap-2"><Phone size={12}/> {d.phone} · <MessageSquare size={12}/> WhatsApp {d.waVerified ? <CheckCircle size={12} className="text-[#34D399] inline"/>:<XCircle size={12} className="text-[#EF4444] inline"/>}</div>
          <div className="flex items-center gap-2"><MapPin size={12}/> {d.location} · <Calendar size={12}/> Joined {d.joined}</div>
        </div>
      </div>
      ...
      <div className="px-5 py-4 space-y-5">
        ...
        <div className="bg-[#111827] border border-[#1E2530] rounded-xl p-4">
          <div className="text-[10px] text-[#64748B] uppercase tracking-widest font-semibold mb-3 flex items-center gap-2"><Activity size={14}/> Performance Metrics</div>
        ...
        {/* Current trip */}
        <div className="bg-[#111827] border border-[#1E2530] rounded-xl p-4">
          <div className="text-[10px] text-[#64748B] uppercase tracking-widest font-semibold mb-2 flex items-center gap-2"><Truck size={14}/> Current Trip</div>
          {d.activeShip ? (
        ...
        <div className="bg-[#111827] border border-[#1E2530] rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-[#1E2530] text-[10px] text-[#64748B] uppercase tracking-widest font-semibold flex items-center gap-2"><Box size={14}/> Trip History (last {d.tripHistory.length})</div>
          <div className="divide-y divide-[#1E2530]">
            {d.tripHistory.map((t,i)=>(
              <div key={i} className="flex items-center gap-3 px-4 py-2.5 text-xs hover:bg-[#0D1117] transition-colors">
                ...
                <span className="text-lg shrink-0 flex items-center justify-center w-6 h-6">{t.icon}</span>
                <span className="text-[#64748B] w-16 shrink-0">{t.route}</span>
                <span className="text-[#94A3B8]">{t.duration}</span>
                <span className="ml-auto flex items-center gap-1" style={{color:t.onTime?'#34D399':'#FBBF24'}}>{t.onTime?<CheckCircle size={12}/>:<AlertTriangle size={12}/>} {t.onTime?'On time':`+${t.delayMin}m`}</span>
        ...
        <div className="bg-[#111827] border border-[#1E2530] rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-[#1E2530] text-[10px] text-[#64748B] uppercase tracking-widest font-semibold flex items-center gap-2"><Bell size={14}/> Alert Response History</div>
          <div className="divide-y divide-[#1E2530]">
            {d.alertHistory.map((a,i)=>{
              const acked = a.ackMin < 999;
              return (
                <div key={i} className="flex items-center gap-3 px-4 py-2.5 text-xs">
                  <span style={{color:acked?'#34D399':'#EF4444'}}>{acked?<CheckCircle size={14}/>:<XCircle size={14}/>}</span>
        ...
        <div className="flex gap-2 flex-wrap">
          {d.products.map(p=><span key={p} className="text-[10px] bg-[#111827] border border-[#1E2530] text-[#94A3B8] px-2 py-1 rounded-full flex items-center gap-1"><CheckCircle size={10} className="text-[#34D399]"/> {p} certified</span>)}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            {icon:<Phone size={14}/>,label:'Call Driver',    color:'#34D399'},
            {icon:<MessageSquare size={14}/>,label:'WhatsApp',       color:'#4DD9AC'},
            {icon:<ArrowRight size={14}/>,label:'Assign Trip',    color:'#60A5FA'},
            {icon:<AlertTriangle size={14}/>,label:'Flag Driver',    color:'#FBBF24'},
            {icon:<FileText size={14}/>,label:'Perf. Report',   color:'#A78BFA'},
            {icon:<Truck size={14}/>,label:'View Shipment',  color:'#F97316'},
          ].map((b,idx)=>(
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto" style={{fontFamily:'Inter,sans-serif'}}>
      ...
      <div className="shrink-0 px-5 py-4 border-b border-[#1E2530] sticky top-0 bg-[#0A0D14] z-10">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#60A5FA]/20 to-[#A78BFA]/20 border border-[#1E2530] flex items-center justify-center text-lg"><Truck size={20} className="text-[#A78BFA]"/></div>
            <div>
      ...
      <div className="px-5 py-4 space-y-4">
        {/* Specs */}
        <div className="bg-[#111827] border border-[#1E2530] rounded-xl p-4">
          <div className="text-[10px] text-[#64748B] uppercase tracking-widest font-semibold mb-3 flex items-center gap-2"><Wrench size={14}/> Specifications</div>
        ...
        {/* Reefer health */}
        <div className="bg-[#111827] border border-[#1E2530] rounded-xl p-4">
          <div className="text-[10px] text-[#64748B] uppercase tracking-widest font-semibold mb-3 flex items-center gap-2"><Snowflake size={14}/> Reefer Health</div>
          {v.reeferFault ? (
            <div className="text-sm font-bold text-[#EF4444] mb-2 flex items-center gap-2"><AlertTriangle size={16}/> FAULT DETECTED — MAINTENANCE REQUIRED</div>
          ) : v.reeferHealth === 0 ? (
        ...
        <div className={`border rounded-xl p-4 ${v.serviceOverdue?'bg-[#1A0D0D] border-[#EF4444]/30':'bg-[#111827] border-[#1E2530]'}`}>
          <div className="text-[10px] text-[#64748B] uppercase tracking-widest font-semibold mb-2 flex items-center gap-2"><Calendar size={14}/> Maintenance</div>
        ...
        {/* IoT sensor */}
        <div className="bg-[#111827] border border-[#1E2530] rounded-xl p-4">
          <div className="text-[10px] text-[#64748B] uppercase tracking-widest font-semibold mb-3 flex items-center gap-2"><Radio size={14}/> IoT Sensor</div>
          {v.sensorId ? (
            <div className="grid grid-cols-2 gap-2 text-xs">
              {[
        ...
        {/* Trip history */}
        <div className="bg-[#111827] border border-[#1E2530] rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-[#1E2530] text-[10px] text-[#64748B] uppercase tracking-widest font-semibold flex items-center gap-2"><Box size={14}/> Trip History</div>
          <div className="divide-y divide-[#1E2530]">
        ...
        {/* Compatibility */}
        <div className="flex gap-2 flex-wrap">
          {v.compatibility.map(p=><span key={p} className="text-[10px] bg-[#111827] border border-[#1E2530] text-[#4DD9AC] px-2 py-1 rounded-full flex items-center gap-1">{p} <CheckCircle size={10} className="text-[#34D399]"/></span>)}
        </div>
...
function AddDriverModal({ onClose, onAdded }: { onClose: () => void; onAdded?: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      const products = [
        form.dairy && 'Dairy', form.seafood && 'Seafood',
        form.frozen && 'Frozen', form.pharma && 'Pharma',
      ...
      setErrMsg(e?.response?.data?.detail || e.message || 'Failed to add driver');
      setStep('error');
    }
  };
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center backdrop-blur-sm">
      <div className="bg-[#111827] border border-[#1E2530] rounded-2xl w-96 shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1E2530]">
          <div className="text-sm font-bold text-[#F1F5F9] flex items-center gap-2"><Plus size={16}/> Add New Driver</div>
...
function AddVehicleModal({ onClose, onAdded }: { onClose: () => void; onAdded?: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setErrMsg(e?.response?.data?.detail || e.message || 'Failed to add vehicle');
      setStep('error');
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center backdrop-blur-sm">
      <div className="bg-[#111827] border border-[#1E2530] rounded-2xl w-96 shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1E2530]">
          <div className="text-sm font-bold text-[#F1F5F9] flex items-center gap-2"><Plus size={16}/> Add New Vehicle</div>
          <button onClick={onClose} className="text-[#64748B] hover:text-white text-lg">×</button>
        </div>
        {step === 'done' ? (
          <div className="flex flex-col items-center justify-center py-10 gap-3">
            <span className="text-4xl text-[#4DD9AC]"><Truck size={40}/></span>
...
export function FleetAndDrivers() {
  ...
  const mergedVehicles: Vehicle[] = liveVehicles.map(lv => ({
    ...
    capacity: lv.capacity_kg ? `${lv.capacity_kg} kg / ${lv.capacity_liters} L` : '—',
  ...
  {allocGuardVehicle && <SmartAllocationGuard vehicle={allocGuardVehicle} driverId={selDriver?.id} onClose={()=>setAllocGuardVehicle(null)}/>}
      ...
      <header className="shrink-0 bg-[#0A0D14] border-b border-[#1E2530] px-5 py-3 flex items-center gap-4 z-40">
        ...
        <span className="text-sm text-[#94A3B8] flex items-center gap-2"><Truck size={16}/> Fleet & Drivers</span>
      ...
      <div className="shrink-0 bg-[#0D1117] border-b border-[#1E2530] px-5 flex items-center gap-1">
        {[
          ['drivers', <><User size={14} className="inline mr-1"/> Drivers</>],
          ['vehicles', <><Truck size={14} className="inline mr-1"/> Vehicles</>]
        ].map(([k,l])=>(
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex flex-col w-[55%] shrink-0 border-r border-[#1E2530] overflow-hidden">
            ...
            <div className="shrink-0 px-4 py-3 border-b border-[#1E2530] flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 text-xs">
                ...
                <span className="text-[#34D399] flex items-center gap-1"><CheckCircle size={14}/> {dAvail} Available</span>
                <span className="text-[#60A5FA] flex items-center gap-1"><Info size={14}/> {dActive} Active</span>
                {dFlagged > 0 && <span className="text-[#EF4444] flex items-center gap-1"><AlertCircle size={14}/> {dFlagged} Flagged</span>}
              </div>
              <div className="flex gap-2">
                <button onClick={()=>setShowLeaderboard(s=>!s)} className="text-[10px] border border-[#1E2530] text-[#A78BFA] hover:border-[#A78BFA]/40 px-2.5 py-1.5 rounded transition-colors flex items-center gap-1"><Award size={14}/> Leaderboard</button>
                <button onClick={()=>setShowAddDriver(true)} className="text-[10px] bg-[#4DD9AC]/10 border border-[#4DD9AC]/30 text-[#4DD9AC] px-2.5 py-1.5 rounded hover:bg-[#4DD9AC]/20 transition-colors flex items-center gap-1"><Plus size={14}/> Add Driver</button>
              </div>
            </div>
            ...
            {showLeaderboard && (
              <div className="shrink-0 border-b border-[#1E2530] overflow-x-auto">
                <div className="px-4 py-2 flex items-center justify-between">
                  <span className="text-[10px] text-[#A78BFA] font-bold uppercase tracking-widest flex items-center gap-1"><Award size={14}/> Leaderboard — October 2024</span>
                ...
                <table className="w-full text-[11px]">
                  <tbody className="divide-y divide-[#1E2530]">
                    {lbSorted.map((d,i)=>{
                      const medal = i===0?<Award size={16} className="text-yellow-400 mx-auto"/>:i===1?<Award size={16} className="text-gray-400 mx-auto"/>:i===2?<Award size={16} className="text-amber-600 mx-auto"/>:String(i+1);
                      return (
              ...
              {filteredDrivers.map(d=>{
                ...
                return (
                  <button key={d.id} onClick={()=>setSelDriver(d)} className="w-full text-left px-4 py-3.5 transition-colors hover:bg-[#111827] relative"
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <span className="text-base"><User size={16} className="text-[#60A5FA]"/></span>
                        <span className="font-semibold text-sm text-[#F1F5F9]">{d.name}</span>
                        {d.flagReason && <span className="text-[9px] text-[#EF4444] bg-[#EF4444]/10 px-1.5 py-0.5 rounded flex items-center">{d.status==='critical'?<AlertCircle size={10}/>:<AlertTriangle size={10}/>}</span>}
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] font-bold flex items-center gap-1" style={{color}}>Ack: {d.ackRate}%{d.ackRate<75 && <AlertTriangle size={10} className="inline ml-1"/>}</span>
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1" style={{color, background:`${color}15`}}>{badge} {d.status.toUpperCase()}</span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between text-[10px] text-[#64748B]">
                      <span className="flex items-center gap-1">{d.phone} · <MessageSquare size={10}/> {d.waVerified?'WA ✓':'WA ✗'}</span>
                      <span>{d.activeShip ? `Trip: ${d.activeShip} active` : `Last trip: ${d.lastTrip}`}</span>
                      <span className="flex items-center gap-1" style={{color:d.avgDelay>25?'#F87171':'#64748B'}}>Delay avg: {d.avgDelay} min{d.avgDelay>25 && <AlertTriangle size={10} className="inline ml-1"/>}</span>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="flex-1 min-w-0 bg-[#080B12] overflow-hidden">
            {selDriver
              ...
              : <div className="flex flex-col items-center justify-center h-full text-center px-8"><span className="text-5xl mb-4"><User size={48} className="text-[#64748B]"/></span><div className="text-[#64748B]">Select a driver to view their profile</div></div>
            }
          </div>
        </div>
      )}
          ...
          <div className="flex flex-col w-[55%] shrink-0 border-r border-[#1E2530] overflow-hidden">
            ...
            <div className="shrink-0 px-4 py-3 border-b border-[#1E2530]">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] text-[#64748B] uppercase tracking-widest font-semibold flex items-center gap-2"><Truck size={14}/> Fleet Health Overview</span>
                <div className="flex gap-2">
                  <button onClick={()=>setShowAddVehicle(true)} className="text-[10px] bg-[#60A5FA]/10 border border-[#60A5FA]/30 text-[#60A5FA] px-2.5 py-1.5 rounded hover:bg-[#60A5FA]/20 transition-colors flex items-center gap-1"><Plus size={14}/> Add Vehicle</button>
                </div>
              </div>
              ...
              {(needService > 0 || noIoT > 0) && (
                <div className="flex gap-2 mt-2">
                  {needService > 0 && <span className="text-[9px] text-[#FBBF24] bg-[#FBBF24]/08 border border-[#FBBF24]/25 px-2.5 py-1 rounded-full flex items-center gap-1"><AlertTriangle size={10}/> {needService} vehicles service due soon</span>}
                  {noIoT > 0 && <span className="text-[9px] text-[#F97316] bg-[#F97316]/08 border border-[#F97316]/25 px-2.5 py-1 rounded-full flex items-center gap-1"><AlertTriangle size={10}/> {noIoT} vehicles without IoT sensor</span>}
                </div>
              )}
            </div>
              ...
              {filteredVehicles.map(v=>{
                ...
                return (
                  <button key={v.id} onClick={()=>setSelVehicle(v)} className="w-full text-left px-4 py-3.5 transition-colors hover:bg-[#111827]"
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <span className="text-base"><Truck size={16} className="text-[#A78BFA]"/></span>
                      ...
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1" style={{color, background:`${color}15`}}>{badge} {v.status.toUpperCase()}</span>
                    </div>
                    <div className="flex items-center justify-between text-[10px] text-[#64748B] gap-3">
                      ...
                      {v.sensorId ? <span className="text-[#34D399] flex items-center gap-1"><CheckCircle size={12}/> {v.sensorId}</span> : <span className="text-[#FBBF24] flex items-center gap-1"><AlertTriangle size={12}/> No sensor</span>}
