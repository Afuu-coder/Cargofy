...
import {
  CheckCircle, ArrowRight, Truck, User, Cpu, Map, Clock, Plus,
  ExternalLink, Bell, Play, Activity, Shield, Zap, Copy, Check,
} from 'lucide-react';
...
export function ShipmentLaunched() {
  ...
  useEffect(() => {
    ...
    return () => { clearTimeout(t); if (pingRef.current) clearInterval(pingRef.current); };
  }, []);
  ...
  const checklist = [
    ...
    { icon: Cpu,   label: 'IoT Sensor',       value: `Active · ${pings} pings`, ok: true, pulse: true },
  ...
  return (
    <>
      ...
      <div className="relative w-full min-h-screen bg-[#080B12] text-[#F1F5F9] overflow-hidden flex items-center justify-center p-4"
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[300px] rounded-full"
            style={{ background: 'radial-gradient(ellipse, rgba(77,217,172,0.12) 0%, transparent 70%)' }} />
        </div>

        {/* Card */}
        <div className="relative z-10 w-full max-w-4xl rounded-2xl border border-[#1E2530] overflow-hidden"
          style={{ background: 'rgba(17,24,39,0.95)', backdropFilter: 'blur(20px)', animation: 'slideUp 0.5s ease forwards' }}>

          {/* Top gradient stripe */}
          <div className="h-1 w-full" style={{ background: 'linear-gradient(90deg, #4DD9AC, #3B82F6, #8B5CF6)' }} />
          ...
          <div className="grid md:grid-cols-12 gap-0">
            ...
            <div className="md:col-span-7 p-8 md:p-10 flex flex-col gap-7 border-r border-[#1E2530]">
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full flex items-center justify-center flex-shrink-0"
                  style={{ background: 'rgba(77,217,172,0.15)', border: '2px solid rgba(77,217,172,0.35)',
                    animation: 'scaleIn 0.6s cubic-bezier(.175,.885,.32,1.275) forwards' }}>
                  <CheckCircle size={32} color="#4DD9AC" />
                </div>
                <div>
                  <h1 className="text-2xl font-bold text-[#F1F5F9] tracking-tight">Shipment Dispatched!</h1>
              ...
              {/* Shipment code card */}
              <div className="rounded-xl border border-[#1E2530] p-5" style={{ background: '#111827' }}>
                <div className="flex items-center justify-between mb-4">
                  <span className="font-mono text-xl text-[#4DD9AC] font-bold">{shipmentCode}</span>
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={copyCode}
                      ...
                      {copied ? <Check size={14} color="#4DD9AC" /> : <Copy size={14} color="#64748B" />}
                    </button>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <p className="text-[10px] text-[#64748B] uppercase mb-1 tracking-wider">Origin</p>
                    <p className="text-sm font-semibold text-[#F1F5F9]">{origin}</p>
                  </div>
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <span className="text-[9px] text-[#4DD9AC] mt-0.5">{etaStr}</span>
                  </div>
                  <div className="flex-1 text-right">
                    <p className="text-[10px] text-[#64748B] uppercase mb-1 tracking-wider">Destination</p>
                    <p className="text-sm font-semibold text-[#F1F5F9]">{destination}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button onClick={() => navigate(`/shipment/${shipmentCode}`)}
            ...
            <div className="md:col-span-5 p-6 flex flex-col gap-5">
              <div>
                <h3 className="text-[11px] font-bold text-[#64748B] uppercase tracking-widest">Operational Status</h3>
              </div>

              <div className="flex flex-col gap-3 flex-1">
                {checklist.map(({ icon: Icon, label, value, ok, pulse }) => (
                  <div key={label} className="flex items-start gap-3 p-3 rounded-xl border border-[#1E2530] bg-[#0D1117]">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className={`text-xs font-mono mt-0.5 truncate ${label === 'Blockchain' ? 'text-[#94A3B8]' : 'text-[#F1F5F9]'}`}>
                        {value}
                      </p>
                    </div>
                    <div className="ml-auto flex-shrink-0">
                      <div className={`w-2 h-2 rounded-full ${ok ? 'bg-[#4DD9AC]' : 'bg-[#EF4444]'} ${pulse ? 'animate-pulse' : ''}`} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-auto p-3 rounded-xl border border-[#4DD9AC]/20 bg-[#4DD9AC]/05">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] text-[#4DD9AC] font-bold uppercase tracking-wider">AI Agent Active</span>
                </div>
                <p className="text-[10px] text-[#64748B]">
                  Monitoring temperature, route deviation & driver behaviour. Intervention threshold: 75/100 risk.
                </p>
              </div>
