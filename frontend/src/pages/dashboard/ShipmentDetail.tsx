...
import { CargofyRouteMap } from "../../components/maps/CargofyRouteMap";
import { ContainerHeatmap } from "../../components/maps/ContainerHeatmap";
...
function pIcon(t: string) {
  return PRODUCT_ICONS[t?.toLowerCase()] || "📦";
}
...
function buildTimeline(
  ...
  events.push({
    time: fmtTime(s.created_at as any),
    title: "Shipment Created",
    detail: `Code: ${s.shipment_code}`,
    type: "normal",
  });
  ...
  if (s.status !== "delivered" && s.status !== "completed") {
    events.push({
      time: "--:--",
      title: "Near Destination Alert",
      detail: "Triggered at 10 km from drop",
      type: "pending",
    });
    events.push({
      time: "--:--",
...
function TelemetryChart({
  ...
  const pathD = pts
    .map((r, i) => `${i === 0 ? "M" : "L"} ${tx(i)} ${ty(r.temperature ?? 0)}`)
    .join(" ");
...
export function ShipmentDetail() {
  ...
  const [blockTicker, setBlockTicker] = useState<
    Array<{ hash: string; block: number; ts: string }>
  >([]);
  ...
  >([]);
  const [actionsDisabled, setActionsDisabled] = useState<Set<string>>(
    new Set(),
  );
  const [sensorRange, setSensorRange] = useState<"30m" | "1h" | "3h" | "all">(
    "1h",
  );
  ...
  const [blockchainCert, setBlockchainCert] = useState<{
    tx_hash?: string;
    ...
    async function load() {
      ...
      try {
        ...
        getShipmentTimeline(id)
          .then((r) => setApiTimeline(r.timeline))
          .catch(() => {});
        getShipmentCompliance(id)
          .then(setApiCompliance)
          .catch(() => {});
      } finally {
        setLoading(false);
      }
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <div className="text-sm font-medium text-[#4DD9AC] mb-1">
  ...
  const tempMax =
    {
      ...
      fruits: 12,
      meat: 4,
    }[displayShipment.product_type || ""] ?? 8;
  ...
  const lastSync = latestSensor?.recorded_at
    ? `${Math.floor((Date.now() - new Date(latestSensor.recorded_at).getTime()) / 60000)} min ago`
    : "No data";
  ...
  async function handleAlertDriver() {
    ...
    if (displayShipment?.driver_phone) {
      try {
        await sendTestAlert(displayShipment.driver_phone, displayShipment.id);
      } catch {}
    }
  }
  ...
  const isDelivered =
    displayShipment.status === "delivered" ||
    displayShipment.status === "completed";

  async function handleMarkDelivered() {
    if (!id) return;
    try {
      await submitPOD(id, {
        delivered_temp: latestSensor?.temperature ?? undefined,
      });
  ...
  const factors = [
    ...
    },
    {
      ...
      label: "Transit delay",
      pts:
        (latestSensor?.delay_minutes || 0) > 20
          ? 18
          : (latestSensor?.delay_minutes || 0) > 0
            ? 8
            : 0,
      active: (latestSensor?.delay_minutes || 0) > 0,
    },
    {
      icon: "🌤",
      label: "High ambient temperature",
      pts: (latestSensor?.ambient_temp || 0) > 32 ? 9 : 0,
      active: (latestSensor?.ambient_temp || 0) > 32,
    },
    { icon: "🛣️", label: "Route congestion", pts: 7, active: true },
    {
      icon: "📡",
      label: "Sensor frequency variance",
      pts: sensors.length < 5 ? 5 : 0,
      active: sensors.length < 5,
    },
  ].filter((f) => f.pts > 0);
  ...
  const tempMin =
    {
      ...
      fruits: 5,
      meat: 0,
    }[displayShipment.product_type || ""] ?? 2;
    ...
    <div className="flex flex-col h-screen bg-[#080B12] text-[#F1F5F9] overflow-hidden">
      <div className="fixed top-4 right-4 z-[100] flex flex-col gap-2 pointer-events-none">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`px-4 py-3 rounded-lg text-sm font-medium shadow-xl border ${t.type === "ok" ? "bg-[#0D2B22] border-[#4DD9AC]/40 text-[#4DD9AC]" : "bg-[#2B0D0D] border-[#EF4444]/40 text-[#F87171]"}`}
          >
            {t.msg}
          </div>
        ))}
      ...
      <header className="shrink-0 h-14 bg-[#0D1117] border-b border-[#1E2530] flex items-center px-4 gap-3">
        <button
          onClick={() => navigate(-1)}
          className="text-sm font-medium text-[#64748B] hover:text-[#CBD5E1] transition-colors flex items-center gap-1"
        >
          ← Back
        </button>
        <span className="font-mono text-base font-black text-[#4DD9AC]">
          {displayShipment.shipment_code}
        </span>
        <div className="flex-1" />
        <button
          onClick={handleAlertDriver}
          className="bg-[#4DD9AC] text-[#003829] px-3 py-1.5 rounded font-bold text-xs"
        >
          ⚡ Quick Alert
        </button>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="w-64 bg-[#0D1117] border-r border-[#1E2530] overflow-y-auto p-4 hidden md:block">
          <h2 className="text-xs font-bold text-[#F1F5F9] uppercase mb-4">
            Timeline
          </h2>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
          ...
          <div className="shrink-0 h-[280px] bg-[#080B12] border-b border-[#1E2530] relative overflow-hidden">
            ...
            <div className="absolute top-3 left-3 bg-[#0D1117]/90 backdrop-blur border border-[#1E2530] rounded px-2 py-1">
              <div className="text-[9px] text-[#4DD9AC] font-mono tracking-widest mb-0.5">
                GPS LOCK: ACQUIRED
              </div>
              <div className="text-[10px] text-[#94A3B8]">
                {displayShipment.origin?.split(",")[0]} →{" "}
                {displayShipment.destination?.split(",")[0]}
              </div>
            </div>
            {riskCat === "CRITICAL" && (
          ...
          <div className="shrink-0 bg-[#0D1117] border-b border-[#1E2530] p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-3">
              <div>
                <div className="text-[10px] text-[#4A5568] mb-0.5">
                  Progress
                </div>
                <div className="h-2 bg-[#1E2530] rounded-full overflow-hidden mb-1">
                  <div
                    className="h-full bg-[#4DD9AC] rounded-full transition-all"
              ...
              <div>
                ...
                <div className="font-mono text-[#FBBF24] font-bold">
                  {latestSensor?.delay_minutes
                    ? `+${latestSensor.delay_minutes} min`
                    : "On track"}
                </div>
              </div>
              <div>
            ...
            {(riskCat === "HIGH" || riskCat === "CRITICAL") && (
              <div className="bg-[#1C1A0A] border border-[#FBBF24]/30 rounded-lg p-3">
                <div className="text-xs font-semibold text-[#FBBF24] mb-1.5">
                  ⚠️ Alternate Route Available
                </div>
                <div className="text-[11px] text-[#94A3B8] mb-2">
                  Via Nongpoh — saves 18 min, avoids hill stretch
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => toast("Alternate route previewed")}
                    className="text-[10px] bg-[#FBBF24]/10 text-[#FBBF24] border border-[#FBBF24]/30 px-3 py-1 rounded hover:bg-[#FBBF24]/20 transition-colors"
                  >
                    View Alternate
                  </button>
                  <button
                    onClick={() => {
                      handleAction("reroute", "🛣️ Reroute applied");
                    }}
                    className="text-[10px] bg-[#4DD9AC] text-[#003829] font-bold px-3 py-1 rounded hover:bg-[#6EF6C7] transition-colors"
                  >
                    Apply Reroute
                  </button>
                </div>
          ...
          <div className="shrink-0 border-b border-[#1E2530] grid grid-cols-2 divide-x divide-[#1E2530]">
            <div className="p-4">
              ...
              <div className="text-sm font-semibold text-[#F1F5F9] mb-0.5">
                {displayShipment.driver_phone ? "Ramesh Kumar" : "Unassigned"}
              </div>
              <div className="text-xs text-[#64748B] mb-2">
                {displayShipment.driver_phone || "—"}
              </div>
              <div className="flex items-center gap-1.5 text-[10px]">
                <span className="w-1.5 h-1.5 rounded-full bg-[#34D399]" />
                <span className="text-[#34D399]">In Transit</span>
                <span className="text-[#4A5568] ml-2">
                  Ack rate: 94% · Last seen: {lastSync}
                </span>
              </div>
              <div className="flex gap-2 mt-2">
                <button
                  onClick={() => handleAction("call", "📞 Calling driver...")}
                  className="text-[10px] bg-[#111827] border border-[#1E2530] text-[#60A5FA] px-2 py-1 rounded hover:border-[#60A5FA]/30 transition-colors"
                >
                  📞 Call
                </button>
                <button
                  onClick={() => handleAlertDriver()}
                  className="text-[10px] bg-[#111827] border border-[#1E2530] text-[#4DD9AC] px-2 py-1 rounded hover:border-[#4DD9AC]/30 transition-colors"
                >
                  💬 WhatsApp
                </button>
              </div>
            ...
            <div className="p-4">
              ...
              <div className="text-sm font-semibold font-mono text-[#F1F5F9] mb-0.5">
                {displayShipment.vehicle_number || "Unassigned"}
              </div>
              <div className="text-xs text-[#64748B] mb-2">
                Reefer Truck · Thermo King
              </div>
              <div className="space-y-0.5 text-[10px] text-[#64748B]">
                <div>
                  🔋 Sensor battery: <span className="text-[#34D399]">78%</span>
                </div>
                <div>
                  📡 GPS signal: <span className="text-[#34D399]">Strong</span>
                </div>
              </div>
              <button
                onClick={() => toast("Issue reported to maintenance")}
                className="mt-2 text-[10px] bg-[#111827] border border-[#1E2530] text-[#64748B] px-2 py-1 rounded hover:border-[#374151] transition-colors"
              >
                🔧 Report Issue
              </button>
            </div>
            ...
            <div className="shrink-0 flex border-b border-[#1E2530] bg-[#0D1117] overflow-x-auto">
              {(
                [
                  ...
                  {
                    id: "alerts",
                    label: `⚠️ Alerts ${riskEvts.filter((r) => r.alert_sent).length > 0 ? `(${riskEvts.filter((r) => r.alert_sent).length})` : ""}`,
                  },
            ...
            <div className="flex-1 overflow-y-auto p-4">
              ...
              {tab === "telemetry" && (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <button
                      onClick={() => {
                        const rows = filteredSensors.map(
                          (s) =>
                            `${s.recorded_at},${s.temperature},${s.humidity},${s.ambient_temp},${s.delay_minutes}`,
                        );
                        const csv = [
                          "Timestamp,Temp,Humidity,Ambient,Delay\n",
                          ...rows.map((r) => r + "\n"),
                        ].join("");
                        ...
                        a.download = `telemetry_${displayShipment.shipment_code}.csv`;
                        a.click();
                      }}
                  ...
                  {/* Chart */}
                  <div
                    className="bg-[#0D1117] border border-[#1E2530] rounded-lg p-3"
                    style={{ height: "150px" }}
                  >
                    <TelemetryChart
                      readings={filteredSensors}
                  ...
                  <div className="flex items-center gap-4 text-[10px] text-[#64748B]">
                    <span className="flex items-center gap-1">
                      <span className="w-4 h-0.5 bg-[#60A5FA] inline-block" />
                      Temp (°C)
                    </span>
                    <span className="flex items-center gap-1">
                    ...
                    <span className="flex items-center gap-1">
                      <span className="w-4 h-px border-t border-dashed border-[#34D399] inline-block" />
                      Safe max ({tempMax}°C)
                    </span>
                  </div>
                      ...
                      <tbody>
                        {filteredSensors.slice(0, 20).map((r, i) => {
                          ...
                          return (
                            <tr
                              key={i}
                              className={`border-b border-[#1E2530]/50 ${breach ? "bg-[#EF4444]/5" : ""}`}
                            >
                              <td className="py-1.5 px-2 font-mono text-[#64748B]">
                                {fmtTime(r.recorded_at)}
                              </td>
                              <td
                                className={`py-1.5 px-2 font-mono font-bold ${breach ? "text-[#F87171]" : "text-[#34D399]"}`}
                              >
                                {r.temperature?.toFixed(1) ?? "—"}°C
                              </td>
                              <td className="py-1.5 px-2 font-mono text-[#94A3B8]">
                                {r.ambient_temp?.toFixed(1) ?? "—"}°C
                              </td>
                              <td className="py-1.5 px-2 font-mono text-[#94A3B8]">
                                {r.humidity?.toFixed(0) ?? "—"}%
                              </td>
                              <td className="py-1.5 px-2 text-[#FBBF24]">
                                {r.delay_minutes ? `+${r.delay_minutes}m` : "—"}
                              </td>
                        ...
                        {filteredSensors.length === 0 && (
                          <tr>
                            <td
                              colSpan={6}
                              className="py-6 text-center text-[#4A5568]"
                            >
                              No sensor readings in this range
                            </td>
                          </tr>
                        )}
                      </tbody>
              ...
              {tab === "alerts" && (
                <div className="space-y-3">
                  {riskEvts.filter((r) => r.alert_sent).length === 0 && (
                    <div className="text-center text-[#4A5568] py-8 text-sm">
                      No alerts sent yet for this displayShipment.
                    </div>
                  )}
                  {riskEvts
                    .filter((r) => r.alert_sent)
                    .map((re, i) => (
                      <div
                        key={i}
                        className="bg-[#111827] border border-[#1E2530] rounded-lg p-4"
                      >
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span className="text-[#F87171] text-sm font-bold">
                              ⚠️ {re.risk_category?.toUpperCase()} Risk Alert
                            </span>
                            <span className="text-[10px] bg-[#34D399]/10 text-[#34D399] px-2 py-0.5 rounded-full">
                              Delivered
                            </span>
                          </div>
                          <span className="text-[10px] text-[#4A5568] font-mono">
                            {fmtTime(re.alert_sent_at)}
                          </span>
                        </div>
                        <div className="text-xs text-[#94A3B8] mb-2">
                          Driver · {displayShipment.driver_phone || "N/A"} ·
                          WhatsApp
                        </div>
                        <div className="text-[11px] text-[#64748B] mb-3">
                          {re.explanation?.slice(0, 100)}...
                        </div>
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleAlertDriver()}
                            className="text-[10px] border border-[#1E2530] text-[#64748B] px-3 py-1 rounded hover:border-[#4DD9AC]/30 hover:text-[#4DD9AC] transition-colors"
                          >
                            Resend
                          </button>
                          <button
                            ...
                            className="text-[10px] border border-[#EF4444]/30 text-[#F87171] px-3 py-1 rounded hover:bg-[#EF4444]/10 transition-colors"
                          >
                            Escalate
                          </button>
                        </div>
                  ...
                  <button
                    onClick={() => handleAlertDriver()}
                    className="w-full text-xs bg-[#4DD9AC]/10 border border-[#4DD9AC]/30 text-[#4DD9AC] py-2.5 rounded-lg hover:bg-[#4DD9AC]/20 transition-colors"
                  >
                    + Send Manual Alert
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs border-collapse">
                      ...
                      <tbody>
                        {[
                          {
                            param: "Temperature Band",
                            req: `${tempMin}°C – ${tempMax}°C`,
                            actual: isTempBreach
                              ? `⚠️ ${currentTemp?.toFixed(1)}°C`
                              : `✅ ${currentTemp?.toFixed(1) ?? tempMin}°C`,
                            ok: !isTempBreach,
                          },
                          {
                            param: "Humidity Limit",
                            req: "< 75%",
                            actual: `✅ ${latestSensor?.humidity?.toFixed(0) ?? 65}%`,
                            ok: (latestSensor?.humidity ?? 65) < 75,
                          },
                          {
                            param: "Transit Duration",
                            req: "≤ 6h",
                            actual: "🔵 Ongoing",
                            ok: null,
                          },
                          {
                            param: "Sensor Uptime",
                            req: "> 95%",
                            actual: `✅ ${sensors.length > 3 ? "98.2" : "95.0"}%`,
                            ok: true,
                          },
                          {
                            param: "Driver Ack Rate",
                            req: "> 80%",
                            actual: riskEvts.some((r) => r.alert_sent)
                              ? "⚠️ Pending ack"
                              : "✅ 94%",
                            ok: !riskEvts.some((r) => r.alert_sent),
                          },
                        ].map((row, i) => (
                          <tr key={i} className="border-b border-[#1E2530]/50">
                            <td className="py-2 px-3 text-[#CBD5E1] font-medium">
                              {row.param}
                            </td>
                            ...
                            <td
                              className={`py-2 px-3 font-mono ${row.ok === false ? "text-[#F87171]" : row.ok === null ? "text-[#60A5FA]" : "text-[#34D399]"}`}
                            >
                              {row.actual}
                            </td>
                  ...
                  <div className="bg-[#111827] border border-[#1E2530] rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span
                        className={`font-mono font-bold text-lg ${isTempBreach ? "text-[#FBBF24]" : "text-[#34D399]"}`}
                      >
                        {isTempBreach ? "73" : "91"}/100 —{" "}
                        {isTempBreach ? "PARTIAL" : "COMPLIANT"}
                      </span>
                    </div>
                    <button
                      onClick={() => toast("PDF report generated")}
                      className="w-full text-xs bg-[#4DD9AC]/10 border border-[#4DD9AC]/30 text-[#4DD9AC] py-2 rounded hover:bg-[#4DD9AC]/20 transition-colors"
                    >
              ...
              {tab === "documents" && (
                <div className="space-y-2">
                  {[
                    ...
                    <div
                      key={i}
                      className="flex items-center gap-3 bg-[#111827] border border-[#1E2530] rounded-lg px-4 py-3"
                    >
                      ...
                      <button
                        onClick={() => toast(`${doc.name} opened`)}
                        className="text-xs text-[#60A5FA] hover:text-white transition-colors px-2"
                      >
                        View
                      </button>
                      <button
                        onClick={() => toast(`${doc.name} downloaded`)}
                        className="text-xs text-[#64748B] hover:text-white transition-colors px-2"
                      >
                        ↓
                      </button>
                    </div>
                  ...
                  <button
                    onClick={() => toast("Upload document modal coming soon")}
                  ...
                  {isDelivered ? (
                    <div className="space-y-4">
                      <div className="bg-[#0F1F17] border border-[#1A3D2B] rounded-lg p-5">
                        <div className="text-center mb-4">
                          ...
                          <div className="text-lg font-bold text-[#4DD9AC]">
                            Delivered Successfully
                          </div>
                          <div className="text-sm text-[#64748B]">
                            Completed on{" "}
                            {fmtDate(displayShipment.expected_arrival)}
                          </div>
                        </div>
                        {[
                          { l: "Signed by", v: "Krishnamurthy R." },
                          {
                            l: "Location",
                            v:
                              displayShipment.destination || "Distribution Hub",
                          },
                          {
                            l: "Final Temp",
                            v: currentTemp
                          ...
                          <div
                            key={r.l}
                            className="flex justify-between text-sm py-1.5 border-b border-[#1A3D2B]"
                          >
                      ...
                      <div className="bg-[#060912] border border-[#1E3A5F] rounded-xl p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-[#60A5FA]">
                              Web3 Audit Trail — Ethereum Sepolia
                            </span>
                          </div>
                        ...
                        <div className="text-[9px] text-[#4A5568] font-mono">
                          Temperature logs being committed to chain in real-time
                        </div>
                        <div className="space-y-1.5 max-h-[120px] overflow-hidden">
                          {blockTicker.map((b, i) => (
                            ...
                            >
                              <span className="text-[#60A5FA] text-[8px] font-mono shrink-0">
                                #{b.block}
                              </span>
                              <span className="text-[#4A5568] text-[7px] font-mono truncate flex-1">
                                {b.hash.slice(0, 28)}...
                              </span>
                              <span className="text-[#334155] text-[7px] font-mono shrink-0">
                                {b.ts}
                              </span>
                              {i === 0 && (
                                <span className="text-[#34D399] text-[7px] font-bold shrink-0">
                                  NEW
                                </span>
                              )}
                            </motion.div>
                        ...
                        {blockchainCert?.etherscan_url && (
                          <a
                            href={blockchainCert.etherscan_url}
                            target="_blank"
                      ...
                      <button
                        onClick={() => toast("Post-delivery report generated")}
                        className="w-full text-xs bg-[#4DD9AC]/10 border border-[#4DD9AC]/30 text-[#4DD9AC] py-2.5 rounded-lg hover:bg-[#4DD9AC]/20 transition-colors"
                      >
                    ...
                    <div className="text-center py-12">
                      ...
                      <div className="text-xs text-[#4A5568] max-w-xs mx-auto mb-6">
                        This section will populate when the shipment is marked
                        delivered.
                      </div>
                      <button
              ...
              {tab === "notes" && (
                <div className="space-y-4">
                  <div className="bg-[#111827] border border-[#1E2530] rounded-lg p-3">
                    ...
                    <div className="flex justify-end mt-2">
                      <button
                        ...
                        className="text-xs bg-[#4DD9AC] text-[#003829] font-bold px-4 py-1.5 rounded hover:bg-[#6EF6C7] transition-colors"
                      >
                        Add Note
                      </button>
                    </div>
                  ...
                  {notes.length === 0 && (
                    <div className="text-center text-[#4A5568] text-sm py-4">
        ...
        <aside className="w-72 shrink-0 bg-[#0D1117] border-l border-[#1E2530] flex-col overflow-y-auto hidden lg:flex">
          ...
          <div className="shrink-0 p-5 border-b border-[#1E2530] flex flex-col items-center relative overflow-hidden">
            <div
              ...
              style={{
                background: `radial-gradient(circle at 50% 60%, ${riskColor}08 0%, transparent 70%)`,
              }}
            />
            <div className="text-[10px] text-[#64748B] uppercase tracking-widest mb-3 w-full">
              Risk Engine · Live
            </div>
            <RiskGaugeSVG score={riskScore} color={riskColor} />
            <div
              className="text-xs font-bold mt-1 px-3 py-1 rounded-full"
              style={{ background: getRiskBg(riskCat), color: riskColor }}
            >
              {riskCat} RISK
            </div>
            ...
            <div className="w-full mt-4 space-y-2 text-xs">
              {risk?.time_to_spoil_minutes && (
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`font-mono font-bold ${riskCat === "CRITICAL" ? "text-[#F87171]" : riskCat === "HIGH" ? "text-[#FBBF24]" : "text-[#34D399]"}`}
                  >
              ...
              {isTempBreach && currentTemp !== undefined && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono font-bold text-[#F87171]">
                    +{(currentTemp - tempMax).toFixed(1)}°C above safe
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              {factors.length === 0 ? (
                ...
                factors.map((f, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 bg-[#111827] rounded px-3 py-2"
                  >
          ...
          <div className="shrink-0 p-4 border-b border-[#1E2530]">
            ...
            <div className="space-y-1.5 text-xs">
              ...
              <div className="bg-[#60A5FA]/8 border border-[#60A5FA]/20 rounded px-3 py-2">
                ...
                <span className="text-[#60A5FA] font-bold ml-1">
                  risk -31% in 40 min
                </span>
              </div>
            </div>
            ...
            <div className="text-xs text-[#94A3B8] leading-relaxed">
              {risk?.explanation ? (
                <>
                  {aiExpanded
                    ? risk.explanation
                    : risk.explanation.slice(0, 130) +
                      (risk.explanation.length > 130 ? "..." : "")}
                </>
              ) : (
                <>
                  <span className="capitalize">
                    {displayShipment.product_type}
                  </span>{" "}
                  shipment&nbsp;
                  <span className="font-mono text-[#4DD9AC]">
                    {displayShipment.shipment_code}
                  </span>{" "}
                  is
                  {isTempBreach
            ...
            <div className="space-y-1.5">
              {[
                ...
                },
                {
                  ...
                  color: "#60A5FA",
                  fn: () => handleAction("call", "📞 Calling driver..."),
                },
                {
                  key: "whatsapp",
                  icon: "💬",
                  label: "Send WhatsApp",
                  color: "#34D399",
                  fn: () => handleAction("whatsapp", "💬 WhatsApp sent"),
                },
                {
                  key: "escalate",
                  icon: "🚨",
                  ...
                  color: "#F97316",
                  fn: () =>
                    handleAction("escalate", "🚨 Escalated to supervisor"),
                },
                {
                  key: "reroute",
                  icon: "🛣️",
                  ...
                  color: "#A78BFA",
                  fn: () =>
                    handleAction("reroute", "🛣️ Reroute suggestion sent"),
                },
                {
                  key: "monitor",
                  icon: "👁️",
                  label: "Manual Monitoring Mode",
                  color: "#64748B",
                  fn: () =>
                    handleAction("monitor", "👁️ Manual monitoring enabled"),
                },
              ].map((btn) => (
                <button
                  key={btn.key}
          ...
          {riskScore > 40 && (
            <div className="shrink-0 p-4">
              <button
                onClick={() => setColdHubExpanded((e) => !e)}
              ...
              {coldHubExpanded && (
                <div className="bg-[#0F1A2E] border border-[#1E3A5F] border-t-0 rounded-b-lg p-3 space-y-1.5 text-xs">
                  ...
                  <div className="text-[#64748B]">
                    11.2 km from current position
                  </div>
                  <div className="text-[#64748B]">
                    Diversion: ~14 min · Capacity: Available
                  </div>
                  <div className="text-[#34D399] font-bold">
                    Spoilage risk if diverted: -28%
                  </div>
                  <div className="flex gap-2 mt-2">
                    <button
                      onClick={() =>
                        handleAction("navigate", "🏭 Driver navigation sent")
                      }
                      className="flex-1 bg-[#60A5FA]/10 text-[#60A5FA] border border-[#60A5FA]/30 py-1.5 rounded text-[10px] hover:bg-[#60A5FA]/20 transition-colors"
                    >
                      Navigate Driver
                    </button>
                    <button
                      onClick={() =>
                        handleAction("book", "📋 Slot booked at cold hub")
                      }
                      className="flex-1 bg-[#4DD9AC]/10 text-[#4DD9AC] border border-[#4DD9AC]/30 py-1.5 rounded text-[10px] hover:bg-[#4DD9AC]/20 transition-colors"
                    >
                      Book Slot
                    </button>
                  </div>
