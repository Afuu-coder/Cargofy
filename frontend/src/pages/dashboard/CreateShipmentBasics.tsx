...
import {
  CargofyRouteMap,
  type RouteData,
} from "../../components/maps/CargofyRouteMap";
...
const PRODUCTS = [
  ...
  },
  {
    ...
    tempMin: 5,
    tempMax: 12,
    hint: "5°C – 12°C ripening control",
  },
  {
    id: "meat",
    icon: "🥩",
...
export function CreateShipmentBasics() {
  ...
  useEffect(() => {
    ...
    if (p) {
      set("tempMin", p.tempMin);
      set("tempMax", p.tempMax);
    }
  }, [form.productType]);
  ...
  const routeInfo =
    ROUTE_PRESETS.find(
      (r) =>
        r.from
          .toLowerCase()
          .includes(form.origin.toLowerCase().split(",")[0]) &&
        r.to
          .toLowerCase()
          .includes(form.destination.toLowerCase().split(",")[0]),
    ) ||
    (form.origin && form.destination
      ? { dist: "~— km (custom)", dur: "Calculating..." }
      : null);
  ...
  async function fetchDispatchSuggestion() {
    ...
    try {
      ...
      if (!form.vehicleNumber && data.vehicle_id)
        set("vehicleNumber", data.vehicle_id);
      if (!form.driverPhone && data.driver_phone)
        set("driverPhone", data.driver_phone);
      if (!form.driverName && data.driver_name)
        set("driverName", data.driver_name);
    } catch {
    ...
    try {
      const pickup =
        form.pickupDate && form.pickupTime
          ? new Date(`${form.pickupDate}T${form.pickupTime}:00`).toISOString()
          : undefined;
      const delivery =
      ...
      let result: any;
      const routeSnapshot = (window as any).__cargofyRouteData as
        | RouteData
        | undefined;
      ...
      setErrors({
        submit: err?.message || "Failed to create shipment. Please try again.",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    ...
    >
      ...
      <header className="shrink-0 h-14 bg-[#0A0D14] border-b border-[#1E2530] flex items-center px-4 gap-4 z-40">
        <div
          className="text-[#4DD9AC] font-black text-xl tracking-tighter font-mono shrink-0 cursor-pointer"
          onClick={() => navigate("/dashboard")}
        >
          CARGOFY
        </div>
        <div className="flex-1" />
        <div className="text-xs text-[#64748B] hidden md:block">
          Create Shipment Wizard
        </div>
        <button
          onClick={() => navigate("/dashboard")}
          className="text-xs text-[#64748B] hover:text-[#CBD5E1] transition-colors px-3 py-1.5 border border-[#1E2530] rounded"
        >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="shrink-0 w-[200px] bg-[#0D1117] border-r border-[#1E2530] flex-col hidden md:flex overflow-y-auto py-4">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 text-sm transition-all text-left border-l-2 ${
                s.n === step
                  ? "bg-[#4DD9AC]/10 text-[#4DD9AC] border-[#4DD9AC] font-semibold"
                  : s.n < step
                    ? "text-[#34D399] border-[#34D399]/40 hover:bg-[#111827] cursor-pointer"
          ...
          <div className="mt-auto p-4 border-t border-[#1E2530]">
            <div className="text-[10px] text-[#4A5568] uppercase tracking-widest mb-2">
              Shipment Code
            </div>
            <div className="font-mono text-xs text-[#4DD9AC] flex items-center gap-2">
              ...
              <button
                onClick={() => {
                  navigator.clipboard.writeText(shipmentCode);
                  setCopied(true);
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="shrink-0 bg-[#0D1117] border-b border-[#1E2530] px-6 py-4">
              <div className="flex items-center gap-0 max-w-xl">
                {["Basics", "Route", "Logistics", "Monitoring", "Review"].map(
                  (label, i) => {
                    const n = i + 1;
                    const done = n < step;
                    const active = n === step;
                    return (
                      <React.Fragment key={n}>
                        <div className="flex flex-col items-center gap-1">
                          <button
                            ...
                            className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all border-2 ${
                                ...
                                ? "bg-[#4DD9AC] border-[#4DD9AC] text-[#003829]"
                                : active
                                  ? "bg-transparent border-[#4DD9AC] text-[#4DD9AC] shadow-[0_0_10px_rgba(77,217,172,0.3)]"
                          ...
                          <span
                            className={`text-[10px] font-medium whitespace-nowrap ${active ? "text-[#4DD9AC]" : done ? "text-[#34D399]" : "text-[#4A5568]"}`}
                          >
                            {label}
                          </span>
                        </div>
                        {i < 4 && (
                          <div
                            className={`flex-1 h-0.5 mx-1 mb-4 transition-colors ${done ? "bg-[#4DD9AC]" : "bg-[#1E2530]"}`}
                          />
                        )}
                      </React.Fragment>
                    );
                  ...
                  >
                    ...
                    {step === 3 && (
                      <Step3
                        ...
                        suggestionLoading={step3Loading}
                      />
                    )}
                    {step === 4 && (
                      <Step4
            ...
            <div className="shrink-0 bg-[#0D1117] border-t border-[#1E2530] px-6 py-4 flex items-center justify-between">
              ...
              {step < 5 ? (
                ...
                >
                  {step1Loading ? (
                    <>
                      <span className="w-4 h-4 border-2 border-[#003829]/30 border-t-[#003829] rounded-full animate-spin" />
                      Validating...
                    </>
                  ) : (
                    <>
                      Continue →{" "}
                      {["", "Route", "Logistics", "Monitoring", "Review"][step]}
                    </>
                  )}
                </button>
              ) : (
                <button
                  ...
                  disabled={submitting}
                  className="px-6 py-2.5 bg-gradient-to-r from-[#4DD9AC] to-[#6EF6C7] text-[#003829] font-bold text-sm rounded-lg hover:opacity-90 active:scale-95 transition-all flex items-center gap-2 disabled:opacity-60"
                >
                  {submitting ? (
                    <>
                      <span className="w-4 h-4 border-2 border-[#003829]/30 border-t-[#003829] rounded-full animate-spin" />
          ...
          <aside className="w-[280px] shrink-0 bg-[#0D1117] border-l border-[#1E2530] flex-col hidden xl:flex overflow-y-auto p-4 gap-4">
            ...
            <div className="bg-[#111827] rounded-lg border border-[#1E2530] p-4">
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <div className="font-mono text-sm text-[#4DD9AC]">
                    {shipmentCode}
                  </div>
                  <div className="text-xs text-[#64748B] capitalize">
                    {form.productType} · {form.quantity || "—"} {form.unit}
                  </div>
                </div>
              </div>
              <div className="space-y-2 text-xs">
                ...
                <PreviewRow
                  label="Temp Band"
                  value={`${form.tempMin}°C – ${form.tempMax}°C`}
                />
                ...
                />
                {form.vehicleNumber && (
                  <PreviewRow label="Vehicle" value={form.vehicleNumber} />
                )}
                {form.driverName && (
            ...
            <div className="bg-[#111827] rounded-lg border border-[#1E2530] p-4">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`font-bold ${form.productType === "dairy" || form.productType === "seafood" || form.productType === "pharma" ? "text-[#F87171]" : "text-[#FBBF24]"}`}
                  >
                    {form.productType === "dairy" ||
                    form.productType === "seafood" ||
                    form.productType === "pharma"
                      ? "HIGH"
                      : "MEDIUM"}
                  </span>
                </div>
              ...
              <div className="mt-3 bg-[#0F1F17] border border-[#1A3D2B] rounded p-3">
                <div className="text-[10px] text-[#4DD9AC] font-semibold mb-1 flex items-center gap-1">
                  💡 Cargofy AI Insight
                </div>
                <div className="text-[11px] text-[#94A3B8] leading-relaxed">
                  {form.productType === "dairy"
                    ? "Dairy shipments on this route had 3 excursions in 7 days. Consider night dispatch or tighter band."
                    : "Set up IoT monitoring for real-time breach detection on this route."}
                </div>
              </div>
            </div>
            ...
            {form.pickupTime && parseInt(form.pickupTime) >= 9 && (
              <div className="bg-[#0F1A2E] border border-[#1E3A5F] rounded-lg p-4">
                <div className="text-[10px] text-[#60A5FA] font-semibold mb-2 flex items-center gap-1">
                  💡 Cargofy AI Suggests
                </div>
                <div className="text-[11px] text-[#94A3B8] leading-relaxed mb-3">
                  Dispatch before 07:00 to avoid peak congestion. Current time (
                  {form.pickupTime}) may add +28 min delay.
                </div>
                <button
                  onClick={() => set("pickupTime", "06:30")}
                  className="w-full text-[10px] bg-[#60A5FA]/10 text-[#60A5FA] border border-[#60A5FA]/30 py-1.5 rounded hover:bg-[#60A5FA]/20 transition-colors"
                >
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#64748B]">
          Define what you're shipping and its cold chain requirements.
        </p>
      </div>
      ...
      {warnings.length > 0 && (
        <div className="space-y-1">
          {warnings.map((w: string, i: number) => (
            <div
              key={i}
              className="flex items-start gap-2 bg-[#FBBF24]/10 border border-[#FBBF24]/30 rounded-lg px-3 py-2"
            >
        ...
        <div className="grid grid-cols-4 gap-2">
          {PRODUCTS.map((p) => (
            ...
            >
              ...
              <span
                className={`text-[11px] font-medium ${form.productType === p.id ? "text-[#4DD9AC]" : "text-[#64748B]"}`}
              >
                {p.label}
              </span>
            </button>
      ...
      <FormSection label="Quantity" required error={errors.quantity}>
        <div className="flex gap-2">
          ...
          >
            {["kg", "g", "litres", "ml", "units", "boxes", "packets"].map(
              (u) => (
      ...
      {/* Temperature band */}
      <FormSection
        label="Safe Temperature Band"
        hint="Auto-filled based on product type"
      >
        <TempBandViz min={form.tempMin} max={form.tempMax} />
        <div className="flex gap-4 mt-3">
          <div className="flex-1">
            ...
            <input
              type="number"
              value={form.tempMin}
              onChange={(e) => set("tempMin", parseFloat(e.target.value))}
              className={Field}
            />
          </div>
          <div className="flex-1">
            ...
            <input
              type="number"
              value={form.tempMax}
              onChange={(e) => set("tempMax", parseFloat(e.target.value))}
              className={Field}
            />
          </div>
        </div>
      ...
      <FormSection label="Shelf Life Class">
        <div className="flex gap-2">
          {[
            { v: "fresh", l: "Fresh (<24h)" },
            { v: "short", l: "Short (1–3 days)" },
            { v: "extended", l: "Extended (3–7 days)" },
          ].map((o) => (
            <button
              key={o.v}
              onClick={() => set("shelfLife", o.v)}
      ...
      <FormSection label="Priority Level">
        <div className="flex gap-2">
          {[
            ...
            { v: "high", l: "🟠 High", c: "#F59E0B" },
            { v: "standard", l: "🟡 Standard", c: "#EAB308" },
            { v: "low", l: "🟢 Low", c: "#22C55E" },
          ].map((o) => (
            <button
              key={o.v}
...
const NE_CITIES_COORDS: Record<string, { lat: number; lng: number }> = {
  ...
  "Imphal, Manipur": { lat: 24.817, lng: 93.9368 },
  ...
  "Dibrugarh, Assam": { lat: 27.4728, lng: 94.912 },
};
...
function Step2({ form, set, errors, routeInfo: _routeInfo }: any) {
  ...
  const resolveCoords = useCallback(
    (name: string): { lat: number; lng: number } | null => {
      const match = Object.entries(NE_CITIES_COORDS).find(([k]) =>
  ...
  const geocodeQuery = useCallback(
    async (
      ...
      // First check local
      const local = NE_CITIES.filter((c) =>
        c.toLowerCase().includes(q.toLowerCase()),
      )
        .slice(0, 4)
        .map((c) => ({ name: c, ...NE_CITIES_COORDS[c] }));
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((data) => {
        ...
        const severe = congestion.filter(
          (c: string) => c === "severe" || c === "heavy",
        ).length;
        ...
        const hubs = [
          {
            ...
            distance_from_route_km: 2,
          },
          {
            name: "Guwahati Logistics Park",
            lat: 26.1445,
            lng: 91.7362,
            capacity_available: true,
          ...
          const dx = h.lng - (originLng + destLng) / 2,
            dy = h.lat - (originLat + destLat) / 2;
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-[#64748B]">
          Define the route — Mapbox calculates live distance, ETA and risk.
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#4DD9AC] text-sm">
            📍
          </span>
          <input
            value={form.origin}
        ...
        {errors.origin && (
          <div className="text-red-400 text-xs mt-1">{errors.origin}</div>
        )}
      ...
      <div className="relative">
        ...
        {errors.destination && (
          <div className="text-red-400 text-xs mt-1">{errors.destination}</div>
        )}
      ...
      {routeData && (
        <div className="grid grid-cols-3 gap-3">
          {[
            {
              label: "Distance",
              value: `${routeData.distance_km} km`,
              icon: "📏",
            },
            {
              label: "Transit",
              value: `${routeData.duration_min} min`,
              icon: "⏱",
            },
            {
              label: "Route Risk",
              ...
              icon: "🛡",
              color:
                routeData.route_risk_preview === "HIGH"
                  ? "#F97316"
                  : routeData.route_risk_preview === "MEDIUM"
                    ? "#FBBF24"
                    : "#34D399",
            },
          ].map((s) => (
            <div
              key={s.label}
              className="bg-[#111827] border border-[#1E2530] rounded-lg p-3 text-center"
            >
      ...
      {/* Cold hubs */}
      {routeData?.cold_hubs_on_route &&
        routeData.cold_hubs_on_route.length > 0 && (
          <div className="bg-[#0F1F17] border border-[#1A3D2B] rounded-lg p-4">
            <div className="text-[10px] text-[#4DD9AC] uppercase tracking-widest font-semibold mb-2">
              ❄️ Cold Hubs On Route
            </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[#64748B]">
                      {h.distance_from_route_km} km detour
                    </span>
                    <span
                    ...
                    >
                      {h.capacity_available ? "Available" : "Full"}
                    </span>
                  </div>
                </div>
              ))}
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {["Retailer", "Distributor", "Cold Hub", "Direct Customer"].map(
            (t) => (
              <button
                key={t}
                onClick={() => set("routeNotes", t)}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#64748B]">
          Assign vehicle, driver and schedule.
        </p>
      </div>
      ...
      {suggestionLoading && (
        <div className="flex items-center gap-2 bg-[#4DD9AC]/10 border border-[#4DD9AC]/25 rounded-lg px-4 py-3">
          ...
          <span className="text-xs text-[#4DD9AC]">
            🤖 ADK DispatchAgent analysing route and traffic…
          </span>
        </div>
      )}
      {!suggestionLoading && suggestion && (
        <div className="bg-[#4DD9AC]/05 border border-[#4DD9AC]/20 rounded-lg px-4 py-3 space-y-1">
          <div className="text-[10px] text-[#4DD9AC] uppercase tracking-widest font-bold">
            🤖 ADK Dispatch Suggestion
          </div>
          <div className="text-xs text-[#CBD5E1]">
            {suggestion.reasoning ||
              "Vehicle and driver auto-selected based on route, capacity, and availability."}
          </div>
          {suggestion.eta_minutes && (
            <div className="text-[11px] text-[#94A3B8]">
              Est. delivery:{" "}
              <span className="text-[#4DD9AC] font-semibold">
      ...
      <FormSection label="Select Vehicle" required error={errors.vehicleNumber}>
        <div className="space-y-2">
          {VEHICLES.map((v) => (
            <button
              key={v.id}
              onClick={() => set("vehicleNumber", v.id)}
            ...
            >
              <div className="flex items-center justify-between mb-1">
                ...
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${v.status === "Available" ? "bg-[#34D399]/10 text-[#34D399]" : "bg-[#EF4444]/10 text-[#F87171]"}`}
                >
                  {v.status}
                </span>
              </div>
              ...
              <div className="text-xs text-[#4DD9AC]">
                Temp: {v.range} · Stability: {v.stability}
              </div>
            </button>
          ))}
        </div>
          ...
          {DRIVERS.map((d) => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                ...
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${d.status === "Available" ? "bg-[#34D399]/10 text-[#34D399]" : "bg-[#EF4444]/10 text-[#F87171]"}`}
                >
                  {d.status}
                </span>
              </div>
              <div className="text-xs text-[#64748B]">
                {d.phone} · Ack rate: {d.ackRate} · Avg delay: {d.delay}
              </div>
            </button>
          ))}
        </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#64748B]">
          Configure IoT, alerts and AI intervention rules.
        </p>
      </div>
      ...
      {iotLoading && (
        <div className="flex items-center gap-2 bg-[#60A5FA]/10 border border-[#60A5FA]/25 rounded-lg px-4 py-3">
          ...
          <span className="text-xs text-[#60A5FA]">
            📡 Pairing IoT device with Firebase RTDB…
          </span>
        </div>
      )}
        ...
        <div className="flex items-center gap-3 bg-[#34D399]/08 border border-[#34D399]/25 rounded-lg px-4 py-3">
          ...
          <div>
            <div className="text-xs font-semibold text-[#34D399]">
              IoT Device Paired — {iotPaired.device_id}
            </div>
      ...
      <div className="flex items-center justify-between bg-[#111827] border border-[#1E2530] rounded-lg p-4">
        <div>
          ...
          <div className="text-xs text-[#64748B]">
            No hardware? Use our built-in IoT simulator for this shipment.
          </div>
        </div>
        <button
          onClick={() => set("simulate", !form.simulate)}
          className={`w-12 h-6 rounded-full transition-colors relative ${form.simulate ? "bg-[#4DD9AC]" : "bg-[#374151]"}`}
        >
          <div
            className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${form.simulate ? "translate-x-6" : "translate-x-0.5"}`}
          />
        </button>
      </div>
      ...
      {!form.simulate && (
        <FormSection label="IoT Device ID">
          <input
            value={form.iotDevice}
            onChange={(e) => set("iotDevice", e.target.value)}
            className={Field}
      ...
      <FormSection label="Sensor Type">
        <div className="flex gap-2">
          {[
            { v: "temp_only", l: "🌡 Temp Only" },
            { v: "temp_humidity", l: "🌡💧 Temp + Humidity" },
            { v: "full_iot", l: "📡 Full IoT" },
          ].map((o) => (
            <button
              key={o.v}
              onClick={() => set("sensorType", o.v)}
      ...
      <FormSection
        label={`Telemetry Frequency: every ${form.telemetryFreq} min`}
      >
        <input
          type="range"
          min="1"
      ...
      <FormSection label="Alert Thresholds">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-[10px] text-[#64748B] mb-1">
              Alert if temp above °C
            </label>
            <input
              type="number"
              value={form.alertTempAbove}
              onChange={(e) =>
                set("alertTempAbove", parseFloat(e.target.value))
              }
              className={Field}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => set("alertMinutes", parseInt(e.target.value))}
              min="1"
              className={Field}
            />
          </div>
        ...
        <div className="mt-3 bg-[#080B12] rounded p-3">
          <div className="flex text-[10px] text-[#64748B] justify-between mb-1">
            <span>{form.tempMin - 1}°C</span>
            <span className="text-[#34D399]">
              SAFE [{form.tempMin}–{form.tempMax}°C]
            </span>
            <span>{form.alertTempAbove}°C ⚠</span>
          </div>
      ...
      <FormSection label="Alert Channels">
        {[
          { k: "whatsappAlert", l: "📱 WhatsApp alerts" },
          { k: "pushAlert", l: "🔔 Push notifications" },
          { k: "autoAlert", l: "🤖 Auto-alert driver on breach" },
        ].map((o) => (
          <label
            key={o.k}
          ...
          >
            <input
              type="checkbox"
              checked={(form as any)[o.k]}
              onChange={(e) => set(o.k as any, e.target.checked)}
              className="w-4 h-4 accent-[#4DD9AC]"
            />
          ...
          {[
            {
              v: "monitor",
              l: "👁 Monitor Only",
              d: "Log events, no automated actions",
            },
            {
              v: "suggest",
              l: "💡 Suggest Actions",
              d: "AI recommends interventions for your approval",
            },
            {
              v: "auto",
              l: "⚡ Auto-Escalate",
              d: "AI acts automatically for CRITICAL events",
            },
          ].map((o) => (
            <button
              key={o.v}
            ...
            >
              <div
                className={`text-sm font-semibold mb-0.5 ${form.aiMode === o.v ? "text-[#4DD9AC]" : "text-[#F1F5F9]"}`}
              >
                {o.l}
              </div>
...
function Step5({
  ...
  const riskCategory =
    riskPreview?.risk_category || (riskLoading ? "…" : "MEDIUM");
  const spoilageRisk =
    riskPreview?.spoilage_risk_pct ?? (riskLoading ? null : 42);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#64748B]">
          Confirm all details before dispatching this shipment.
        </p>
      </div>
      ...
      <div className="bg-[#111827] border border-[#1E2530] rounded-lg overflow-hidden">
        <div className="bg-[#0D1117] px-5 py-3 border-b border-[#1E2530]">
          <div className="flex items-center justify-between">
            <span className="font-mono text-[#4DD9AC] font-bold">
              {shipmentCode}
            </span>
            <span className="text-xs bg-[#FBBF24]/10 text-[#FBBF24] border border-[#FBBF24]/30 px-2 py-0.5 rounded-full font-semibold">
        ...
        <div className="p-5 grid grid-cols-2 gap-5">
          <div>
            <ReviewBlock
              title={`${selectedProduct?.icon || "📦"} Product`}
              rows={[
                [
                  "Type",
                  `${form.productType} ${form.productName ? `— ${form.productName}` : ""}`,
                ],
          ...
          <div>
            <ReviewBlock
              ...
              rows={[
                ...
                [
                  "Alerts",
                  [form.whatsappAlert && "WhatsApp", form.pushAlert && "Push"]
                    .filter(Boolean)
                    .join(" + ") || "None",
                ],
              ]}
        ...
        <div className="border-t border-[#1E2530] p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="text-xs text-[#64748B] uppercase tracking-widest">
              Pre-Dispatch Risk Preview
            </div>
            {riskLoading && (
              <span className="text-[10px] text-[#4DD9AC] flex items-center gap-1">
                <span className="w-2 h-2 border border-[#4DD9AC]/40 border-t-[#4DD9AC] rounded-full animate-spin" />
                Vertex AI computing…
              </span>
            )}
            {riskPreview && (
              <span className="text-[10px] text-[#34D399]">✓ Vertex AI</span>
            )}
          </div>
            ...
            <div className="text-center">
              ...
              {riskLoading ? (
                ...
                <div className="text-lg font-bold text-[#F1F5F9]">
                  {spoilageRisk !== null ? `${spoilageRisk}%` : "—"}
                </div>
              )}
            </div>
            <div className="text-center">
              <div className="text-[10px] text-[#64748B] mb-1">
                {ambientTemp != null ? "Ambient Temp" : "Nearest Cold Hub"}
              </div>
              {riskLoading ? (
          ...
          <div className="mt-3 h-2 bg-[#1E2530] rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-[#34D399] via-[#FBBF24] to-[#EF4444] rounded-full transition-all duration-700"
              style={{ width: `${spoilageRisk ?? 45}%` }}
            />
          </div>
          <div className="mt-1 text-[10px] text-[#4A5568]">
            {riskPreview?.recommendation ||
              `Overall dispatch risk: ${spoilageRisk ?? 45}% of critical threshold`}
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#0F1F17] border border-[#1A3D2B] rounded-lg p-4 text-sm text-[#94A3B8]">
        ...
        On confirmation, the driver will be notified via WhatsApp and monitoring
...
function LaunchScreen({
  ...
  return (
    <div
      className="min-h-screen bg-[#080B12] flex items-center justify-center p-6"
      style={{ fontFamily: "Inter, sans-serif" }}
    >
      <div className="w-full max-w-lg text-center">
        ...
        <p className="text-[#64748B] text-sm mb-6">
          Your shipment is now live. Monitoring is active.
        </p>
        ...
        <div className="bg-[#0D1117] border border-[#1E2530] rounded-xl p-5 text-left mb-6 space-y-3">
          <div className="flex items-center justify-between border-b border-[#1E2530] pb-3">
            <span className="font-mono text-[#4DD9AC] font-bold text-lg">
              {shipment.shipment_code}
            </span>
            <span className="text-xs bg-[#34D399]/10 text-[#34D399] border border-[#34D399]/30 px-2 py-1 rounded-full font-semibold">
              ACTIVE
            </span>
          </div>
          <LaunchRow
            ...
            value={form.vehicleNumber || "Unassigned"}
          />
          <LaunchRow
            icon="👤"
            label="Driver"
            value={
              form.driverName
                ? `${form.driverName} (notified via WhatsApp)`
                : "Unassigned"
            }
          />
          <LaunchRow
            icon="📡"
            label="IoT Sensor"
            value={`${form.simulate ? "Simulator" : form.iotDevice} — active every ${form.telemetryFreq} min`}
          />
          <LaunchRow
            icon="📍"
            label="Route"
            value={
              form.origin && form.destination
                ? `${form.origin.split(",")[0]} → ${form.destination.split(",")[0]}`
                : "Defined"
            }
          />
          <LaunchRow
            icon="🌡"
            label="Monitoring"
            value={`${form.tempMin}°C – ${form.tempMax}°C · Alert if >{form.alertTempAbove}°C`}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <div>
              <div className="text-sm font-semibold text-[#F1F5F9] group-hover:text-[#4DD9AC] transition-colors">
                Start Simulation
              </div>
          ...
          <button
            onClick={() => onNavigate(`/shipments/${shipment.id}`)}
            className="group flex items-center gap-3 bg-[#111827] border border-[#1E2530] hover:border-[#4DD9AC]/40 p-4 rounded-lg transition-all text-left"
          >
            <span className="text-2xl">📋</span>
            <div>
          ...
          >
            <span className="text-2xl">+</span>
            <div>
              <div className="text-sm font-semibold text-[#4DD9AC]">
                New Shipment
              </div>
...
const Field =
  "w-full bg-[#111827] border border-[#1E2530] text-[#F1F5F9] text-sm px-3 py-2.5 rounded-lg focus:outline-none focus:border-[#4DD9AC]/60 transition-colors placeholder-[#4A5568]";
      ...
      <div className="space-y-1">
        {rows.map(([k, v]) => (
          <div key={k} className="flex items-start gap-2">
            <span className="text-[10px] text-[#4A5568] shrink-0 w-20 mt-0.5 capitalize">
              {k}
            </span>
            <span className="text-xs text-[#CBD5E1] font-medium break-all">
              {v}
            </span>
          </div>
        ))}
...
function PreviewRow({
  ...
  return (
    <div className="flex items-start justify-between gap-2">
      ...
      <span
        className="text-right text-[11px] font-medium truncate max-w-[160px]"
