...
import { Cesium3DFleetMap } from "../../components/maps/Cesium3DFleetMap";
import {
  Bot,
  Sparkles,
  Loader2,
...
function useToasts() {
  const [toasts, setToasts] = useState<
    { id: string; msg: string; type: ToastType }[]
  >([]);
...
function StageRail({
  ...
  const eventDetails: Record<string, { time?: string; detail: string }> = {
    created: {
      time: fmtTime(shipment.created_at as any),
      detail: `Code: ${shipment.shipment_code} · System created`,
    },
    picked: {
      time: fmtTime(shipment.created_at as any),
      detail: `Driver: ${shipment.driver_phone ?? "Assigned"}`,
    },
    loaded: {
      time: latestSensor ? fmtTime(latestSensor.recorded_at) : "--:--",
      detail: `${shipment.product_qty ? `${shipment.product_qty} ${shipment.product_unit}` : "Cargo"} loaded · Cold seal applied`,
    },
    departed: {
      time: fmtTime(shipment.expected_departure as any) ?? "--:--",
      detail: `Route locked`,
    },
    transit: { time: "--:--", detail: `${progress}% complete` },
    checkpoint: {
      time: "--:--",
      detail: `Midpoint reached · All sensors nominal`,
    },
    approaching: { time: "--:--", detail: `Within 20 km of destination` },
    delivered: {
      time: fmtTime(shipment.expected_arrival as any) ?? "--:--",
      ...
      <div className="relative">
        ...
        <div className="space-y-1">
          {JOURNEY_STAGES.map((stage, i) => {
            const isDone =
              completedKeys.includes(stage.key) && stage.key !== activeKey;
            ...
            return (
              <div key={stage.key} className="relative pl-8">
                {i < JOURNEY_STAGES.length - 1 && (
                  <div
                    className="absolute left-[11px] top-5 h-full w-px z-0 transition-colors duration-500"
                    style={{
                  ...
                  {isActive ? (
                    ...
                    <div
                      ...
                      style={{
                        ...
                        background: isDone ? dotColor : "#0D1117",
                      }}
                    >
                      {isDone && (
                ...
                >
                  <div className="flex items-center gap-1.5">
                    <span
                      className={`text-xs font-semibold ${isActive ? "text-[#60A5FA]" : isDone ? "text-[#F1F5F9]" : "text-[#374151]"}`}
                    >
                ...
                <AnimatePresence>
                  {isOpen && !isPending && (
                    ...
                    >
                      <div className="font-semibold text-[#F1F5F9] mb-1">
                        {stage.icon} {stage.label}
                      </div>
                      {eventDetails[stage.key]?.time && (
                        <div className="text-[#4DD9AC] font-mono mb-1">
                          {eventDetails[stage.key].time}
                        </div>
                      )}
                      <div className="text-[#94A3B8] leading-relaxed">
                        {eventDetails[stage.key]?.detail}
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
...
function MiniSparkline({
  ...
  if (sensors.length < 2)
    return (
      <div className="h-16 flex items-center justify-center text-[#4A5568] text-xs">
        No data
      </div>
    );
...
function PredictiveETAPanel({
  ...
  const [factors, setFactors] = useState<
    { label: string; impact: string; color: string }[]
  >([]);
  ...
  const marginColor =
    safetyMargin < 0 ? "#EF4444" : safetyMargin < 30 ? "#FBBF24" : "#34D399";

  const SATELLITE_FACTORS = [
    {
      label: "Ambient temp (satellite)",
      impact:
        sensors[0]?.ambient_temp && sensors[0].ambient_temp > 35
          ? "+8 risk pts"
          : "−2 risk pts",
      color:
        sensors[0]?.ambient_temp && sensors[0].ambient_temp > 35
    ...
    {
      label: "Road surface heat index",
      impact: "+5 risk pts",
      color: "#FBBF24",
    },
    {
      label: "Altitude profile ahead",
      impact: "−4 risk pts",
      color: "#34D399",
    },
    {
      label: "Traffic congestion zone",
      impact: "+12 min ETA",
      color: "#FBBF24",
    },
    {
      label: "Rain probability (47%)",
      impact: "−3 risk pts",
      color: "#60A5FA",
    },
  ];
    ...
    <div className="px-4 py-4 border-b border-[#1E2530]">
      ...
      <div className="bg-[#111827] border border-[#1E2530] rounded-xl p-3 mb-3">
        <div className="flex items-center justify-between mb-2">
          ...
          >
            {safetyMargin >= 0
              ? `+${safetyMargin} min safe`
              : `${Math.abs(safetyMargin)} min OVER`}
          </span>
        </div>
        ...
        <div className="flex justify-between text-[9px] text-[#4A5568] font-mono">
          <span>
            Spoils in {Math.floor(spoil / 60)}h {spoil % 60}m
          </span>
          <span>
            ETA {Math.floor(remainEta / 60)}h {remainEta % 60}m
          </span>
        </div>
            ...
            <span className="text-[10px] text-[#94A3B8]">
              {safetyMargin < 0 ? (
                <span className="text-[#F87171] font-bold">
                  ⚠️ AI: Divert NOW — cargo will spoil before arrival
                </span>
              ) : safetyMargin < 30 ? (
                <span className="text-[#FBBF24] font-bold">
                  ⚡ AI: Tight window — consider rerouting
                </span>
              ) : (
                <span className="text-[#34D399]">
                  ✅ AI: Delivery within safe window
                </span>
              )}
            </span>
          </div>
...
function generateSwarmEvent(ships: Shipment[], tick: number): SwarmEvent {
  ...
  const actions = [
    ...
    (c: string) =>
      `Spoilage window extended by 22 min — reefer restored on ${c}`,
    (c: string) => `Escalated ${c} to Fleet Manager — no driver ACK in 10 min`,
    (c: string) =>
      `Blockchain SLA contract for ${c} auto-triggered penalty clause`,
    (c: string) =>
      `Gemini Negotiator secured emergency slot for ${c} at ₹2,400`,
  ];
  ...
  return {
    ...
    severity: (
      ["crit", "crit", "warn", "info", "info", "warn", "crit", "crit"] as const
    )[sevidx[idx]],
  };
}
  ...
  return (
    <div className="px-4 py-4">
      ...
      <div ref={scrollRef} className="space-y-1.5 max-h-52 overflow-y-auto">
        {events.length === 0 && (
          <div className="text-[10px] text-[#4A5568] text-center py-4">
            Swarm initializing…
          </div>
        )}
        <AnimatePresence>
          {events.map((ev) => (
            ...
            >
              <div
                className="w-1.5 h-1.5 rounded-full mt-1 shrink-0 ring-2 ring-current"
                style={{
                  color: sevBg(ev.severity),
                  background: sevBg(ev.severity) + "40",
                }}
              />
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] text-[#CBD5E1] leading-snug mt-0.5">
                  {ev.action}
                </div>
              </div>
            </motion.div>
...
export function LiveTracking() {
  ...
  // Self-fetch if navigated directly (store might be empty)
  useEffect(() => {
    if (storeShipments.length === 0) {
  ...
  const shipments = storeShipments;
    ...
    return shipments.map((ship) => {
      ...
      return {
        ...
        status:
          rt.stage === "IN_TRANSIT"
            ? "active"
            : (rt.stage?.toLowerCase() ?? ship.status),
        current_risk: {
          ...ship.current_risk,
  ...
  const fetchDetail = useCallback(async (ship: Shipment) => {
    try {
      const [sr, re] = await Promise.all([
        getSensorHistory(ship.id),
        getRiskEvents(ship.id),
      ]);
      setSensors(sr);
      setRiskEvents(re);
    } catch {
  ...
  const s = selected
    ? displayShipments.find((x) => x.id === selected.id) || selected
    : null;
  ...
  const liveRemKm =
    rtLive?.remaining_km ?? Math.round((100 - liveProgress) * 2.14);
  const liveTravelKm = rtLive
    ? Math.round(
        ((rtLive.progress_pct ?? 0) / 100) *
          (liveRemKm + (rtLive.progress_pct / 100) * 200),
      )
    : Math.round(liveProgress * 2.14);
  const liveBattery =
    rtLive?.battery_pct != null ? `${Math.round(rtLive.battery_pct)}%` : "78%";
  const liveDoor = rtLive?.door_status ?? "Closed";
  const liveSpeed =
    rtLive?.speed_kmh != null ? `${Math.round(rtLive.speed_kmh)} km/h` : "—";
  ...
  return (
    <div
      className={`flex flex-col bg-[#080B12] text-[#F1F5F9] overflow-hidden ${fullscreen ? "fixed inset-0 z-50" : "h-screen"}`}
    ...
    >
      ...
      >
        ...
        <div className="hidden md:flex bg-[#111827] border border-[#1E2530] rounded-lg p-0.5 mr-2">
          <button
            ...
            className={`px-3 py-1 text-xs rounded-md font-medium transition-colors ${mapMode === "2d" ? "bg-[#1E2530] text-white" : "text-[#64748B] hover:text-[#CBD5E1]"}`}
          >
            2D Route
          </button>
          <button
            onClick={() => setMapMode("3d")}
            className={`px-3 py-1 text-xs rounded-md font-medium transition-colors ${mapMode === "3d" ? "bg-[#6366F1]/20 text-[#818CF8]" : "text-[#64748B] hover:text-[#CBD5E1]"}`}
          >
            3D Globe
          </button>
        </div>
        ...
        <motion.button
          ...
          onClick={() => {
            setWeatherOn((w) => !w);
            if (!weatherOn) {
              setTimeout(() => setDeviation(true), 8000);
            } else {
              setDeviation(false);
            }
          }}
          className={`hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold transition-all mr-1 ${
            weatherOn
              ? "bg-[#3B82F6]/15 border-[#3B82F6]/50 text-[#60A5FA]"
        ...
        <div className="flex items-center gap-2 text-[10px] font-mono bg-[#1E2530]/50 px-3 py-1.5 rounded-full border border-[#1E2530]">
          <span
            className={`w-2 h-2 rounded-full ${connected ? "bg-[#34D399] animate-pulse" : "bg-[#EF4444]"}`}
          />
          <span className={connected ? "text-[#34D399]" : "text-[#EF4444]"}>
            {connected ? "LIVE · GPS ACTIVE" : "RECONNECTING..."}
          </span>
        </div>
        <button
          onClick={() => navigate("/active-shipments")}
          className="text-xs text-[#64748B] hover:text-[#CBD5E1] border border-[#1E2530] bg-[#111827] px-3 py-1.5 rounded-lg transition-colors"
        >
          ← Shipments
        </button>
      </motion.header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <motion.aside
          initial={{ x: -200 }}
          animate={{ x: 0 }}
          className="w-56 shrink-0 bg-[#0D1117] border-r border-[#1E2530] flex flex-col overflow-hidden"
        >
          <div className="shrink-0 border-b border-[#1E2530]">
            <div className="px-3 py-2 text-[10px] text-[#64748B] uppercase tracking-widest font-semibold flex items-center justify-between">
              ...
              <span className="bg-[#1E2530] text-[#94A3B8] px-1.5 py-0.5 rounded-full text-[9px]">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-[10px] text-[#64748B] truncate">
                      {pIcon(ship.product_type)} {ship.origin?.split(",")[0]} →{" "}
                      {ship.destination?.split(",")[0]}
                    </div>
                  </button>
                );
              })}
          ...
          {s && (
            <>
              <div className="shrink-0 px-3 py-2 border-b border-[#1E2530] bg-[#0A0D14]">
                ...
                <div className="font-mono text-xs font-bold text-[#4DD9AC]">
                  {s.shipment_code}
                </div>
              </div>
              <StageRail
        ...
        <div className="flex flex-1 min-w-0 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <>
                  ...
                  <div className="absolute top-3 right-3 flex items-center gap-1.5 z-10">
                    <button
                      ...
                      className="bg-[#0D1117]/90 backdrop-blur border border-[#1E2530] text-[#64748B] hover:text-white p-1.5 rounded-lg transition-colors text-xs pointer-events-auto"
                    >
                      {fullscreen ? "⤡" : "⤢"}
                    </button>
                  </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div
                    className={`text-xs font-bold px-3 py-1 rounded-full ${
                      liveEtaMin
                        ? "bg-[#4DD9AC]/10 text-[#4DD9AC] border border-[#4DD9AC]/20"
                        : sensors[0]?.delay_minutes &&
                            sensors[0].delay_minutes > 30
            ...
            {s && (
              <div className="shrink-0 bg-[#0D1117] border-t border-[#1E2530] px-4 py-3 flex items-center gap-2 flex-wrap">
                <span className="text-[10px] text-[#64748B] uppercase tracking-widest mr-2 font-semibold">
                  Quick Actions
                </span>
                {[
                  {
                    ...
                    l: "⚠️ Alert Driver",
                    c: "#EF4444",
                    p: "/api/v1/alerts/send-manual",
                    b: {
                      shipment_id: s.shipment_code,
                      alert_type: "TEMP_BREACH",
                      channel: "WHATSAPP",
                    },
                    sm: "Alert sent to driver",
                  },
                  {
                    k: "escalate",
                    l: "🚨 Escalate",
                    c: "#F97316",
                    p: "/api/v1/interventions/alert-driver",
                    b: {
                      shipment_id: s.shipment_code,
                      alert_type: "ESCALATION",
                      escalate_to: "FLEET_MANAGER",
                    },
                    sm: "Escalated to supervisor",
                  },
                  {
                    k: "reroute",
                    l: "🛣️ Reroute AI",
                    c: "#A78BFA",
                    p: "/api/v1/interventions/reroute-impact",
                    b: { shipment_id: s.shipment_code },
                    sm: "AI Reroute simulation sent",
                  },
                  {
                    k: "coldhub",
                    l: "🏭 Cold Hub",
                    c: "#60A5FA",
                    p: "/api/v1/interventions/trigger-agent",
                    b: { shipment_id: s.shipment_code, scenario: "temp_spike" },
                    sm: "Cold hub navigation initialized",
                  },
                ].map((btn) => (
                  <button
                    key={btn.k}
                    disabled={actionLoading === btn.k}
          ...
          >
            ...
            <div className="shrink-0 px-4 pt-4 pb-3 border-b border-[#1E2530]">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-1.5 text-[10px] text-[#34D399]">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#34D399] animate-pulse" />
                  {latestSensor
                    ? `${Math.floor((Date.now() - new Date(latestSensor.recorded_at!).getTime()) / 1000)}s ago`
                    : "Waiting"}
                </div>
              </div>
              <motion.div
              ...
              >
                ...
                <div className="flex items-end gap-2">
                  ...
                  {isBreach && (
                    <span className="text-xs text-[#F87171] mb-1 font-bold">
                      +{((latestSensor?.temperature ?? 0) - tMax).toFixed(1)}°
                    </span>
                  )}
                </div>
                <div className="text-[10px] text-[#4A5568] mt-1 font-mono">
                  Safe band: {tMin}°C – {tMax}°C
                </div>
              </motion.div>
              {/* Readings */}
                ...
                {[
                  {
                    icon: "🌤",
                    label: "Ambient Temp",
                    value: liveAmbient ? `${liveAmbient.toFixed(0)}°C` : "38°C",
                    warn: (liveAmbient ?? 0) > 35,
                  },
                  {
                    icon: "💧",
                    label: "Humidity",
                    value: liveHumidity
                      ? `${(liveHumidity as number).toFixed(0)}%`
                      : "—",
                    warn: ((liveHumidity as number) ?? 0) > 75,
                  },
                  { icon: "🚛", label: "Speed", value: liveSpeed, warn: false },
                  {
                    ...
                    value: liveBattery,
                    warn: liveBattery !== "78%" && parseInt(liveBattery) < 20,
                  },
                  {
                    icon: "🚪",
                    label: "Door Status",
                    value: liveDoor,
                    warn: liveDoor === "OPEN" || liveDoor === "OPENED",
                  },
                ].map((row) => (
                  <div
                    key={row.label}
                    className="flex items-center justify-between text-[11px] bg-[#111827] px-3 py-1.5 rounded-md border border-[#1E2530]"
                  >
            ...
            <div className="shrink-0 px-4 py-3 border-b border-[#1E2530]">
              <div className="text-[10px] text-[#64748B] uppercase tracking-widest mb-2 font-semibold">
                Temp (Last 30 min)
              </div>
              <MiniSparkline
                    ...
                    {s.current_risk?.time_to_spoil_minutes && (
                      <div className="text-[11px] text-[#64748B] mt-0.5">
                        ...
                        <span className="font-mono font-bold text-[#FBBF24]">
                          {fmtSpoil(s.current_risk.time_to_spoil_minutes)}
                        </span>
                      </div>
                    )}
                  </div>
            ...
            {s && (
              <div className="shrink-0 p-4">
                <button
                  ...
                  className="w-full text-xs bg-gradient-to-r from-[#4DD9AC] to-[#34C994] text-[#003829] font-bold hover:brightness-110 py-2.5 rounded-lg transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#4DD9AC]/20"
                >
                  + Create New Shipment
                </button>
              </div>
