...
function pIcon(t?: string) {
  return PRODUCT_ICONS[t?.toLowerCase() ?? ""] ?? "ðŸ“¦";
}
...
function getRiskCat(s: Shipment) {
  return s.current_risk?.risk_category?.toUpperCase() ?? "LOW";
}
function getRiskScore(s: Shipment) {
  return Math.round((s.current_risk?.risk_score ?? 0) * 100);
}

// SOP steps by product type
const SOP: Record<string, string[]> = {
  dairy: [
    "Alert driver immediately via WhatsApp",
    "Driver checks reefer unit and seals",
    "If breach >15 min â†’ escalate to manager",
    "If reefer fails â†’ divert to cold hub",
    "Document breach start time and temp",
    "Capture photographic evidence",
    "Notify recipient of potential delay",
  ],
  seafood: [
    "Check reefer temperature reading",
    "Driver verifies ice pack status if applicable",
    "Escalate immediately if >4Â°C",
    "Consider diversion to nearest cold hub",
    "Notify consignee of possible delay",
    "Document all actions taken",
  ],
  pharma: [
    "Alert driver â€” check reefer immediately",
    "Verify sensor calibration status",
    "If breach >2Â°C â€” stop shipment at nearest hub",
    "Contact quality assurance team",
    "Generate deviation report",
    "Issue hold notice for cargo",
  ],
  default: [
    "Alert driver via WhatsApp",
    "Check reefer unit status",
    "Escalate to fleet manager if unresolved >10 min",
    "Consider cold hub diversion",
    "Document breach in system",
    "Notify recipient",
  ],
};
...
function RiskCard({
  ...
  const myAlerts = alerts.filter(
    (a) =>
      a.shipment_id === shipment.id ||
      a.shipment_code === shipment.shipment_code,
  );
  ...
  return (
    <button
      ...
      style={{
        ...
        boxShadow: selected
          ? `0 0 0 1px ${color}30, 0 4px 24px rgba(0,0,0,0.3)`
          : "0 2px 12px rgba(0,0,0,0.2)",
        animation: isCrit ? "pulseBorder 3s ease-in-out infinite" : undefined,
      }}
    >
      {/* Watermark */}
      <div className="absolute right-3 top-3 text-4xl opacity-5 pointer-events-none select-none">
        {pIcon(shipment.product_type)}
      </div>
      ...
      <div className="text-xs text-[#94A3B8] mb-2">
        {pIcon(shipment.product_type)}{" "}
        <span className="capitalize">{shipment.product_type}</span>
        {" Â· "}
        <span className="text-[#64748B]">
          {shipment.origin?.split(",")[0]} â†’{" "}
      ...
      <div className="space-y-1 mb-3">
        {isBreach && latestS && (
          <div className="flex items-center gap-1.5 text-[11px]">
            ...
            <span className="text-[#F87171]">
              Cargo temp: {latestS.temperature?.toFixed(1)}Â°C
            </span>
            <span className="text-[#64748B]">
              (safe: {tMin}â€“{tMax}Â°C)
            </span>
        ...
        {delay > 0 ? (
          <div className="text-[11px] text-[#FBBF24]">
            â± Delay: +{delay} min from planned route
          </div>
        ) : (
          <div className="text-[11px] text-[#34D399]">
            â± On time â€” no delay
          </div>
        )}
      </div>
      ...
      <div className="text-[10px] flex items-center gap-1.5 text-[#64748B]">
        ...
        <span
          className={`ml-1 font-bold ${alertSent ? "text-[#34D399]" : "text-[#FBBF24]"}`}
        >
...
const CM_STEPS: CMStep[] = [
  {
    ...
    icon: "🔍",
    detail: "Gemini ADK scans 47 IoT telemetry parameters — Severity: CRITICAL",
    auto: true,
    delay: 0,
    color: "#A78BFA",
  },
  {
    id: "s2",
    label: "Driver WhatsApp Alert",
    icon: "📲",
    detail: "Auto-dispatched to +91-98XX-XXXXX — ACK within 45s",
    auto: true,
    delay: 4000,
    color: "#60A5FA",
  },
  {
    id: "s3",
    label: "Cold Hub Redirect",
    icon: "🏭",
    detail: "Nearest NovaCold Hub — 12.4 km — Dock 4B reserved",
    auto: true,
    delay: 9000,
    color: "#FBBF24",
  },
  {
    id: "s4",
    label: "Reroute via Mapbox AI",
    icon: "🗺️",
    detail: "Alternate NH-48 route — ETA adjusted +22 min",
    auto: true,
    delay: 15000,
    color: "#34D399",
  },
  {
    id: "s5",
    label: "Blockchain Audit Log",
    icon: "⛓️",
    detail: "Incident hash committed to Sepolia — Block #8,847,221",
    auto: true,
    delay: 21000,
    color: "#F97316",
  },
...
function AICountermeasureEngine({
  score,
  shipmentCode,
  onToast,
}: {
  ...
  return (
    <div className="bg-[#060912] border border-[#1E2530] rounded-2xl overflow-hidden mb-4">
      {/* Header */}
      <div
        className="px-4 py-3 flex items-center justify-between border-b border-[#1E2530]"
        style={{
          background: isHot
            ? "linear-gradient(90deg,rgba(239,68,68,0.06),rgba(167,139,250,0.06))"
            : "transparent",
        }}
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[9px] text-[#64748B]">
              Gemini ADK Multi-Agent · 5-Step Response Protocol
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {done ? (
            <button
              onClick={reset}
              className="text-[10px] text-[#64748B] border border-[#1E2530] px-2 py-1 rounded hover:border-[#374151] transition-colors"
            >
              ↩ Reset
            </button>
          ) : (
      ...
      <div className="px-4 py-3 space-y-2">
        {!isHot && !running && !done && (
          <div className="text-[10px] text-[#4A5568] text-center py-3 flex flex-col items-center gap-1">
            ...
            <span>
              Risk score must exceed{" "}
              <strong className="text-[#FBBF24]">65</strong> to arm
              countermeasures.
            </span>
          </div>
        )}
        {CM_STEPS.map((step, idx) => {
          ...
          const isPending =
            running && !isActive && !isComplete && elapsed < step.delay;
          ...
          return (
            ...
            >
              <div className="shrink-0 text-base mt-0.5">
                {isComplete ? (
                  <CheckCheck size={14} className="text-[#34D399] mt-0.5" />
                ) : isActive ? (
                  <motion.div
                    animate={{ rotate: 360 }}
                    transition={{
                  ...
                  >
                    <Loader2 size={14} style={{ color: step.color }} />
                  </motion.div>
                ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                {(isActive || isComplete) && (
                  <motion.p
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="text-[9px] text-[#64748B] mt-0.5 leading-relaxed"
                  >
                    {step.detail}
                  </motion.p>
                )}
...
function InterventionPanel({
  ...
  const delay = latestS?.delay_minutes ?? 0;
  const explanation =
    shipment.current_risk?.explanation ?? riskEvts[0]?.explanation;
  ...
  const factorIconMap: Record<string, string> = {
    "Cargo temp above safe band": "ðŸŒ¡",
    "Transit delay": "â±",
    "Ambient heat index": "ðŸŒ¤",
    "Temperature breach duration": "ðŸ”¥",
    "Reefer unit degradation": "ðŸ§Š",
    "High humidity": "ðŸ’§",
    "Sensor connectivity gap": "ðŸ“¡",
    "Door open time": "ðŸšª",
    "Time-of-day risk": "ðŸ•",
  };
  const factors =
    Object.keys(liveFactors).length > 0
      ? Object.entries(liveFactors)
          .filter(([, p]) => (p as number) > 0)
          .sort(([, a], [, b]) => (b as number) - (a as number))
          .map(([label, p]) => ({
            icon: factorIconMap[label] ?? "âš¡",
            label,
            pts: p as number,
      ...
      : [
          {
            icon: "ðŸŒ¡",
            label: "Cargo temp above safe band",
            pts: isBreach ? 22 : 0,
            detail: `Cargo: ${latestS?.temperature?.toFixed(1) ?? "â€”"}Â°C Â· Safe max: ${tMax}Â°C`,
            why: `Temperature-sensitive cargo spoils exponentially faster above safe ceiling.`,
          },
          {
            icon: "â±",
            label: "Transit delay",
            pts: delay > 20 ? 18 : delay > 0 ? 8 : 0,
            detail: `Current delay: +${delay} min`,
            why: `Delay extends total exposure time, compressing the safe delivery window.`,
          },
          {
            icon: "ðŸŒ¤",
            label: "High ambient temperature",
            pts: (latestS?.ambient_temp ?? 0) > 32 ? 9 : 0,
            detail: `Ambient: ${latestS?.ambient_temp?.toFixed(0) ?? 38}Â°C`,
            why: `High ambient heat increases reefer workload.`,
          },
          {
            icon: "ðŸ“¡",
            label: "Sensor connectivity gap",
            pts: sensors.length < 3 ? 5 : 0,
            detail: `Last sync: ${latestS?.recorded_at ? `${Math.floor((Date.now() - new Date(latestS.recorded_at).getTime()) / 60000)} min ago` : "Unknown"}`,
            why: `Sensor gaps create visibility blindspots.`,
          },
        ].filter((f) => f.pts > 0);
  ...
  const predictions = [
    {
      cond: "If current conditions persist 15 min",
      outcome: "CRITICAL",
      delta: "+12",
      color: "#EF4444",
    },
    {
      cond: "If reefer restored in next 10 min",
      outcome: "Drop to MEDIUM",
      delta: "-28",
      color: "#34D399",
    },
    {
      cond: "If rerouted to cold hub now",
      outcome: "Risk drops 31%",
      delta: "-31%",
      color: "#34D399",
    },
    {
      cond: "If delay resolved in next 20 min",
      outcome: "Risk drops 12%",
      delta: "-12%",
      color: "#60A5FA",
    },
  ];
  ...
  const sopSteps =
    SOP[shipment.product_type?.toLowerCase() ?? ""] ?? SOP.default;
    ...
    >
      ...
      <div className="shrink-0 px-5 py-4 border-b border-[#1E2530] bg-[#0A0D14] sticky top-0 z-10">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            <span className="font-mono text-lg font-black text-[#4DD9AC]">
              {shipment.shipment_code}
            </span>
            <span
              className="text-[10px] font-black px-2 py-0.5 rounded-full"
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
        ...
        <div className="bg-[#111827] rounded-xl p-4 relative overflow-hidden">
          ...
          />
          <div className="text-[10px] text-[#64748B] uppercase tracking-widest mb-3">
            Risk Score
          </div>
              ...
              <div className="space-y-1.5 text-xs">
                {spoil && (
                  <div className="flex justify-between">
                    <span className="text-[#64748B]">âš¡ Spoilage window</span>
                ...
                {isBreach && (
                  <div className="flex justify-between">
                    <span className="text-[#64748B]">ðŸŒ¡ Temp delta</span>
                    <span className="font-mono text-[#F87171]">
                      +{tempDelta}Â°C above safe
                    </span>
                  </div>
                )}
                {delay > 0 && (
                  <div className="flex justify-between">
                    <span className="text-[#64748B]">â± Breach ongoing</span>
        ...
        <div className="bg-[#111827] rounded-xl p-4">
          <div className="text-[10px] text-[#64748B] uppercase tracking-widest mb-3 font-semibold">
            Risk Factors
          </div>
            ...
            {factors.length > 0 && (
              <div className="border-t border-[#1E2530] pt-2 mt-2 space-y-1 text-xs">
                <div className="flex justify-between text-[#64748B]">
                  <span>Baseline (product + route)</span>
                  <span className="font-mono">{Math.max(0, baseline)}</span>
                </div>
        ...
        <div className="bg-[#111827] rounded-xl p-4">
          ...
          <div className="space-y-2">
            {predictions.map((p, i) => (
              <div
                key={i}
                className="flex items-center gap-2 text-xs py-1.5 border-b border-[#1E2530] last:border-0"
              >
        ...
        <div className="bg-[#111827] rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="text-[10px] text-[#64748B] uppercase tracking-widest font-semibold">
              Why This Risk?
            </div>
          ...
          {driverMode ? (
            <div className="bg-[#0D1A2E] border border-[#1E3A5F] rounded-lg p-3">
              <div className="text-[10px] text-[#60A5FA] uppercase tracking-widest mb-2 font-bold">
                ðŸš› Driver-friendly version
              </div>
            ...
            <div>
              ...
              {(explanation?.length ?? 0) > 160 && (
                <button
                  ...
                  className="text-[10px] text-[#60A5FA] mt-2 hover:text-white transition-colors"
                >
            ...
            {[
              {
                key: "alert",
                icon: "âš ï¸",
                label: "Alert Driver Now",
                sub: "WhatsApp + Push notification",
                color: "#EF4444",
                fn: async () => {
                  try {
                    ...
                    doneAction(
                      "alert",
                      "âš ï¸ Alert sent to driver via ADK agent",
                    );
                  } catch {
                    doneAction("alert", "âš ï¸ Alert queued (offline)");
                  }
                },
              },
              {
                key: "call",
                icon: "ðŸ“ž",
                label: "Call Driver Directly",
                sub: `Call ${shipment.driver_phone ?? "driver"}`,
                color: "#60A5FA",
                fn: () => doneAction("call", "ðŸ“ž Calling driver..."),
              },
              {
                key: "whatsapp",
                icon: "ðŸ’¬",
                label: "Send WhatsApp Template",
                sub: "Temperature alert SOP",
                color: "#34D399",
                fn: async () => {
                  try {
                    ...
                    doneAction(
                      "whatsapp",
                      "ðŸ’¬ WhatsApp message sent to driver",
                    );
                  } catch (e) {
                    doneAction(
                      "whatsapp",
                      "âš ï¸ Failed to send WhatsApp",
                      "warn",
                    );
                  }
                },
              },
              {
                key: "escalate",
                icon: "ðŸš¨",
                label: "Escalate to Fleet Manager",
                sub: "Fleet Manager Â· Auto-notify",
                color: "#F97316",
                fn: () => doneAction("escalate", "ðŸš¨ Escalated", "warn"),
              },
              {
                key: "reroute",
                icon: "🛣️",
                label: "Suggest Reroute (ADK Agent)",
                sub: "Autonomous rerouting via Gemini ADK",
                color: "#A78BFA",
                fn: async () => {
                  try {
                    await triggerRerouteAgent({
                      shipment_code: shipment.shipment_code,
                      force: true,
                    });
              ...
              },
              {
                key: "coldhub",
                icon: "ðŸ­",
                label: "Nearest Cold Hub",
                sub: "Meghalaya Hub Â· 11.2 km",
                color: "#60A5FA",
                fn: () => {
                  setColdHubOpen(true);
                  doneAction("coldhub", "ðŸ­ Cold hub navigation sent");
                },
              },
              {
                key: "monitor",
                icon: "ðŸ‘ï¸",
                label: "Mark: Manual Monitoring",
                sub: "Remove from auto-alert queue",
                color: "#64748B",
                fn: () =>
                  doneAction("monitor", "ðŸ‘ï¸ Manual monitoring enabled"),
              },
              {
                key: "fp",
                icon: "âœ…",
                label: "Mark: False Positive",
                sub: "Flag as reviewed, no action needed",
                color: "#374151",
                fn: () => doneAction("fp", "âœ… Marked as false positive"),
              },
            ].map((btn) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[#4A5568] text-xs group-hover:text-current transition-colors">
                    â†’
                  </span>
                </button>
              );
          ...
          <div>
            ...
            {coldHubOpen && (
              <div className="bg-[#0F1A2E] border border-[#1E3A5F] border-t-0 rounded-b-xl px-4 pb-4 space-y-2">
                <div className="text-sm font-bold text-[#F1F5F9] pt-3">
                  Meghalaya Cold Storage Hub
                </div>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-[11px]">
                  {[
                    ["Distance", "11.2 km"],
                    ["Diversion", "14 min"],
                    ["Capacity", "Available (3 slots)"],
                    ["Spoilage impact", "-28% if diverted now"],
                    ["SLA impact", "+14 min to delivery"],
                    ["Cost est.", "â‚¹2,400 diversion fee"],
                  ].map(([k, v]) => (
                    <div key={k}>
                      <span className="text-[#64748B]">{k}: </span>
                      <span className="text-[#CBD5E1] font-medium">{v}</span>
                    </div>
                  ))}
                </div>
                <div className="text-[11px] text-[#34D399] bg-[#34D399]/8 border border-[#34D399]/20 rounded px-3 py-2 font-semibold">
                  If diverted: spoilage risk drops from HIGH (74%) to MEDIUM
                  (46%). Recommended.
                </div>
                <div className="flex gap-2 pt-1">
                  {[
                    {
                      l: "ðŸ“ Navigate Driver",
                      fn: () => doneAction("nav", "ðŸ“ Driver navigation sent"),
                    },
                    {
                      l: "ðŸ“ž Call Hub",
                      fn: () =>
                        doneAction("callhub", "ðŸ“ž Calling hub manager..."),
                    },
                    {
                      l: "ðŸ“‹ Book Slot",
                      fn: () =>
                        doneAction("book", "ðŸ“‹ Slot booked at cold hub"),
                    },
                  ].map((b) => (
                    <button
                      key={b.l}
        ...
        <div>
          ...
          >
            <span>
              ðŸ“‹ {shipment.product_type?.toUpperCase() ?? "PRODUCT"}{" "}
              TEMPERATURE BREACH SOP
            </span>
          ...
          {sopOpen && (
            <div className="bg-[#111827] border border-[#1E2530] border-t-0 rounded-b-xl px-4 pb-4">
              <div className="space-y-2 pt-3">
                {sopSteps.map((step, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-5 h-5 shrink-0 rounded-full border-2 flex items-center justify-center mt-0.5 transition-all ${done ? "bg-[#34D399] border-[#34D399]" : "border-[#374151] group-hover:border-[#64748B]"}`}
                      >
                        {done && (
                          <span className="text-[8px] text-[#003829] font-black">
                            âœ“
                          </span>
                        )}
                      </div>
                      <span
                        className={`flex-1 leading-relaxed ${done ? "text-[#4A5568] line-through" : "text-[#CBD5E1]"}`}
                      >
                        Step {i + 1}: {step}
                      </span>
                    </button>
              ...
              <div className="flex gap-2 mt-3">
                <button
                  onClick={() => setSopDone(new Set(sopSteps.map((_, i) => i)))}
                ...
                <button
                  onClick={() => setSopDone(new Set())}
                  className="text-[10px] text-[#64748B] border border-[#1E2530] px-3 py-1.5 rounded hover:border-[#374151] transition-colors"
                >
                  Reset
                </button>
              </div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[10px] text-[#60A5FA] uppercase tracking-widest font-bold">
              ðŸ¤ Gemini Negotiator
            </span>
            <span className="text-[9px] bg-[#60A5FA]/10 border border-[#60A5FA]/25 text-[#60A5FA] px-2 py-0.5 rounded-full">
              AI â€” Emergency Storage
            </span>
          </div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[10px] text-[#4DD9AC] uppercase tracking-widest font-bold">
              ðŸ—ºï¸ AI Route Optimizer
            </span>
            <span className="text-[9px] bg-[#4DD9AC]/10 border border-[#4DD9AC]/25 text-[#4DD9AC] px-2 py-0.5 rounded-full">
              AI â€” Live Rerouting
            </span>
          </div>
...
const NEG_SCRIPT = (
...
): NegotiationMsg[] => [
  {
    ...
    from: "system",
    ts: "00:00",
    text: `ðŸ”´ Gemini Agent initiating emergency negotiation for ${shipCode}. Contacting ${hub.name}â€¦`,
  },
  {
    id: "n1",
    from: "gemini",
    ts: "00:03",
    text: `Hello, this is Cargofy AI. We have a CRITICAL cold-chain emergency on shipment ${shipCode} â€” cargo temp breach, reefer failure risk. We need emergency cold storage immediately. Do you have capacity?`,
  },
  {
    id: "n2",
    from: "warehouse",
    ts: "00:08",
    text: `Yes, we have ${hub.capacity} slots available. Standard rate is ${hub.rate}. ETA from your current location is about ${hub.eta}.`,
  },
  {
    id: "n3",
    from: "gemini",
    ts: "00:12",
    text: `Understood. Our cargo value is â‚¹18.4L â€” spoilage risk is CRITICAL. Can you confirm immediate slot reservation and expedited intake without pre-booking paperwork?`,
  },
  {
    id: "n4",
    from: "warehouse",
    ts: "00:20",
    text: `Yes, we can do emergency intake. Send the vehicle registration and consignment note via WhatsApp to our manager. Slot held for 25 minutes.`,
  },
  {
    id: "n5",
    from: "gemini",
    ts: "00:24",
    text: `Confirmed. Sending vehicle details now. Requesting priority cold tunnel access on arrival. Estimated arrival: ${hub.eta}. Booking reference: CRG-EMG-${Math.floor(Math.random() * 9000 + 1000)}.`,
  },
  {
    id: "n6",
    from: "system",
    ts: "00:28",
    text: `âœ… Negotiation complete. Slot secured at ${hub.name}. Driver navigation sent. Estimated cargo save: â‚¹18.4L.`,
  },
];
    ...
    <div className="flex flex-col h-full bg-[#080B12] rounded-xl overflow-hidden border border-[#1E2530]">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] text-[#64748B]">
              Emergency cold storage negotiation Â· {shipment.shipment_code}
            </div>
          </div>
        </div>
        <motion.button
          ...
          className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-bold transition-all ${
            running
              ? "bg-[#60A5FA]/10 text-[#60A5FA] cursor-wait"
              : "bg-[#60A5FA] text-[#030712] hover:bg-[#93C5FD]"
          }`}
        >
          {running ? (
      ...
      <div className="shrink-0 px-4 py-2 border-b border-[#1E2530] flex gap-2 overflow-x-auto">
        {NEGOTIATION_HUBS.map((h, i) => (
          <button
            key={h.name}
            onClick={() => !running && setSelectedHub(i)}
      ...
      <div className="shrink-0 px-4 py-2 border-b border-[#1E2530] grid grid-cols-4 gap-3">
        {[
          ...
          <div key={label} className="text-center">
            <div className="text-[9px] text-[#64748B] uppercase tracking-widest">
              {label}
            </div>
            <div className="text-xs font-bold text-[#F1F5F9] mt-0.5">
              {value}
            </div>
          </div>
        ))}
      ...
      >
        {messages.length === 0 && !running && (
          <div className="flex flex-col items-center justify-center h-24 gap-2 opacity-50">
            ...
            <p className="text-xs text-[#64748B] text-center">
              Click <strong>Start Negotiation</strong> to watch Gemini AI
              autonomously call the cold hub manager and secure a slot
            </p>
          </div>
        )}
        <AnimatePresence>
          {messages.map((msg) => {
            ...
            return (
              <motion.div
                key={msg.id}
                initial={{ opacity: 0, x: isGemini ? 20 : -20 }}
                animate={{ opacity: 1, x: 0 }}
                className={`flex flex-col gap-0.5 ${isGemini ? "items-end" : "items-start"}`}
              >
                {isGemini ? (
                  <span className="text-[9px] text-[#60A5FA]/70 px-1 flex items-center gap-1">
                    <Bot size={9} /> Gemini AI
                  </span>
                ) : (
                  <span className="text-[9px] text-[#64748B] px-1 flex items-center gap-1">
                    <Building2 size={9} /> {hub.name}
                  </span>
                )}
                <div
                  className={`max-w-[85%] px-3 py-2.5 rounded-xl text-xs leading-relaxed border ${
                    isGemini
                ...
                <div
                  className={`text-[9px] text-[#4A5568] px-1 flex items-center gap-1 ${isGemini ? "flex-row-reverse" : ""}`}
                >
                  <span>{msg.ts}</span>
                  {isGemini && (
        ...
        {typing && (
          ...
          >
            ...
            <span className="text-[10px] text-[#60A5FA] mb-1">
              Gemini composingâ€¦
            </span>
          </motion.div>
        )}
        ...
        >
          ...
          <div className="flex-1">
            <span className="text-xs font-bold text-[#34D399]">
              Slot Secured at {hub.name}
            </span>
            <span className="text-[10px] text-[#64748B] ml-2">
              Â· {hub.dist} away Â· ETA {hub.eta} Â· {hub.rate}
            </span>
          </div>
  ...
  return [
    {
      id: "r0",
      name: "Current Route",
      via: shipment.origin?.split(",")[0] ?? "Origin",
      distKm: 142,
      etaMin: 195,
      riskDelta: 0,
      spoilDelta: 0,
      aiScore: 100 - risk,
      reason:
        "Baseline route â€” current conditions apply. Reefer failure risk if unchanged.",
      recommended: false,
      tags: ["âš ï¸ Current", `Risk: ${risk}/100`],
    },
    {
      id: "r1",
      name: "NH-6 Expressway Alt",
      via: "Nongpoh Bypass",
      distKm: 118,
      etaMin: 148,
      riskDelta: -22,
      spoilDelta: Math.round(base * 0.18),
      aiScore: 100 - risk + 22,
      reason:
        "Shorter route avoids high-altitude temperature variation. NH-6 bypass reduces transit time by 47 min, compressing exposure window significantly.",
      recommended: true,
      tags: ["âš¡ Fastest", "ðŸŸ¢ AI Pick", "âˆ’22 risk pts"],
    },
    {
      id: "r2",
      name: "Cold Hub Diversion",
      via: `${shipment.destination?.split(",")[0] ?? "Dest"} via Meghalaya Hub`,
      distKm: 153,
      etaMin: 209,
      riskDelta: -31,
      spoilDelta: Math.round(base * 0.31),
      aiScore: 100 - risk + 31 - 8,
      reason:
        "Route via emergency cold hub for cargo temperature reset. Longer ETA but eliminates spoilage risk. Recommended for CRITICAL breaches only.",
      recommended: false,
      tags: ["ðŸ­ Safe", "âˆ’31 risk pts", "+14 min ETA"],
    },
    {
      id: "r3",
      name: "Night Route (Delay)",
      via: "State Highway 5 â€” low traffic",
      distKm: 168,
      etaMin: 171,
      riskDelta: -9,
      spoilDelta: Math.round(base * 0.07),
      aiScore: 100 - risk + 9 - 5,
      reason:
        "Night road conditions reduce ambient temperature exposure by 6Â°C. Slightly longer distance but cooler ambient means less reefer stress.",
      recommended: false,
      tags: ["ðŸŒ™ Night route", "âˆ’9 risk pts"],
    },
  ];
}
      ...
      <div className="px-4 py-3 border-b border-[#1E2530]">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 relative h-1 bg-[#1E2530] rounded-full">
            <motion.div
              ...
              animate={{
                width: optimizing ? "60%" : optimized ? "100%" : "30%",
              }}
            ...
            {optimizing && (
              ...
              >
                <Truck
                  size={8}
                  className="text-[#003829] absolute inset-0 m-auto"
                />
              </motion.div>
            )}
          </div>
        ...
        <div className="mt-1.5 flex gap-3 text-[10px]">
          {[
            {
              label: "Current route",
              value: `${sel.distKm} km Â· ${sel.etaMin} min`,
            },
            {
              label: "AI Score",
              value: `${sel.aiScore}/100`,
              color: scoreColor(sel.aiScore),
            },
            {
              label: "Risk delta",
              value:
                sel.riskDelta === 0
                  ? "Baseline"
                  : `${sel.riskDelta > 0 ? "+" : ""}${sel.riskDelta} pts`,
              color: sel.riskDelta < 0 ? "#34D399" : "#F87171",
            },
            ...
            <div key={r.id}>
              <motion.button
                ...
                className={`w-full text-left rounded-lg px-3 py-2.5 border transition-all ${
                  isSelected
                    ? `border-[${col}]/40 bg-[${col}]/05`
                    : "border-[#1E2530] bg-[#111827]/60"
                }`}
                style={{
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div className="w-10 h-10 shrink-0 relative">
                    ...
                    >
                      ...
                      <circle
                        ...
                        strokeLinecap="round"
                        style={{
                          filter: `drop-shadow(0 0 4px ${scoreColor(r.aiScore)}60)`,
                        }}
                      />
                    </svg>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-[#64748B]">
                      via {r.via} Â· {r.distKm} km Â· {r.etaMin} min
                    </div>
                    <div className="flex gap-1.5 mt-1 flex-wrap">
                      {r.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[9px] px-1.5 py-0.5 rounded bg-[#0D1117] border border-[#1E2530] text-[#94A3B8]"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="shrink-0 flex flex-col items-end gap-1">
                    {r.riskDelta !== 0 && (
                      <span
                        className="text-[10px] font-bold"
                        style={{
                          color: r.riskDelta < 0 ? "#34D399" : "#F87171",
                        }}
                      >
                        {r.riskDelta > 0 ? "+" : ""}
                        {r.riskDelta} risk
                    ...
                    {r.spoilDelta > 0 && (
                      <span className="text-[10px] text-[#4DD9AC] font-mono">
                        +{r.spoilDelta}m window
                      </span>
                    )}
                    <ChevronRight
                      size={12}
...
export function RiskInterventions() {
  ...
  const [toasts, setToasts] = useState<
    Array<{ id: string; msg: string; type: "ok" | "warn" }>
  >([]);
  const [statsOpen, setStatsOpen] = useState(true);
  const { data: rtActiveShipments } =
    useRealtimeData<Record<string, any>>("/active_shipments");
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [ships, alts] = await Promise.all([
        getShipments("all"),
        getAlerts(),
      ]);
      setShipments(ships);
  ...
  const critCount = sortedShips.filter(
    (s) => getRiskCat(s) === "CRITICAL",
  ).length;
  ...
  if (loading)
    return (
      <div
        className="flex items-center justify-center h-screen bg-[#080B12] text-[#4DD9AC]"
        style={{ fontFamily: "Inter,sans-serif" }}
      >
      ...
      <div className="shrink-0 bg-[#0A0D14] border-b border-[#1E2530] px-6 py-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${autoRefresh ? "bg-[#34D399] animate-pulse" : "bg-[#64748B]"}`}
              />
          ...
          <div className="grid grid-cols-4 gap-3 mt-3 pt-3 border-t border-[#1E2530]">
            {[
              ...
              },
              {
                ...
                color: activeIncidents > 0 ? "#EF4444" : "#34D399",
              },
              {
                label: "Loss est. prevented",
                value: "â‚¹2.4L",
                color: "#34D399",
              },
              {
        ...
        <div className="w-80 shrink-0 flex flex-col border-r border-[#1E2530] bg-[#0D1117] overflow-hidden">
          ...
          <div className="shrink-0 px-3 py-3 border-b border-[#1E2530] space-y-2">
            <input
              ...
              placeholder="ðŸ” Search shipments..."
          ...
          {selected ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
              ...
              <div className="text-sm text-[#64748B] max-w-xs">
                Click any card on the left to see the full intervention panel
                â€” risk breakdown, predictions, AI explanation, and actions.
              </div>
            </div>
          )}
        </div>
