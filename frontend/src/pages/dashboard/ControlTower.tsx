...
import {
  AICommandBar,
  type CommandResult,
} from "../../components/ui/AICommandBar";
import { SwarmActivityFeed } from "../../components/ui/SwarmActivityFeed";
import {
  Search,
  Bell,
  Package,
...
function useUser() {
  try {
    return JSON.parse(localStorage.getItem("cargofy_user") || "null");
  } catch {
    return null;
  }
}
...
export function ControlTower() {
  ...
  const [liveRiskEvent, setLiveRiskEvent] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const displayAlerts: Alert[] = React.useMemo(() => {
    ...
    const live = Object.entries(rtLiveAlerts).map(
      ([id, item]: [string, any]) => ({
        id,
  ...
  const aiActions = React.useMemo(() => {
    if (rtAiQueue) {
      const arr = Array.isArray(rtAiQueue)
        ? rtAiQueue
        : Object.values(rtAiQueue);
      return (arr as any[]).map((a: any) => ({
        id: a.id ?? Math.random().toString(),
        text: a.message ?? "",
        cta:
          a.action_type === "REROUTE"
            ? "Reroute Now"
            : a.action_type === "ALERT"
              ? "Alert Driver"
              : "Review",
    ...
    return displayShips
      .filter(
        (s) =>
          ["CRITICAL", "HIGH", "MEDIUM"].includes(getRisk(s)) &&
          !dismissed.includes(s.id),
      )
      .slice(0, 3)
      .map((s) => {
        const r = getRisk(s);
        const sp = s.current_risk?.time_to_spoil_minutes;
        return {
          id: s.id,
    ...
    .filter((s) => {
      ...
      if (aiFilter) {
        ...
        if (
          f.region &&
          !(
            s.origin?.toLowerCase().includes(f.region.toLowerCase()) ||
            s.destination?.toLowerCase().includes(f.region.toLowerCase())
          )
        )
          return false;
      }
  ...
  const watchlist =
    rtStats?.watchlist_count ??
    displayShips.filter((s) => ["HIGH", "MEDIUM"].includes(getRisk(s))).length;
  const savings =
    rtStats?.loss_prevented_today_inr ?? analytics?.estimated_savings_inr ?? 0;
  const savingsFmt =
    savings >= 100000
      ...
      : savings >= 1000
        ...
        : `₹${savings}`;
  const onTime =
    rtStats?.on_time_rate_7d ??
    (analytics
      ? Math.max(0, 94 - (analytics.high_risk_shipments ?? 0) * 2)
      : 94);
  const pendingAcks = displayShips.filter(
    (s) =>
      ["CRITICAL", "HIGH"].includes(getRisk(s)) && s.current_risk?.computed_at,
  );
  const mapShips = displayShips.filter((s) => {
    if (mapFilter === "critical") return getRisk(s) === "CRITICAL";
    if (mapFilter === "delayed")
      return (s.current_risk?.time_to_spoil_minutes ?? 9999) < 120;
    return true;
  });
  ...
  const handleCrisis = async (scenario: string) => {
    ...
    try {
      const data = await apiPost("/api/v1/agent/simulate-critical", {
        scenario,
        shipment_id: first?.shipment_code ?? "SHP-DEMO-001",
        product_type: first?.product_type ?? "milk",
      });
      if (data.agent_result?.should_reroute) {
        toast(
          `🤖 Agent: Rerouting → ${data.agent_result?.nearest_facility?.name ?? "nearest cold hub"}`,
          "warn",
        );
      } else {
          ...
          {toasts.map((t) => (
            <motion.div
              ...
              className={`px-4 py-3 rounded-xl text-sm font-semibold shadow-2xl border backdrop-blur-sm ${
                t.type === "ok"
                  ? "bg-[#0D2B22]/90 border-[#4DD9AC]/40 text-[#4DD9AC]"
                  : t.type === "err"
                    ? "bg-[#2B0D0D]/90 border-[#EF4444]/40 text-[#F87171]"
        ...
        <div className="relative group">
          ...
          >
            ...
            <span className="hidden sm:inline">
              {crisisLoading ? "AI Running…" : "AI Crisis Sim"}
            </span>
          </motion.button>
          <div className="absolute right-0 top-10 hidden group-hover:block bg-[#0D1117] border border-[#1E2530] rounded-xl shadow-2xl py-1 min-w-[200px] z-50 backdrop-blur">
            <div className="px-3 py-1.5 text-[9px] text-[#4A5568] uppercase tracking-widest border-b border-[#1E2530] mb-1">
              Gemini AI Scenarios
            </div>
            {[
        ...
        >
          <nav className="flex-1 overflow-y-auto py-3 space-y-0.5">
            ...
            <NavItem
              ...
              href="/risk"
              badge={critical.length + high.length}
              badgeColor="#EF4444"
            />
            <NavItem
              icon={<Zap size={16} />}
              ...
              <div className="flex items-center gap-2 min-w-0">
                {userAvatar ? (
                  <img
                    src={userAvatar}
                    alt={userName}
                    className="w-8 h-8 rounded-full object-cover border-2 border-[#4DD9AC]/40 shrink-0"
                  />
                ) : (
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#4DD9AC] to-[#34c994] flex items-center justify-center text-[#022518] text-xs font-black shrink-0">
                    {initials}
                  </div>
                )}
                <div className="min-w-0">
                  <div className="text-xs text-[#F1F5F9] font-semibold truncate">
                    {userName}
                  </div>
          ...
          <div className="shrink-0 bg-[#0D1117]/95 border-b border-[#1E2530]">
            {loading ? (
              <div className="flex items-center justify-center h-[72px] text-[#64748B] text-sm gap-2">
                <RefreshCw size={14} className="animate-spin" /> Loading live
                data…
              </div>
            ) : (
              <motion.div
                ...
                {[
                  {
                    icon: <Truck size={17} />,
                    label: "Active",
                    value: rtStats?.active_shipments ?? displayShips.length,
                    color: "#60A5FA",
                  },
                  {
                    icon: <ThermometerSnowflake size={17} />,
                    label: "Reefer Live",
                    value: rtStats?.live_reefer_vehicles ?? displayShips.length,
                    color: "#4DD9AC",
                    div: true,
                  },
                  {
                  ...
                  >
                    <div
                      className="text-[9px] uppercase tracking-widest mb-0.5 flex items-center gap-1"
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-[#F1F5F9] flex items-center gap-2">
                  ...
                  <span className="text-xs bg-[#1E2530] text-[#94A3B8] px-2 py-0.5 rounded-full">
              ...
              {loading ? (
                ...
                <div className="text-center py-12 text-[#64748B] flex flex-col items-center gap-2">
                  ...
                  <button
                    onClick={() => navigate("/create-shipment")}
                    className="text-[#4DD9AC] underline text-sm"
                  >
              ...
              <div className="bg-[#0D1117] border-b border-[#1E2530] px-4 py-2 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className="text-[10px] text-[#64748B] uppercase tracking-widest font-medium">
                    {mapMode === "3d"
                      ? "🌐 Cesium 3D Globe — Live Fleet"
                      : "🗺 2D Live Map — Live Fleet"}
                  </span>
          ...
          <RailSection title="Live Alerts" dot="red">
            {loading ? (
              ...
              <div className="flex items-center gap-1.5 text-xs text-[#64748B]">
                <CheckCircle size={12} className="text-[#4DD9AC]" />
                All nominal ✓
              </div>
            ) : (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-[10px] text-[#64748B]">
                          {timeAgo(s.current_risk?.computed_at)}
                        </span>
                      </div>
                      <div className="text-xs text-[#CBD5E1] mb-1 truncate">
                        {s.product_type} · {s.origin?.split(",")[0]} →{" "}
                        {s.destination?.split(",")[0]}
                      </div>
                      {s.current_risk?.time_to_spoil_minutes && (
                        <div className="text-[11px] text-[#FBBF24] mb-2">
                          Spoils in{" "}
                          {spoilFmt(s.current_risk.time_to_spoil_minutes)}
                        </div>
                      )}
                      <div className="flex gap-1.5">
                        <button
                          onClick={() => navigate(`/shipments/${s.id}`)}
                          className="text-[10px] bg-[#1E2530] text-[#94A3B8] hover:text-white px-2 py-1 rounded transition-colors"
                        >
                          View
                        </button>
                        <button
                        ...
                        <button
                          disabled={actionLoading === s.id + "_esc"}
            ...
            {displayShips.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {displayShips
                  .sort(
                    (a, b) =>
                      (b.current_risk?.risk_score ?? 0) -
                      (a.current_risk?.risk_score ?? 0),
                  )
                  .slice(0, 5)
                  .map((s) => {
                    const col = RISK_COLOR[getRisk(s)];
                    const score = Math.round(
                      (s.current_risk?.risk_score ?? 0) * 100,
                    );
                    return (
                      <button
                        key={s.id}
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-mono text-xs text-[#CBD5E1] group-hover:text-white transition-colors">
                            {s.shipment_code}
                          </span>
                          <span
              ...
              <div className="space-y-2">
                {pendingAcks.slice(0, 3).map((s) => {
                  ...
                  return (
                    <div
                      key={s.id}
                      className="bg-[#111827] rounded-lg p-2.5 border border-[#1E2530]"
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-[10px] text-[#64748B]">
                          {timeAgo(
                            al?.created_at ?? s.current_risk?.computed_at,
                          )}
                        </span>
                      </div>
                      <div className="text-[10px] text-[#94A3B8] mb-2">
                        Alert sent — awaiting acknowledgement
                      </div>
                      <div className="flex gap-1.5">
                        <button
                          disabled={
                            actionLoading === "resend_" + (al?.id ?? s.id)
                          }
                          onClick={() =>
                            handleResend(al?.id ?? s.id, s.shipment_code)
                          }
                        ...
                        <button
                          disabled={actionLoading === s.id + "_esc"}
                          onClick={() => handleEscalate(s)}
                          className="text-[10px] bg-[#EF4444]/10 text-[#F87171] px-2 py-1 rounded transition-colors disabled:opacity-50"
                        >
                          Escalate
                        </button>
                      </div>
                        ...
                        <button
                          onClick={() => {
                            if (a.cta === "Reroute Now") navigate("/risk");
                              ...
                              const s = displayShips.find(
                                (s) => s.shipment_code === a.shipmentCode,
                              );
...
function SectionLabel({ label }: { label: string }) {
  return (
    <div className="px-4 pt-3 pb-0.5">
      <span className="text-[9px] font-semibold text-[#3A4255] uppercase tracking-widest">
        {label}
      </span>
    </div>
  );
...
function NavItem({
  ...
  return (
    ...
    >
      <span
        className={`w-4 flex justify-center shrink-0 ${active ? "" : "group-hover:text-[#4DD9AC]"}`}
      >
        {icon}
      </span>
    ...
    >
      ...
      <div className="p-3 pb-2 border-b border-[#1E2530]">
        <div className="flex items-start justify-between mb-0.5">
          ...
          <div className="flex items-center gap-1.5">
            {s.product_type?.toLowerCase().includes("milk") ||
            s.product_type?.toLowerCase().includes("frozen") ? (
      ...
      <div className="p-3 space-y-2">
        <div className="grid grid-cols-2 gap-x-3 text-[10px]">
          ...
          <span className="text-xs text-[#CBD5E1] font-medium capitalize">
            {s.product_type ?? "—"}
          </span>
          <span className="text-xs text-[#CBD5E1] font-mono truncate">
            {s.vehicle_number ?? "—"}
          </span>
        </div>
        ...
        {s.current_risk && (
          <div className="grid grid-cols-2 gap-x-3 text-[10px]">
            ...
            <span
              className={`text-sm font-bold font-mono ${risk === "CRITICAL" ? "text-[#F87171]" : risk === "HIGH" ? "text-[#FBBF24]" : "text-[#CBD5E1]"}`}
            >
              {spoilFmt(s.current_risk.time_to_spoil_minutes)}
            </span>
            <span
