...
import {
  getShipments, simulatorEmit, sendSensor, computeRisk,
  type Shipment, type RiskResult,
} from '../lib/api';
...
interface GpsState { lat: number; lng: number; accuracy: number; }

// ─────────────────────────────────────────────────────────────────────────────
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
const TEMP_BANDS: Record<string, [number,number]> = {
  dairy:[2,6], milk:[2,6], seafood:[0,4], fish:[0,4], meat:[0,4],
  pharma:[2,8], frozen:[-20,-15], produce:[4,10], fruits:[5,12], other:[2,8],
};
...
function estimateRisk(s: SensorState, productType: string): number {
  ...
  const tempF    = s.temp > tMax ? Math.min(30, (s.temp - tMax) * 5) : s.temp < tMin ? Math.min(10, (tMin - s.temp) * 3) : 0;
  ...
  return Math.min(100, Math.round(13 + tempF + delayF + humF + doorF + reeferF));
}

function getRiskColor(score: number) {
  return score >= 75 ? '#EF4444' : score >= 50 ? '#F97316' : score >= 25 ? '#FBBF24' : '#34D399';
}
...
function BigSlider({
  ...
  safeMin, safeMax, warnAt, dangerAt, onChange,
}: {
  label: string; icon: string; value: number; min: number; max: number;
  step?: number; unit: string; safeMin?: number; safeMax?: number;
  warnAt?: number; dangerAt?: number; onChange: (v: number) => void;
}) {
    ...
    <div className="w-full">
      ...
      <div className="relative h-8 flex items-center">
        <div className="absolute inset-x-0 h-3 bg-[#1E2530] rounded-full overflow-hidden">
          ...
          {safeMin !== undefined && safeMax !== undefined && (
            <div className="absolute top-0 h-full border-x-2 border-[#34D399] bg-[#34D399]/10"
              style={{
      ...
      {safeMin !== undefined && safeMax !== undefined && (
        <div className="text-[10px] text-[#34D399] mt-1 text-right">
          Safe zone: {safeMin}–{safeMax}{unit}
        </div>
      )}
    </div>
  );
...
export function MobileIoTController() {
  ...
  const preselectedCode = (location.state as any)?.shipmentCode as string | undefined;
  ...
  const [sensors, setSensors] = useState<SensorState>({
    temp: 4.0, humidity: 60, doorClosed: true, delay: 0, reefer: 100,
  });
  ...
  const toggleGps = useCallback(() => {
    if (gpsEnabled) {
      ...
      gpsWatchRef.current = navigator.geolocation.watchPosition(
        ...
        () => { showToast('GPS not available', false); setGpsEnabled(false); },
  ...
  useEffect(() => {
    ...
    if (!autoTransmit) { setCountdown(30); return; }
    setCountdown(30);
    autoTimerRef.current = setInterval(() => {
      setCountdown(c => {
        if (c <= 1) { transmit(); return 30; }
        return c - 1;
      });
    }, 1000);
  ...
  return (
    <div className="min-h-screen bg-[#080B12] text-[#F1F5F9] flex flex-col"
      style={{ fontFamily: 'Inter, sans-serif', maxWidth: 480, margin: '0 auto' }}>

      {/* ── Toast ── */}
      {toast && (
        <div className={`fixed top-4 left-4 right-4 z-[100] px-4 py-3 rounded-xl text-sm font-semibold text-center shadow-2xl transition-all ${
          toast.ok ? 'bg-[#0D2B22] border border-[#4DD9AC]/40 text-[#4DD9AC]'
      ...
      <header className="sticky top-0 z-40 bg-[#0A0D14]/95 backdrop-blur border-b border-[#1E2530] px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-[#4DD9AC] font-black text-lg tracking-tighter font-mono">CARGOFY</span>
            <div className="flex items-center gap-1.5 bg-[#4DD9AC]/10 border border-[#4DD9AC]/25 px-2.5 py-1 rounded-full">
              ...
              <span className="text-[10px] font-bold text-[#4DD9AC] uppercase tracking-widest">IoT Node</span>
            </div>
          </div>
          <div className="flex items-center gap-3 text-xs text-[#64748B]">
            {phoneBattery !== null && (
              <div className="flex items-center gap-1">
                <span>🔋</span>
                <span className={`font-mono font-bold ${phoneBattery < 20 ? 'text-[#EF4444]' : phoneBattery < 40 ? 'text-[#FBBF24]' : 'text-[#34D399]'}`}>
                  {phoneBattery}%
                </span>
              </div>
            )}
            {bump && <span className="text-[#FBBF24] animate-bounce font-bold text-base">📳</span>}
          </div>
        </div>
        {/* Hardware info line */}
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-32 pt-4 space-y-4">
        ...
        <div className="rounded-2xl border border-[#1E2530] bg-[#111827] p-4">
          <div className="text-[10px] text-[#64748B] uppercase tracking-widest font-bold mb-2">Active Shipment</div>
          <select
            value={selShip?.id ?? ''}
            onChange={e => {
          ...
          {selShip && (
            <div className="flex items-center justify-between mt-2 text-xs text-[#64748B]">
              <span>{selShip.origin?.split(',')[0]} → {selShip.destination?.split(',')[0]}</span>
              <span className="text-[#34D399] font-mono">Safe: {tMin}°C–{tMax}°C</span>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {/* Temp */}
          <div className={`rounded-2xl border p-3 text-center ${isBreach ? 'border-[#EF4444]/40 bg-[#2B0D0D]' : 'border-[#1E2530] bg-[#111827]'}`}>
            <div className="text-lg mb-1">🌡️</div>
            <div className="font-mono text-xl font-black" style={{ color: isBreach ? '#EF4444' : sensors.temp > tMax * 0.8 ? '#FBBF24' : '#4DD9AC' }}>
              {sensors.temp.toFixed(1)}
            </div>
            <div className="text-[9px] text-[#64748B] mt-0.5">°C · DS18B20</div>
          </div>
          {/* Humidity */}
          <div className={`rounded-2xl border p-3 text-center ${sensors.humidity > 80 ? 'border-[#EF4444]/40 bg-[#2B0D0D]' : 'border-[#1E2530] bg-[#111827]'}`}>
            <div className="text-lg mb-1">💧</div>
            <div className="font-mono text-xl font-black" style={{ color: sensors.humidity > 80 ? '#EF4444' : sensors.humidity > 70 ? '#FBBF24' : '#60A5FA' }}>
              {sensors.humidity}
            </div>
            <div className="text-[9px] text-[#64748B] mt-0.5">% · AM2302</div>
          </div>
          {/* Door */}
          <div className={`rounded-2xl border p-3 text-center ${!sensors.doorClosed ? 'border-[#FBBF24]/40 bg-[#2B1F0D]' : 'border-[#1E2530] bg-[#111827]'}`}>
            <div className="text-lg mb-1">🚪</div>
            <div className="font-mono text-sm font-black" style={{ color: sensors.doorClosed ? '#34D399' : '#FBBF24' }}>
        ...
        <div className="rounded-2xl border border-[#1E2530] bg-[#111827] p-5 flex items-center gap-5">
          ...
          <div className="relative flex-shrink-0" style={{ width: 120, height: 120 }}>
            <svg viewBox="0 0 120 120" width={120} height={120}>
              <circle cx={60} cy={60} r={RADIUS} fill="none" stroke="#1E2530" strokeWidth={10}/>
              <circle cx={60} cy={60} r={RADIUS} fill="none"
                stroke={riskColor} strokeWidth={10}
                strokeDasharray={`${dash} ${CIRCUM}`}
                strokeLinecap="round"
                transform="rotate(-90 60 60)"
                style={{ filter: `drop-shadow(0 0 8px ${riskColor}60)`, transition: 'stroke-dasharray 0.8s ease, stroke 0.5s' }}
              />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="font-black text-2xl font-mono" style={{ color: riskColor }}>{finalScore}</span>
              <span className="text-[9px] text-[#64748B]">/ 100</span>
            </div>
          </div>
          {/* Risk text */}
          <div className="flex-1">
            <div className="text-[10px] text-[#64748B] uppercase tracking-widest mb-1">Risk Level</div>
            <div className="text-2xl font-black mb-1" style={{ color: riskColor }}>{riskLabel}</div>
            {apiRisk?.time_to_spoil_minutes && (
              <div className="text-xs text-[#64748B]">
                Spoils in <span className="text-[#FBBF24] font-mono font-bold">
                  {Math.floor(apiRisk.time_to_spoil_minutes / 60)}h {apiRisk.time_to_spoil_minutes % 60}m
                </span>
              </div>
            )}
            <div className={`text-xs font-bold mt-2 ${finalScore > 50 ? 'text-[#F87171]' : finalScore > 25 ? 'text-[#FBBF24]' : 'text-[#34D399]'}`}>
        ...
        <div className="rounded-2xl border border-[#1E2530] bg-[#111827] p-4 space-y-5">
          <div className="text-[10px] text-[#64748B] uppercase tracking-widest font-bold">Sensor Controls</div>

          <BigSlider icon="🌡️" label="Cargo Temp (DS18B20+)" value={sensors.temp}
            min={-5} max={25} step={0.1} unit="°C"
            safeMin={tMin} safeMax={tMax} warnAt={tMax} dangerAt={tMax + 2}
            onChange={v => setSensors(p => ({ ...p, temp: v }))} />

          <BigSlider icon="💧" label="Humidity (AM2302/DHT22)" value={sensors.humidity}
            min={30} max={100} step={1} unit="%"
            warnAt={75} dangerAt={85}
            onChange={v => setSensors(p => ({ ...p, humidity: v }))} />

          <BigSlider icon="❄️" label="Reefer Health" value={sensors.reefer}
            min={0} max={100} step={1} unit="%"
            safeMin={80} safeMax={100} warnAt={79} dangerAt={50}
            onChange={v => setSensors(p => ({ ...p, reefer: v }))} />

          <BigSlider icon="⏱️" label="Transit Delay" value={sensors.delay}
            min={0} max={120} step={1} unit="m"
            warnAt={20} dangerAt={60}
            onChange={v => setSensors(p => ({ ...p, delay: v }))} />

          {/* Door Reed Switch toggle */}
          <div className="flex items-center justify-between">
            <span className="text-sm text-[#94A3B8] font-medium">🚪 Door (Reed Switch MK24)</span>
            <button
              onClick={() => setSensors(p => ({ ...p, doorClosed: !p.doorClosed }))}
              className="relative flex items-center"
            >
              <div className={`w-16 h-8 rounded-full border-2 transition-all duration-300 flex items-center px-1 ${
                sensors.doorClosed
              ...
              <span className={`ml-2 text-xs font-bold w-14 ${sensors.doorClosed ? 'text-[#34D399]' : 'text-[#FBBF24]'}`}>
        ...
        <div className="rounded-2xl border border-[#1E2530] bg-[#111827] p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="text-[10px] text-[#64748B] uppercase tracking-widest font-bold">📍 GPS Location</div>
            <button
              onClick={toggleGps}
          ...
          {gps ? (
            <div className="grid grid-cols-2 gap-2 text-xs font-mono">
              <div className="bg-[#0D1117] rounded-xl p-2">
                ...
                <div className="text-[#4DD9AC] font-bold">{gps.lat.toFixed(5)}</div>
              </div>
              <div className="bg-[#0D1117] rounded-xl p-2">
                <div className="text-[#64748B] text-[9px] mb-1">LNG</div>
                <div className="text-[#4DD9AC] font-bold">{gps.lng.toFixed(5)}</div>
              </div>
              <div className="col-span-2 bg-[#0D1117] rounded-xl p-2 flex items-center gap-2">
                ...
                <span className="text-[#34D399]">±{gps.accuracy.toFixed(0)}m</span>
              </div>
            </div>
          ) : (
            <div className="text-xs text-[#4A5568] text-center py-2">
              {gpsEnabled ? 'Acquiring GPS fix…' : 'Press Start GPS to use real phone location'}
            </div>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-[#1E2530] bg-[#111827] p-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-[#F1F5F9]">Auto-Transmit</div>
              <div className="text-[10px] text-[#64748B] mt-0.5">Every 30 seconds to Cargofy</div>
            </div>
            <button
              onClick={() => setAutoTransmit(p => !p)}
              className={`relative w-14 h-7 rounded-full border-2 transition-all duration-300 ${
                autoTransmit ? 'bg-[#4DD9AC]/20 border-[#4DD9AC]' : 'bg-[#1E2530] border-[#374151]'
              }`}
            >
              <div className={`absolute top-0.5 w-5 h-5 rounded-full transition-all ${
                autoTransmit ? 'translate-x-7 bg-[#4DD9AC]' : 'translate-x-0.5 bg-[#374151]'
              }`}/>
            </button>
          </div>
          {autoTransmit && (
            <div className="mt-3 flex items-center gap-3">
              <div className="flex-1 h-1.5 bg-[#1E2530] rounded-full overflow-hidden">
                <div className="h-full bg-[#4DD9AC] rounded-full transition-all duration-1000"
                  style={{ width: `${((30 - countdown) / 30) * 100}%` }}/>
              </div>
              <span className="text-xs font-mono text-[#4DD9AC] font-bold w-8 text-right">{countdown}s</span>
            </div>
          )}
          {lastTx && (
            <div className="mt-2 text-[10px] text-[#64748B]">
              Last TX: <span className="text-[#34D399] font-mono">{lastTx}</span>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 p-4 bg-[#080B12]/95 backdrop-blur border-t border-[#1E2530]"
