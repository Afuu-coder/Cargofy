...
import { Eye, EyeOff, Loader, AlertCircle, ArrowRight, Shield, Zap, Globe } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#050810] flex overflow-hidden">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[48%] relative p-14 overflow-hidden">
        ...
        {/* Glow orb */}
        <div className="absolute top-[38%] left-[45%] w-72 h-72 rounded-full pointer-events-none" style={{
          background: 'radial-gradient(circle, rgba(77,217,172,0.07) 0%, transparent 70%)', filter: 'blur(60px)'
        }} />
        ...
        <div className="relative z-10">
          ...
          <div className="mb-14">
            ...
            <p className="text-slate-400 text-[15px] leading-relaxed max-w-[300px]">
              AI-powered logistics that monitors, predicts, and autonomously reroutes cargo — in under 2 seconds.
            </p>
          </div>

          <div className="space-y-3">
            {[
              { icon: <Zap size={13} />, label: 'Gemini AI risk scoring in real-time', c: '#4DD9AC' },
              { icon: <Globe size={13} />, label: '3D fleet tracking across India', c: '#818cf8' },
              { icon: <Shield size={13} />, label: 'Blockchain-certified audit trail', c: '#F59E0B' },
            ].map((f, i) => (
              <div key={i} className="flex items-center gap-3 px-4 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.055)' }}>
                <div className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: `${f.c}15`, color: f.c }}>{f.icon}</div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-14 bg-[#080C16]">
        <div className="w-full max-w-[390px]">
          ...
          <p className="text-slate-500 text-sm mb-8">Sign in to your operations dashboard</p>
          ...
          <form onSubmit={handleEmail} className="space-y-4">
            <div>
              <label className="block text-[11px] text-slate-500 uppercase tracking-wider mb-1.5 font-semibold">Email</label>
          ...
          <p className="text-center text-slate-600 text-[13px] mt-7">
            ...
            <Link to="/signup" className="text-[#4DD9AC] hover:text-[#6ef6c7] font-semibold transition-colors">Create one</Link>
          </p>
