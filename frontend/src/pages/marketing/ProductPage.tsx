...
import { 
  Menu, X, ChevronDown, ArrowRight, PlayCircle, Map, PlusSquare, 
  MapPin, BrainCircuit, Cpu, MessageSquareWarning, Warehouse, 
  LineChart, FileCheck, Truck, CheckCircle2, XCircle, Server, Shield, Zap
} from 'lucide-react';
...
export const ProductPage: React.FC = () => {
  ...
  return (
    <div style={{ backgroundColor: COLORS.bg, color: COLORS.textMain, fontFamily: FONTS.body, minHeight: '100vh' }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden md:flex items-center space-x-8">
              <Link to="/product" className="text-white border-b-2 border-[#4DD9AC] pb-1 text-sm font-medium">Platform</Link>
          ...
          <div className="min-w-[800px] flex flex-col gap-12 items-center">
            ...
            <div className="flex items-center gap-4 justify-between w-full">
              {[
                ...
                { id: 'tracking', title: 'MONITOR', desc: 'Live IoT + AI risk' },
                { id: 'alerts', title: 'INTERVENE', desc: 'Risk alerts + reroute' }
              ].map((step, i) => (
                <React.Fragment key={step.id}>
                  <button onClick={() => scrollToModule(step.id)} className="flex flex-col items-center p-6 border border-[#1E293B] rounded-xl bg-[#0A0D14] hover:border-[#4DD9AC] hover:-translate-y-1 transition-all flex-1 group">
                    <span className="text-[#4DD9AC] font-bold tracking-widest text-sm mb-2">{step.title}</span>
                    <span className="text-[#94A3B8] text-center text-sm">{step.desc}</span>
                  </button>
                  {i < 3 && <ArrowRight className="w-6 h-6 text-[#1E293B] flex-shrink-0" />}
                </React.Fragment>
              ))}
            </div>
            ...
            <div className="flex items-center gap-4 justify-between w-full">
              {[
                { id: 'compliance', title: 'DELIVER', desc: 'Proof of delivery' },
                ...
                <React.Fragment key={step.id}>
                  ...
                  {i < 3 && <div className="text-[#1E293B] text-2xl font-bold flex-shrink-0">←</div>}
                </React.Fragment>
              ))}
            </div>
          </div>
      ...
      <section className="py-24 bg-[#080B12]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-32">
          ...
          {MODULES.map((mod, index) => {
            ...
            return (
              <div id={`module-${mod.id}`} key={mod.id} className={`flex flex-col ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-16 items-center scroll-mt-32`}>
                ...
                <div className="lg:w-1/2">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-[#94A3B8] font-mono text-sm tracking-widest uppercase">Module {index + 1}: {mod.name}</h2>
                  </div>
                  
                  <h3 style={{ fontFamily: FONTS.display }} className="text-3xl md:text-4xl font-bold text-white mb-6 leading-tight">
                    {mod.title}
                  </h3>
                  ...
                  <ul className="space-y-4 mb-8">
                    {mod.bullets.map((bullet, i) => (
                      <li key={i} className="flex items-start">
                        <ArrowRight className="w-5 h-5 mr-3 mt-1 flex-shrink-0" style={{ color: mod.color }} />
                  ...
                  {mod.chips.length > 0 && (
                    <div className="flex flex-wrap gap-3 mb-10">
                      {mod.chips.map((chip, i) => (
                        <div key={i} className="bg-[#111622] border border-[#1E293B] text-[#94A3B8] text-sm px-4 py-2 rounded-full flex items-center">
                  ...
                  <div className="pt-6 border-t border-[#1E293B]">
                    <Link to="/signup" className="inline-flex items-center font-bold text-lg hover:opacity-80 transition-opacity" style={{ color: mod.color }}>
                ...
                <div className="lg:w-1/2 w-full">
                  <div className="aspect-square md:aspect-video lg:aspect-[4/3] w-full rounded-2xl border border-[#1E293B] bg-[#0A0D14] shadow-2xl relative overflow-hidden flex items-center justify-center group">
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 blur-[100px] rounded-full opacity-10 transition-opacity duration-700 group-hover:opacity-20 pointer-events-none" style={{ backgroundColor: mod.color }}></div>
                    ...
                    <div className="absolute inset-4 border border-[#1E293B] rounded-xl bg-[#111622] overflow-hidden flex flex-col">
                      ...
                      <div className="flex-1 p-6 relative flex flex-col gap-4">
                         ...
                         <div className="flex gap-4 mb-2">
                           ...
                           <div className="h-24 w-1/3 rounded-lg border border-[#1E293B] bg-[#080B12] p-4 flex flex-col justify-between">
                             <div className="w-8 h-8 rounded-full" style={{ backgroundColor: `${mod.color}30` }}></div>
                             <div className="w-2/3 h-2 rounded" style={{ backgroundColor: mod.color }}></div>
                           </div>
                         </div>
                         
                         <div className="flex-1 rounded-lg border border-[#1E293B] bg-[#080B12] relative overflow-hidden flex items-center justify-center">
                            <Icon className="w-32 h-32 opacity-10 absolute" style={{ color: mod.color }} />
                            
                            {/* Animated skeleton rows */}
                            <div className="w-full h-full p-6 flex flex-col gap-4 justify-center">
                              {[1, 2, 3, 4].map(row => (
                                <div key={row} className="w-full h-8 flex gap-4 items-center">
                                  ...
                                  <div className="w-16 h-4 rounded-full" style={{ backgroundColor: row === 2 ? `${mod.color}50` : '#1E293B' }}></div>
                                </div>
                              ))}
                            </div>
                         </div>
      ...
      <section className="py-24 border-y border-[#1E293B]" style={{ backgroundColor: '#05070A' }}>
                ...
                {[
                  ...
                  <tr key={idx} className="hover:bg-[#111622] transition-colors">
                    ...
                    <td className="p-6 border-l border-[#1E293B] text-center">
                      {row.c1 === 'yes' ? <CheckCircle2 className="w-6 h-6 text-[#94A3B8] mx-auto opacity-50" /> : 
                       row.c1 === 'partial' ? <span className="text-[#FFB020] font-bold">Partial</span> : 
                       <XCircle className="w-6 h-6 text-[#FF4D4D] mx-auto opacity-50" />}
                    </td>
                    <td className="p-6 border-l border-[#1E293B] text-center">
                      {row.c2 === 'yes' ? <CheckCircle2 className="w-6 h-6 text-[#94A3B8] mx-auto opacity-50" /> : 
                       row.c2 === 'partial' ? <span className="text-[#FFB020] font-bold">Partial</span> : 
                       <XCircle className="w-6 h-6 text-[#FF4D4D] mx-auto opacity-50" />}
                    </td>
                  </tr>
                ))}
      ...
      <section className="py-24 bg-[#080B12]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-16">
            ...
            <p className="text-xl text-[#94A3B8]">Technical specifications for IT & Engineering leadership.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            <div className="p-8 border border-[#1E293B] rounded-xl bg-[#111622]">
              <Server className="w-10 h-10 text-[#4DD9AC] mb-6" />
              <h3 className="text-xl font-bold text-white mb-6 uppercase tracking-wider font-mono">Architecture</h3>
              <ul className="space-y-4 text-[#94A3B8]">
                <li><strong className="text-white">Cloud-native:</strong> runs on Google Cloud Platform</li>
                <li><strong className="text-white">Real-time pipeline:</strong> Pub/Sub → Dataflow → Firebase</li>
                <li><strong className="text-white">AI/ML:</strong> Vertex AI for risk scoring & anomaly detection</li>
                <li><strong className="text-white">API-first:</strong> REST API + Webhooks for all events</li>
                <li><strong className="text-white">IoT:</strong> MQTT / HTTP ingestion endpoints</li>
                <li><strong className="text-white">Integrations:</strong> SAP, Oracle, Tally, WhatsApp, Maps</li>
              </ul>
            </div>

            <div className="p-8 border border-[#1E293B] rounded-xl bg-[#111622]">
              <Zap className="w-10 h-10 text-[#FFB020] mb-6" />
              <h3 className="text-xl font-bold text-white mb-6 uppercase tracking-wider font-mono">Performance</h3>
              <ul className="space-y-4 text-[#94A3B8]">
                <li><strong className="text-white">Ingestion latency:</strong> &lt; 3 seconds</li>
                <li><strong className="text-white">Risk score updates:</strong> every 60 seconds</li>
                <li><strong className="text-white">Alert delivery:</strong> &lt; 90 seconds from trigger</li>
                <li><strong className="text-white">Map updates:</strong> every 30 seconds</li>
                <li><strong className="text-white">Uptime SLA:</strong> 99.9% guarantee</li>
              </ul>
            </div>

            <div className="p-8 border border-[#1E293B] rounded-xl bg-[#111622]">
              <Shield className="w-10 h-10 text-[#3B82F6] mb-6" />
              <h3 className="text-xl font-bold text-white mb-6 uppercase tracking-wider font-mono">Security</h3>
              <ul className="space-y-4 text-[#94A3B8]">
                <li><strong className="text-white">Encryption:</strong> TLS 1.3 (transit) & AES-256 (rest)</li>
                <li><strong className="text-white">Access:</strong> Role-based access control (RBAC)</li>
                <li><strong className="text-white">Compliance:</strong> SOC 2 Type II (in process)</li>
                <li><strong className="text-white">Privacy:</strong> GDPR compliant for EU customers</li>
                <li><strong className="text-white">Audit:</strong> Full immutable logs for user actions</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <section className="py-32 border-t border-[#1E293B] relative overflow-hidden" style={{ backgroundColor: '#05070A' }}>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
          <h2 style={{ fontFamily: FONTS.display }} className="text-4xl md:text-6xl font-bold text-white mb-8 leading-tight">
            Everything Your Cold Chain Needs. <br/>In One Platform.
          </h2>
          <div className="flex flex-col sm:flex-row justify-center gap-6">
            ...
            <Link to="/pricing" style={{ borderColor: COLORS.border }} className="bg-[#111622] border text-white px-8 py-4 rounded-md font-bold text-lg hover:bg-[#1A2133] transition-colors">
              View Pricing
            </Link>
            <Link to="/about" style={{ borderColor: COLORS.border }} className="bg-transparent border text-white px-8 py-4 rounded-md font-bold text-lg hover:bg-[#111622] transition-colors">
              Read Documentation
            </Link>
          </div>
        </div>
