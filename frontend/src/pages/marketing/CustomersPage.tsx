...
import { 
  Menu, X, ChevronDown, ArrowRight, Quote, CheckCircle2, TrendingDown, Clock, ShieldCheck
} from 'lucide-react';
...
const CASE_STUDIES = [
  {
    ...
    quote: "We went from finding out about a breach 3 hours later to getting alerted in under 4 minutes. That changed everything for our NE India dairy network.",
    author: "VP Operations",
    featured: true,
    challenge: "Temperature breaches going undetected for hours on the Guwahati to Shillong route. By the time the destination hub found out, the entire load of fresh milk and paneer had spoiled. The existing GPS trackers only showed location, not reefer health.",
    solution: [
...
export const CustomersPage: React.FC = () => {
  ...
  const filteredStudies = filter === 'All' ? CASE_STUDIES : CASE_STUDIES.filter(c => c.industry === filter);

  // If a slug is provided, render the Detail View
  if (slug) {
    const study = CASE_STUDIES.find(c => c.id === slug);
    if (!study) return <div className="p-20 text-white text-center">Case Study not found. <Link to="/customer-stories" className="text-[#4DD9AC] underline">Go back</Link></div>;
      ...
      <div style={{ backgroundColor: COLORS.bg, color: COLORS.textMain, fontFamily: FONTS.body, minHeight: '100vh' }}>
        ...
        <section className="pt-32 pb-16 lg:pt-48 lg:pb-16 border-b border-[#1E293B]">
          <div className="max-w-4xl mx-auto px-4">
            <Link to="/customer-stories" className="text-[#94A3B8] hover:text-white mb-8 inline-block transition-colors font-mono text-sm">← Back to all stories</Link>
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h1 style={{ fontFamily: FONTS.display }} className="text-3xl md:text-5xl font-bold text-white mb-2">{study.name}</h1>
                <div className="flex gap-3 text-[#94A3B8] text-sm font-mono">
                  <span>{study.industry}</span> • <span>{study.size}</span> • <span>{study.region}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="py-16">
          <div className="max-w-4xl mx-auto px-4 space-y-16">
            ...
            <div>
              <h2 className="text-sm font-bold text-[#4DD9AC] uppercase tracking-widest mb-4">The Challenge</h2>
              <p className="text-[#94A3B8] text-lg leading-relaxed mb-6">{study.challenge}</p>
            </div>
            ...
            <div className="p-8 border border-[#1E293B] rounded-2xl bg-[#0A0D14]">
              <h2 className="text-sm font-bold text-[#4DD9AC] uppercase tracking-widest mb-8">The Results</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div className="bg-[#111622] border border-[#1E293B] rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-white mb-1">{study.results.lossRed}</div>
                  <div className="text-[#94A3B8] text-xs uppercase">Loss Reduction</div>
                </div>
                <div className="bg-[#111622] border border-[#1E293B] rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-[#4DD9AC] mb-1">{study.results.saved}</div>
                  <div className="text-[#94A3B8] text-xs uppercase">Saved/mo</div>
                </div>
                <div className="bg-[#111622] border border-[#1E293B] rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-white mb-1">{study.results.alert}</div>
                  <div className="text-[#94A3B8] text-xs uppercase">Alert Time</div>
                </div>
                <div className="bg-[#111622] border border-[#1E293B] rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-white mb-1">{study.results.comp}</div>
                  <div className="text-[#94A3B8] text-xs uppercase">Compliance</div>
                </div>
              </div>
            </div>

            <div className="border-l-4 border-[#4DD9AC] pl-8 py-2">
              <p className="text-2xl text-white italic leading-relaxed mb-6">"{study.longQuote}"</p>
              <div className="text-[#94A3B8] font-mono text-sm">— {study.author}, {study.name}</div>
            </div>

            <div className="pt-8 border-t border-[#1E293B] flex items-center gap-4">
              <span className="text-[#94A3B8] text-sm font-bold uppercase tracking-widest">Share:</span>
              <button className="text-[#94A3B8] hover:text-[#4DD9AC] font-medium text-sm transition-colors border border-[#1E293B] px-4 py-2 rounded">Twitter/X</button>
              <button className="text-[#94A3B8] hover:text-[#4DD9AC] font-medium text-sm transition-colors border border-[#1E293B] px-4 py-2 rounded">LinkedIn</button>
            </div>
        ...
        <section className="py-24 border-t border-[#1E293B] bg-[#0A0D14] text-center">
          <div className="max-w-3xl mx-auto px-4">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-6">
              ...
              <Link to="/pricing" className="bg-[#111622] border border-[#1E293B] text-white px-8 py-4 rounded-md font-bold text-lg hover:bg-[#1A2133] transition-colors">
                See Pricing
              </Link>
            </div>
          </div>
    ...
    <div style={{ backgroundColor: COLORS.bg, color: COLORS.textMain, fontFamily: FONTS.body, minHeight: '100vh' }}>
      ...
      <section className="pt-32 pb-16 lg:pt-48 lg:pb-24 border-b border-[#1E293B] text-center">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center gap-4 md:gap-8 font-mono text-sm font-bold text-white uppercase tracking-wider">
            ...
            <span className="bg-[#111622] border border-[#1E293B] px-4 py-2 rounded-full">2.4M shipments monitored</span>
            <span className="bg-[#111622] border border-[#1E293B] px-4 py-2 rounded-full">14 states</span>
            <span className="bg-[#111622] border border-[#1E293B] px-4 py-2 rounded-full">94.8% cold compliance</span>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 border-b border-[#1E293B] bg-[#0A0D14]">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-xs font-bold text-[#94A3B8] uppercase tracking-widest mb-8">Trusted by India's Leading Cold Chain Operations</p>
          <div className="flex flex-wrap justify-center gap-12 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
             {/* Simulating logos with stylized text */}
             <div className="text-2xl font-bold font-serif">AMULYA</div>
             <div className="text-2xl font-bold font-mono">B-PHARMA</div>
             <div className="text-2xl font-bold italic">FreshCatch</div>
             <div className="text-2xl font-bold tracking-widest">SNOWLOG</div>
             <div className="text-2xl font-bold font-serif">Natura</div>
          </div>
        </div>
      </section>
          ...
          <div className="border border-[#1E293B] rounded-2xl bg-[#111622] relative overflow-hidden flex flex-col md:flex-row group">
            ...
            <div className="p-8 md:p-12 md:w-2/3 z-10">
              <div className="flex items-center gap-4 mb-8">
                <div className="w-12 h-12 bg-[#1E293B] rounded flex items-center justify-center font-bold text-white">{featured.logo}</div>
                <div className="text-[#4DD9AC] font-bold text-sm tracking-widest uppercase">FEATURED STORY</div>
              </div>
              <p className="text-2xl md:text-3xl text-white italic leading-relaxed mb-6 font-light">
                "{featured.quote}"
              </p>
              <div className="text-[#94A3B8] font-mono text-sm mb-12">— {featured.author}, {featured.name}</div>
              
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                <div className="border border-[#1E293B] bg-[#0A0D14] rounded p-4 text-center">
                  <div className="text-2xl font-bold text-white mb-1">{featured.results.lossRed}</div>
                  <div className="text-[#94A3B8] text-xs uppercase">Loss Red.</div>
                </div>
                <div className="border border-[#1E293B] bg-[#0A0D14] rounded p-4 text-center">
                  <div className="text-2xl font-bold text-[#4DD9AC] mb-1">{featured.results.saved}</div>
                  <div className="text-[#94A3B8] text-xs uppercase">Saved/mo</div>
                </div>
                <div className="border border-[#1E293B] bg-[#0A0D14] rounded p-4 text-center">
                  <div className="text-2xl font-bold text-white mb-1">{featured.results.alert}</div>
                  <div className="text-[#94A3B8] text-xs uppercase">Alert Resp.</div>
                </div>
                <div className="border border-[#1E293B] bg-[#0A0D14] rounded p-4 text-center">
                  <div className="text-2xl font-bold text-white mb-1">{featured.results.comp}</div>
                  <div className="text-[#94A3B8] text-xs uppercase">Compliance</div>
                </div>
              </div>

              <div className="flex items-center gap-6">
                <Link to={`/customer-stories/${featured.id}`} className="inline-flex items-center text-[#4DD9AC] font-bold hover:text-white transition-colors">
                  Read Full Story <ArrowRight className="ml-2 w-5 h-5" />
                </Link>
                <div className="text-[#94A3B8] text-sm border-l border-[#1E293B] pl-6">Industry: {featured.industry}</div>
              </div>
            </div>

            {/* Right/Visual */}
            <div className="md:w-1/3 bg-[#05070A] border-l border-[#1E293B] p-8 flex items-center justify-center relative">
              <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(circle at center, #4DD9AC 1px, transparent 1px)', backgroundSize: '20px 20px' }}></div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {filteredStudies.map((study) => (
              <div key={study.id} className="border border-[#1E293B] bg-[#111622] rounded-xl p-8 hover:border-[#4DD9AC]/50 transition-colors flex flex-col h-full">
                <div className="flex justify-between items-start mb-6">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-[#1E293B] rounded flex items-center justify-center font-bold text-white text-xs">{study.logo}</div>
                    <div className="text-white font-bold">{study.name}</div>
                  </div>
                  <div className="text-[#94A3B8] text-xs font-mono border border-[#1E293B] px-2 py-1 rounded">{study.industry}</div>
                </div>

                <div className="mb-6 flex-1">
                  <div className="text-[#4DD9AC] text-xs font-bold uppercase tracking-widest mb-2">Challenge:</div>
                  <p className="text-[#94A3B8] italic">"{study.challenge.substring(0, 100)}..."</p>
                </div>

                <div className="mb-8">
                  <div className="text-[#4DD9AC] text-xs font-bold uppercase tracking-widest mb-3">Result:</div>
                  <div className="space-y-2 text-sm text-white">
                    <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#FF4D4D]"></div> Before: High unmonitored risk</div>
                    <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#4DD9AC]"></div> After: {study.results.comp} Compliance</div>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-[#1E293B] mb-6 flex justify-between font-mono text-sm text-white">
                  ...
                  <span className="text-[#4DD9AC]">{study.results.lossRed} loss reduction</span>
                </div>

                <Link to={`/customer-stories/${study.id}`} className="mt-auto inline-flex items-center text-[#94A3B8] hover:text-[#4DD9AC] font-bold transition-colors">
      ...
      <section className="py-24 border-t border-[#1E293B] bg-[#080B12]">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            ...
            <div>
              <div style={{ fontFamily: FONTS.display }} className="text-5xl font-bold text-[#4DD9AC] mb-2">₹42Cr+</div>
              <div className="text-[#94A3B8]">Loss Prevented</div>
            </div>
            <div>
              <div style={{ fontFamily: FONTS.display }} className="text-5xl font-bold text-white mb-2">14</div>
              <div className="text-[#94A3B8]">States Covered</div>
            </div>
            <div>
              <div style={{ fontFamily: FONTS.display }} className="text-5xl font-bold text-white mb-2">67%</div>
              <div className="text-[#94A3B8]">Avg. Reduction in Excursion Time</div>
            </div>
          </div>
        </div>
      </section>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex justify-between items-center h-20">
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <Link to="/customer-stories" className="text-white border-b-2 border-[#4DD9AC] pb-1 text-sm font-medium">Customers</Link>
